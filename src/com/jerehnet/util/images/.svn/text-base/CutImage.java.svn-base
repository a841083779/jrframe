...
public class CutImage {
	...
	private String subpath = ""; // 剪切图片存放的目录
	...
	private CutImage(String srcpath, String subpath, int x, int y, int width,
			int height) {
		...
		this.width = width;
		this.height = height;
		this.extName = srcpath.substring(srcpath.lastIndexOf(".") + 1);
	}
	...
	private void cut() throws IOException {
		...
		try {
			...
			// 保存新图片
			ImageIO.write(bi, extName, new File(subpath));
		} finally {
			if (is != null)
				is.close();
			if (iis != null)
	 ...
	 *            高度
	 */
	public static void doCut(String srcpath, String subpath, int x, int y,
			int width, int height) throws IOException {
		CutImage ci = new CutImage(srcpath, subpath, x, y, width, height);
		ci.cut();
	}
