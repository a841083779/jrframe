...
<%
	...
	int score3 = 0;
	int score4 = 0;
	Map productMap = dbHelper.getMap("select * from pro_products where id=" + product_id + " order by id desc");
	if(productMap!=null){
		...
		contact_address = CommonString.getFormatPara(productMap.get("contact_address"));
	...
	if(!"".equals(product_id)){
		whereStr.append(" and pp.id="+product_id) ;
	}
	...
	pageBean.setFields(" pm.id,pm.score1,pm.score2,pm.score3,pm.score4,pm.content,pm.name contact_address,pm.product_id,pp.factoryid,pp.factoryname,pp.catalognum,pp.catalogname,pp.name product_name,pp.img2,pp.file_name,pm.add_date,pm.is_help,pm.username,pp.view_count,pp.catalognum") ;
...
<div class="contain980 mb10">
  <h3 class="breadCrumbs"><a href="/" title="铁臂口碑">铁臂口碑</a>
&gt;&gt;&nbsp;<a href="/0-<%=factoryid %>-0.htm"><%=CommonString.getFormatPara(brandMap.get(factoryid))%></a>
&gt;&gt;&nbsp;<a href="/<%=catalog.substring(0,6) %>-0-0.htm"><%=CommonString.getFormatPara(catalogMap.get(catalog.substring(0,6))) %></a>
&gt;&gt;&nbsp;<a href="/<%=catalog.substring(0,6) %>-<%=factoryid %>-0.htm"><%=CommonString.getFormatPara(brandMap.get(factoryid)).replace("卡特彼勒","卡特")%><%=CommonString.getFormatPara(catalogMap.get(catalog.substring(0,6))) %></a>
&gt;&gt;&nbsp;<%=factoryname+name+catalogname %>口碑</h3>
</div>
<!--面包屑结束-->
<div class="contain980 mb10 np_fix"> 
  ...
  <div class="w210 l n_pp_left np_left nkb_detleft">
	<div class="w208  l leftPart02 mb10 np_addpcat border02">
      <div class="stitle np_fix">
      	<% if(catalog.length() >= 6){  %>
        <h3 class="l"><%="".equals(factoryid) ? "" : CommonString.getFormatPara(brandMap.get(factoryid)).replace("卡特彼勒","卡特") %><%=CommonString.getFormatPara(catalogMap.get(catalog.substring(0,6))) %></h3>
        <span class="r"><a title="更多<%="".equals(factoryid) ? "" : CommonString.getFormatPara(brandMap.get(factoryid)).replace("卡特彼勒","卡特") %><%=CommonString.getFormatPara(catalogMap.get(catalog.substring(0,6))) %>口碑" href="/<% if(catalognum.length()>=6){ %><%=catalognum.substring(0, 6) %><%}else{ %><%=catalognum %><%} %>-<%=factoryid %>-0.htm">更多&gt;&gt;</a></span>
        <%	} %>
      </div>
      <ul class="list003">
    ...
    <div class="mb10 nkb_zyyw border02 l w208">
        ...
        <%
       	...
       	%> 
        	<li><a target="_blank"  href="/<%=num %>-<%=factoryid %>-0.htm" title="<%=factoryname.replace("卡特彼勒","卡特") %><%=cataname %>"><%=cataname %></a></li>
         <%}}}%>
        </ul>
    </div>
  ...
  <div class="w757 r np_right">
    ...
    <div class="pdTop2 mb10">
      ...
      <div class="score">
        <div class="total" style="position:relative;">网友平均评分 <font id="totalScoreId"></font> 分
		<span style="display: inline-block;position: absolute;top: 12px;left: 190px;">	
    ...
    <div class="l border03 n_koubei_list border03">
      ...
      <div class="plList nkb_pro_list np_fix kalist" style="height:580px;;overflow:hidden;" id="morebrand">
        <ul>
        <%
		   if(searchProducts !=null && searchProducts.size()>0){
			   for(Map oneMap:searchProducts){
				   ...
				   String pGood = CommonString.getFormatPara(oneMap.get("good"));
				   String pBad = CommonString.getFormatPara(oneMap.get("bad"));
				   ...
				   int pScore1 = Integer.parseInt(CommonString.getFormatPara(oneMap.get("score1")));
				   int pScore2 = Integer.parseInt(CommonString.getFormatPara(oneMap.get("score2")));
				   int pScore3 = Integer.parseInt(CommonString.getFormatPara(oneMap.get("score3")));
				   int pScore4 = Integer.parseInt(CommonString.getFormatPara(oneMap.get("score4")));
				   int pScore = (pScore1+pScore2+pScore3 +pScore4)/4;
				   
		%>
          <li>
            <div class="content_new">
              <div class="np_fix ktitle">
                <h3><%=pUsername.equals("")?pContact_address:pUsername %></h3>
              ...
              <div class="n_kb_add_pf">
                <ul class="np_fix nkb_pflist np_fix">
                  <li><span class="nkb_pfwz">价格：</span><div id="pricescore_<%=pId %>" title="regular"></div><span class="nkb_pffs"><%=pScore1 %>分</span></li>
                    <li><span class="nkb_pfwz">性能：</span><div id="abilityscore_<%=pId %>" title="regular"></div><span class="nkb_pffs"><%=pScore2 %>分</span></li>
                    <li><span class="nkb_pfwz">质量：</span><div id="qualityscore_<%=pId %>" title="regular"></div><span class="nkb_pffs"><%=pScore3 %>分</span></li>
                    <li><span class="nkb_pfwz">售后：</span><div id="servicescore_<%=pId %>"></div><span class="nkb_pffs"><%=pScore4 %>分</span></li>
                </ul>
              </div>
              ...
              <%
              	String imgSql = "select top 10 * from pro_comments_image where comment_id='"+pId+"' and is_show=1";
              ...
              %>
              <li><a href="javascript:void(0)"><img onclick="showmore()" src="<%=image %>" width="40" height="40"><i></i></a></li>
              <%} %>
              </ul>
    ...
    <form name="comment_form" id="comment_form" method="post" action="/action/ajax.jsp">
      <div class="sentComment" name="sentComment" id="sentComment">
        ...
        <tr>
          <td>&nbsp;</td>
          <td align="left">
            <p><input id="button" name="button" value="&nbsp;" type="button" onclick="doComments();" class="ti_btn" style="border:0" /></p>
         </td>
        </tr>
      </table>
        </div>
