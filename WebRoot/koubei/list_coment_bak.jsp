...
<%
	...
	//所有分类
	List<Map> allcatalogsList = dbHelper.getMapList("select name,num from pro_catalog where len(num) = 6 and is_show=1" );
	// 根据条件查出的产品
	//List<Map> searchProducts = pageBean.getDatas() ; 
	List<Map> searchProducts = dbHelper.getMapList("select id,add_date,product_id,content,is_help,username,score1,score2,score3,score4,touxiang from pro_koubei_comment_user where product_id="+product_id+" order by add_date desc");
	String tdkTtile = "";
	if(good.equals("")){
        ...
        <%
		   if(searchProducts !=null && searchProducts.size()>0){
			   for(Map oneMap:searchProducts){
	               ...
	               list_touxiang=CommonString.getFormatPara(oneMap.get("touxiang"));
		 
		%>
          <li>
            <div class="content_new">
						     <img style="width:41px;height:41px;"  src="<%=list_touxiang%>" onerror="showImgDelay(this,'http://product.21-sun.com/catalog/images/s_face.png',2);" > 

              <div class="np_fix ktitle" style="width:660px;float:right">
              ...
              <%
              	String imgSql = "select top 10 image from pro_comments_image where comment_id='"+pId+"' and is_show=1";
