...
<!--topBar end-->
<!--top-->
<jsp:include page="/include_new/top_search_plist.htm" flush="true"/>
<!--top end-->
<!--nav-->
<jsp:include page="/include_new/catalog_new.htm" flush="true"/>
<!--面包屑-->
<div class="contain1190 mb10">
  <h3 class="breadCrumbs"><a href="/" title="铁臂口碑">铁臂口碑</a>&nbsp;&gt;&gt;&nbsp;<a href="/new/">最新口碑</a>
    ...
    <%} %>
    &gt;&gt;&nbsp;<a href="/comment-<%=catalog %>-0-0-0-0.htm"><%=CommonString.getFormatPara(catalogMap.get(catalog)) %></a> &gt;&gt;&nbsp;<a href="/comment-0-<%=factory %>-0-0-0.htm"><%=CommonString.getFormatPara(brandMap.get(factory)) %></a> &gt;&gt;&nbsp;<a href="/comment-<%=catalog %>-<%=factory %>-0-0-0.htm"><%="0".equals(factory) ? "" : CommonString.getFormatPara(brandMap.get(factory)).replace("卡特彼勒","卡特") %><%=CommonString.getFormatPara(catalogMap.get(catalog)) %></a> &gt;&gt;&nbsp;<%="0".equals(factory) ? "" : CommonString.getFormatPara(brandMap.get(factory)) %><%=CommonString.getFormatPara(catalogMap.get(catalog)) %>怎么样
    <% }else if(!catalog.equals("")&&factory.equals("")){ %>
    &gt;&gt;&nbsp;<a href="/comment-<%=catalog.substring(0, 3) %>-0-0-0-0.htm"><%=CommonString.getFormatPara(catalogMap.get(catalog.substring(0, 3))) %></a>
    <% if(catalog.length() == 6){ %>
    &gt;&gt;&nbsp;<a href="/comment-<%=catalog %>-<%=factory %>-0-0-0.htm"><%="0".equals(factory) ? "" : CommonString.getFormatPara(brandMap.get(factory)) %><%=CommonString.getFormatPara(catalogMap.get(catalog)) %></a> &gt;&gt;&nbsp;<%=CommonString.getFormatPara(catalogMap.get(catalog)) %>哪个品牌好
    <% }else{ %>
    &gt;&gt;&nbsp;<%=CommonString.getFormatPara(catalogMap.get(catalog)) %>哪个品牌好
    <% } %>
    <% }else if(catalog.equals("")&&!factory.equals("")){ %>
    &gt;&gt;&nbsp;<a href="/comment-0-<%=factory %>-0-0-0.htm"><%=CommonString.getFormatPara(brandMap.get(factory)) %>口碑</a> &gt;&gt;&nbsp;<%=CommonString.getFormatPara(brandMap.get(factory)) %>怎么样
    <% } %>
  </h3>
</div>
<!--面包屑结束-->
  ...
  <div class="w210 l">
    <div id="leftId">
      ...
      <div class="w208 border01 l leftPart mb10">
	    ...
	    %>
            <li class="root" id="cat_<%=topCatNum %>"> <b style="font-size:14px;"><a href="/comment-<%=topCatNum %>-0-0-0-0.htm"><%=topCatName %></a></b>
		...
		%>
                <li id="cat_<%=topCatNum %>_<%=subCatNum %>"<%=catalog.equals(subCatNum) ? " class=\"rootCur\"" : "" %>> <a href="/comment-<%=subCatNum %>-0-0-0-0.htm" class="mainBrand"><big><%=subCatName %></big></a> </li>
                <%
						}
					}
	    %>
  ...
  <div class="txd_nright">
    ...
    <div class="txd_shaixuan">
      <dl class="fix">
        <dt>品牌</dt>
        <dd>
          <div class="tsx_ddheight"> <a href="/comment-<%=catalogInt %>-0-0-0-0.htm" <%if("".equals(factory)){%> class="select" <%} %>>不限</a> <a <%if("133".equals(factory)){%> class='select' <%} %> href="/comment-<%=catalogInt %>-133-0-0-0.htm">三一</a> <a <%if("182".equals(factory)){%>class='select' <%} %> href="/comment-<%=catalogInt %>-182-0-0-0.htm">小松</a> <a <%if("192".equals(factory)){%>class='select' <%} %> href="/comment-<%=catalogInt %>-192-0-0-0.htm">斗山</a> <a <%if("184".equals(factory)){%>class='select' <%} %> href="/comment-<%=catalogInt %>-184-0-0-0.htm">日立</a> <a <%if("183".equals(factory)){%>class='select' <%} %> href="/comment-<%=catalogInt %>-183-0-0-0.htm">神钢</a> <a <%if("175".equals(factory)){%>class='select' <%} %> href="/comment-<%=catalogInt %>-175-0-0-0.htm">沃尔沃</a> <a <%if("174".equals(factory)){%>class='select' <%} %> href="/comment-<%=catalogInt %>-174-0-0-0.htm">卡特彼勒</a> <a <%if("146".equals(factory)){%>class='select' <%} %> href="/comment-<%=catalogInt %>-146-0-0-0.htm">玉柴重工</a> <a <%if("136".equals(factory)){%>class='select' <%} %> href="/comment-<%=catalogInt %>-136-0-0-0.htm">柳工</a> <a <%if("194".equals(factory)){%>class='select' <%} %> href="/comment-<%=catalogInt %>-194-0-0-0.htm">现代京城</a> <a <%if("1220".equals(factory)){%>class='select' <%} %> href="/comment-<%=catalogInt %>-1220-0-0-0.htm">华通动力</a>
    ...
    <div class="l border03 n_koubei_list border03" style="border: #cccccc 0px solid">
      ...
      <div class="plList nkb_pro_list np_fix" style="width:900px">
        <ul>
          <%
            ...
            <div class="content" style="width:680px">
              ...
              <div class="w100 np_fix">
                <p class="textContent">
                  ...
                  缺点：<%=bad %><br />
                  <%}}%>
                  <%=content %><a title="查看全文" href="/detail/<%=id %>.htm" target="_blank" style="color:red;">查看全文&gt;&gt;</a></p>
              ...
              <div class="nkb_moregg" style="clear:both"><a href="/0-<%=factoryid %>-0.htm"  title="<%=factoryname %>"><%=factoryname %></a></span>&nbsp;&nbsp;|&nbsp;&nbsp;<span><a href="/<% if(catalognum.length()>=6){ %><%=catalognum.substring(0, 6) %><%}else{ %><%=catalognum %><%} %>-<%=factoryid %>-0.htm" title="<%=factoryname.replace("卡特彼勒","卡特")%><% if(catalognum.length()>=6){ %><%=CommonString.getFormatPara(catalogMap.get(catalognum.substring(0, 6))) %><%}else{ %><%=CommonString.getFormatPara(catalogMap.get(catalognum)) %><%}%>"><%=factoryname.replace("卡特彼勒","卡特") %>
                <% if(catalognum.length()>=6){ %>
                <%=CommonString.getFormatPara(catalogMap.get(catalognum.substring(0, 6))) %>
                <%} %>
                </a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="/comment-<% if(catalognum.length()>=6){ %><%=catalognum.substring(0, 6) %><%}else{ %><%=catalognum %><%} %>-<%=factoryid %>-0-0-0.htm" target="_blank" title="更多<%=factoryname %><% if(catalognum.length()>=6){ %><%=CommonString.getFormatPara(catalogMap.get(catalognum.substring(0, 6))) %><%} %>口碑">更多<%=factoryname.replace("卡特彼勒","卡特") %>
                <% if(catalognum.length()>=6){ %>
                <%=CommonString.getFormatPara(catalogMap.get(catalognum.substring(0, 6))) %>
                <%} %>
                口碑</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="/list/<%=product_id%>.htm#00b" target="_blank" title="询<%=factoryname +product_name+catalogname %>最低价">询最低价</a>&nbsp;&nbsp;|&nbsp;&nbsp;<span class="youk"><a href="javascript:void(0)" onclick="voteHelp('<%=id %>')"><b style="color:red">有用</b></a> (<span id="vote_<%=id %>"><%=CommonString.getFormatInt(oneMap.get("is_help")) %></span>)</span></div>
            </div>
          </li>
          <%
			   }
