...
<% 
	...
	if(proId!=null && !proId.equals("")){
		...
		if(proMap == null){
			...
			introduce = CommonString.getFormatPara(proMap.get("introduce")).replace("?", "");
			...
			//查询图片
			String imgSqlStr = "select top 5 name,img_name from pro_product_pic where  product_id= ?";
			proImgList = dbHelper.getMapList(imgSqlStr, new Object[]{proId});
			
			String inquirySql = " select top 10 * from pro_product_form where is_deleted=0 and product_name is not null and product_name != '' and product_id = ? order by add_date desc,id desc ";
			inquiryList = dbHelper.getMapList(inquirySql,new Object[]{proId});
		}
	}else{
		response.setStatus(HttpServletResponse.SC_NOT_FOUND);
		Common.doForward(request,response,"/404.htm");
	}
		List <Map> categoryParamsList =(List<Map>) paramListMap.get(CommonString.getFormatPara(proMap.get("catalognum")));
	...
	paramIds=paramIds+"0"; 
    List <Map> categoryList=dbHelper.getMapList(" select id,name   from pro_category where parent_no="+CommonString.getFormatPara(proMap.get("catalognum"))+" and id in (select category_id from pro_catalog_param where id  in ("+paramIds+") group by  category_id)"); 
%>
...
<div class="wrap_list">
  ...
  <div class="list_con clearfix">
	...
	%>
      ...
      <div class="right_down clearfix" id="inquirys">
        <h3 class="tit">询价单</h3>
        <div class="wfull l tableContain mb10 list08" id="myTab2_ContentA2">
          <ul class="ti">
            <li class="i2">询价产品</li>
            <li class="i3">联系人</li>
            <li class="i4">电话</li>
	  ...
	  %>
            <ul class="libg0<%=i%2+1%>">
              <li class="i2"><%=CommonString.getFormatPara(inquiryList.get(i).get("factoryname"))%><%=CommonString.getFormatPara(inquiryList.get(i).get("product_name"))%></li>
              <li class="i3"><%=CommonString.getFormatPara(inquiryList.get(i).get("name")).length()>=1?CommonString.getFormatPara(inquiryList.get(i).get("name")).substring(0,1)+"***":"***"%></li>
              <li class="i4"><%=CommonString.getFormatPara(inquiryList.get(i).get("contact_tel")).length()>=3?CommonString.getFormatPara(inquiryList.get(i).get("contact_tel")).substring(0,3)+"****":"****"%><%=CommonString.getFormatPara(inquiryList.get(i).get("contact_tel")).length()>=7?CommonString.getFormatPara(inquiryList.get(i).get("contact_tel")).substring(CommonString.getFormatPara(inquiryList.get(i).get("contact_tel")).length()-4,CommonString.getFormatPara(inquiryList.get(i).get("contact_tel")).length()):""%></li>
              <li class="i6"><%=CommonString.getFormatPara(inquiryList.get(i).get("province"))+CommonString.getFormatPara(inquiryList.get(i).get("city"))%></li>
              <li class="i7"><%=CommonString.getFormatPara(inquiryList.get(i).get("add_date")).length() >= 16?CommonString.getFormatPara(inquiryList.get(i).get("add_date")).substring(0,16):CommonString.getFormatPara(inquiryList.get(i).get("add_date"))%></li>
            </ul>
            <%
		}
	     }else{
