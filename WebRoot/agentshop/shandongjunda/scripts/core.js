...
DK.add("tabs", {
                ...
                $(g).hover(function(h) {
                    ...
                    e = setTimeout(function() {
                        if (a === g) {
                            d._resetTabCSS(b);
                            d._setTabCSS(g);
                            d.conf.callback.call(this, g, h, f)
                        }
                    },
                    d.conf.timeout)
                },
    ...
    _setTabCSS: function(a) {
        if ($(a).hasClass("active")) {
            return null
        }
        $(a).addClass("active")
    },
    _resetTabCSS: function(a) {
        $.each(a, 
        function(b, c) {
    ...
    _setSelectedTab: function(a) {
        var b = this;
        b._resetTabCSS(a);
        b._setTabCSS(a[b.conf.selectedIndex])
    },
    _autoPlay: function(b, a) {
        var c = this;
        if (c.conf.autoPlay) {
            c.conf.intervalIndex = setInterval(function() {
                if (a === b.length) {
                    a = 0
                }
                c._resetTabCSS(b);
    ...
    _clearAutoPlay: function(a) {
        var b = this;
        if (b.conf.event === "mouseover" && b.conf.autoPlay) {
            $.each(a, 
            function(c, d) {
                $(d).mouseover(function() {
                    b.conf.intervalIndex ? clearInterval(b.conf.intervalIndex) : ""
                })
            })
        }
    },
    init: function(c, b) {
        var a = this;
        a.conf = DK.merge(a.defconf, b);
        a._setSelectedTab(c);
        a._eventHandler(c);
        a._autoPlay(c, a.conf.selectedIndex);
        a._clearAutoPlay(c)
    }
});
DK.add("index", {
    defconf: {
        trigger: {
            pop: ".J_pop-top",
            net: ".J_net-top",
            pop_nav: ".J_pop-top-nav",
            net_nav: ".J_net-top-nav"
        },
        container: {
            pop_top: "#J_popular-top",
            net_top: "#J_netnovel-top"
        }
    },
    _ajaxTopList: function() {
        var f = this;
        var b = f.conf.trigger,
        a = f.conf.container,
        e = {};		
        $(b.pop_nav + " + " + b.pop).attr("idx", 0);
        $(b.net_nav + " + " + b.net).attr("idx", 0);
        $('<div id="' + b.pop.replace(/./, "") + '-loading" class="loading hide"></div>').insertAfter(b.pop + " " + b.pop_nav);
        $('<div id="' + b.net.replace(/./, "") + '-loading" class="loading hide"></div>').insertAfter(b.net + " " + b.net_nav);
        var d = function(j) {
            if ($(j).length > 0) {
                return true
            }
        ...
        var c = function(l, k) {
            ...
            m = n.parent(),
            j = "";
            if (m.hasClass("J_pop-top-nav")) {
                j = b.pop
            } else {
                if (m.hasClass("J_net-top-nav")) {
                    j = b.net
                }
            }
            $.each($(j + "[idx]"), 
            function(o, p) {
                if ($(p).attr("idx") !== k && $(p).is(":visible")) {
                    $(p).hide()
                }
            })
        };
        ...
        var g = function(n, l) {
            ...
            if (o.hasClass("J_pop-top-nav")) {
                j.pop_active = l;
                k = b.pop
            } else {
                if (o.hasClass("J_net-top-nav")) {
                    j.net_active = l;
                    k = b.net
                }
            }
            ...
            function(q) {
                var r = $.parseJSON(q);
                if (j.pop_active) {
                    $(a.pop_top + " > .J_pop-top").append($(r.content).attr("idx", l))
                } else {
                    if (j.net_active) {
                        $(a.net_top + " > .J_net-top").append($(r.content).attr("idx", l))
                    }
                }
                $(m).hide();
                c(n, l);
        ...
        DK.tabs.init($(b.pop_nav).children(), e);
        DK.tabs.init($(b.net_nav).children(), e)
    },
    init: function(b) {
        var a = this;
        a.conf = DK.merge(a.defconf, b);
