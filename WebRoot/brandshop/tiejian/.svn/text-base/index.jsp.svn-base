<%@ page language="java" import="com.jerehnet.util.dbutil.PageBean,com.jerehnet.util.common.CommonString,com.jerehnet.util.dbutil.DBHelper,java.util.*" pageEncoding="UTF-8"%><%@ taglib tagdir="/WEB-INF/tags" prefix="tags"%><%
	...
	String sel_sql = "select id,name,file_name,img2,factoryname,catalogname_spe,catalognum_spe,zoomlion_hot_pic ,catalognum,catalogname,zoomlion_hot from pro_products where factoryid=1758 and is_show=1 order by order_no desc" ;
...
<title>铁建重工_铁建重工品牌专区 - 铁臂商城</title>
...
<div class="proNav png">
  <ul>
    <li><a href="#lp01">全断面隧道掘进机</a></li>
    <li><a href="#lp02">矿山法隧道机械</a></li>
    <li><a href="#lp06">桩基础机械</a></li>
    <li><a href="#lp05">搅拌运输车</a></li>
      ...
      <%
	       if(null!=productsList && productsList.size()>0){
	    	   ...
	    	   for(int i=0;i<productsList.size();i++){
	    		   ...
	    		   if(CommonString.getFormatPara(oneMap.get("catalognum_spe")).startsWith("101") && sum<=4){
	    			   sum++ ;
	    			   %>
    ...
    <ul class="list">
      <%
	       if(null!=productsList && productsList.size()>0){
	    	   ...
	    	   for(int i=0;i<productsList.size();i++){
	    		   oneMap = (HashMap)productsList.get(i) ;
	    		   if(CommonString.getFormatPara(oneMap.get("catalognum_spe")).startsWith("102")){
	    			   //sum++ ;
	    			   %>
	    	  <li><a href="/proDetail/<%=CommonString.getFormatPara(oneMap.get("file_name")) %>" target="_blank"><img src="/uploadfiles<%=CommonString.getFormatPara(oneMap.get("img2")) %>" onerror="showImgDelay(this,'/uploadfiles/no_small.gif',2);" />
		        <h3><%=CommonString.getFormatPara(oneMap.get("factoryname"))+CommonString.getFormatPara(oneMap.get("name"))+CommonString.getFormatPara(oneMap.get("catalogname")) %></h3></a>
...
<div class="contain980 clearfix specialPart">
    ...
    <ul class="list">
      <%
	       if(null!=productsList && productsList.size()>0){
	    	   ...
	    	   for(int i=0;i<productsList.size();i++){
	    		   oneMap = (HashMap)productsList.get(i) ;
	    		   if(CommonString.getFormatPara(oneMap.get("catalognum_spe")).startsWith("106") && sum<=4){
	    			   sum++ ;
	    			   %>
	    	  <li><a href="/proDetail/<%=CommonString.getFormatPara(oneMap.get("file_name")) %>" target="_blank"><img src="/uploadfiles<%=CommonString.getFormatPara(oneMap.get("img2")) %>" onerror="showImgDelay(this,'/uploadfiles/no_small.gif',2);" />
		        <h3><%=CommonString.getFormatPara(oneMap.get("factoryname"))+CommonString.getFormatPara(oneMap.get("name"))+CommonString.getFormatPara(oneMap.get("catalogname")) %></h3></a>
...
<div class="ilistPart" style="float:right;">
    ...
    <ul class="list">
      <%
	       if(null!=productsList && productsList.size()>0){
	    	   ...
	    	   for(int i=0;i<productsList.size();i++){
	    		   oneMap = (HashMap)productsList.get(i) ;
	    		   if(CommonString.getFormatPara(oneMap.get("catalognum_spe")).startsWith("105") && sum<=4){
	    			   sum++ ;
	    			   %>
	    	  <li><a href="/proDetail/<%=CommonString.getFormatPara(oneMap.get("file_name")) %>" target="_blank"><img src="/uploadfiles<%=CommonString.getFormatPara(oneMap.get("img2")) %>" onerror="showImgDelay(this,'/uploadfiles/no_small.gif',2);" />
		        <h3><%=CommonString.getFormatPara(oneMap.get("factoryname"))+CommonString.getFormatPara(oneMap.get("name"))+CommonString.getFormatPara(oneMap.get("catalogname")) %></h3></a>
      ...
      <ul class="list">
        <%
             if(null!=productsList && productsList.size()>0){
                 ...
                 for(int i=0;i<productsList.size();i++){
                     oneMap = (HashMap)productsList.get(i) ;
                     if(CommonString.getFormatPara(oneMap.get("catalognum_spe")).startsWith("104") && sum<=4){
                         sum++ ;
                         %>
                <li><a href="/proDetail/<%=CommonString.getFormatPara(oneMap.get("file_name")) %>" target="_blank"><img src="/uploadfiles<%=CommonString.getFormatPara(oneMap.get("img2")) %>" onerror="showImgDelay(this,'/uploadfiles/no_small.gif',2);" />
                  <h3><%=CommonString.getFormatPara(oneMap.get("factoryname"))+CommonString.getFormatPara(oneMap.get("name"))+CommonString.getFormatPara(oneMap.get("catalogname")) %></h3></a>
      ...
      <ul class="list">
        <%
             if(null!=productsList && productsList.size()>0){
                 ...
                 for(int i=0;i<productsList.size();i++){
                     oneMap = (HashMap)productsList.get(i) ;
                     if(CommonString.getFormatPara(oneMap.get("catalognum_spe")).startsWith("103") && sum<=4){
                         sum++ ;
                         %>
                <li><a href="/proDetail/<%=CommonString.getFormatPara(oneMap.get("file_name")) %>" target="_blank"><img src="/uploadfiles<%=CommonString.getFormatPara(oneMap.get("img2")) %>" onerror="showImgDelay(this,'/uploadfiles/no_small.gif',2);" />
                  <h3><%=CommonString.getFormatPara(oneMap.get("factoryname"))+CommonString.getFormatPara(oneMap.get("name"))+CommonString.getFormatPara(oneMap.get("catalogname")) %></h3></a>
...
<div class="ilistPart">
 ...
 <div class="content">
      <!--询价单--> 
     <div style="clear:both; border: 1px solid #D3D3D3; margin-bottom: 10px !important;border-top:#ff9000 2px solid;">
	 
	 <jsp:include page="/include/products/product_order_brandshop.jsp?factoryid=1758&indexflag=1"  flush="true"/>
