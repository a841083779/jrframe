...
<%
    ...
    String whereStr=" factoryid=151   and is_show=1";
	if(!keyword.equals("")){
		whereStr+=" and name like '%"+keyword+"%' ";
	}
	String pro_sql = "select   id,name,file_name,img2,catalogname,zoomlion_hot_pic from pro_products where "+whereStr+" order by id desc" ;
		...
		<div class="list_tline">
			...
			%>
				<li class="l pr goods_item imgZoom">
					<a href="/proDetail/<%=CommonString.getFormatPara(oneMap.get("file_name")) %>" target="_blank"><img class="goods_img" src="/uploadfiles/<%=CommonString.getFormatPara(oneMap.get("img2")) %>" alt=""></a>
					<div class="item_info pa fix">
						<p class="goods_tit l"><%=CommonString.getFormatPara(oneMap.get("name")) %><%=CommonString.getFormatPara(oneMap.get("catalogname")) %></p>
