...
<%
	...
	String 	sel_sql="select * from pro_weixiu_info where id= ?";
		...
		String username="";
	    if(!"".equals(id)){
		oneNews = dbHelper.getMap(sel_sql,new Object[]{id}) ;
		title=CommonString.getFormatPara(oneNews.get("title"));
		factory=CommonString.getFormatPara(oneNews.get("factory"));
		catalognum=CommonString.getFormatPara(oneNews.get("catalog"));
		username=CommonString.getFormatPara(oneNews.get("username"));
		String cata="";
		if(catalognum.length()>=6){cata=catalognum.substring(0,6);}else{cata=catalognum;}
	    content =(String)(oneNews.get("content"));
		if(!cata.equals("")&&!factory.equals("")){
			content= content.replaceFirst(CommonString.getFormatPara(brandMap.get(factory))+CommonString.getFormatPara(catalogMap.get(cata)),"1-1-1-1");
		}
		if(!cata.equals("")){
		    content= content.replaceFirst(CommonString.getFormatPara(catalogMap.get(cata)),"2-2-2-2");
		}
		if(!factory.equals("")){
			content= content.replaceFirst(CommonString.getFormatPara(brandMap.get(factory)),"3-3-3-3");
		}
		   content= content.replace("1-1-1-1","<a href='http://product.21-sun.com/products/prolist.jsp?factory&="+factory+"catalog="+cata+"' style='color:#204C89;text-decoration:underline' target='_blank' >"+CommonString.getFormatPara(brandMap.get(factory))+CommonString.getFormatPara(catalogMap.get(cata))+"</a>");
		    content= content.replace("2-2-2-2","<a href='http://product.21-sun.com/products/prolist.jsp?catalog="+cata+"' style='color:#204C89;text-decoration:underline' target='_blank' >"+CommonString.getFormatPara(catalogMap.get(cata))+"</a>");
			content= content.replace("3-3-3-3","<a href='http://product.21-sun.com/products/prolist.jsp?factory="+factory+"'  style='color:#204C89;text-decoration:underline'  target='_blank'>"+CommonString.getFormatPara(brandMap.get(factory))+"</a>");
		...
		pub_date=CommonDate.getFormatDate("yyyy-MM-dd",oneNews.get("add_date")) ;
...
<div class="contain980 np_fix">
  ...
  <div class="kb_det_left">
    <div class="listleft02">
      ...
      <div class="listtitle03"><font class="font08" style="color:black">发布时间:<%=pub_date%> &nbsp;&nbsp;&nbsp;&nbsp;来源:<a href="http://weixiu.21-sun.com/">铁臂维修</a>&nbsp;&nbsp;&nbsp;&nbsp;作者:<%=username.equals("front")?"铁臂用户":username%></font></div>
      <div class="detail" id="mainDetail"> <%=content%> </div>
	  <%if(!factory.equals("")||!catalognum.equals("")){%>
      <div class="listfoot01" style="padding-left:10px;height:30px;">标签：<%=factory.equals("")?"":"&nbsp;&nbsp;&nbsp;<a target='_blank' href='http://weixiu.21-sun.com/0-0-"+factory+".htm'>"+CommonString.getFormatPara(brandMap.get(factory))+"</a>"%>
	  <%=catalognum.equals("")?"":"&nbsp;&nbsp;&nbsp;<a target='_blank' href='http://weixiu.21-sun.com/"+catalognum+"-0-0.htm'>"+CommonString.getFormatPara(catalogMap.get(catalognum))+"</a>"%>
	  <%=(!factory.equals("")&&!catalognum.equals(""))?"&nbsp;&nbsp;&nbsp;<a target='_blank' href='http://weixiu.21-sun.com/"+catalognum+"-0-"+factory+".htm'>"+CommonString.getFormatPara(brandMap.get(factory))+CommonString.getFormatPara(catalogMap.get(catalognum))+"</a>":""%></div>
	  <%}%>
	  <div class="listfoot01" style="padding-left:10px;height:30px;">
	  铁臂维修发布的信息确系本人原创，严禁剽窃、转投他人信息，若因此引起任何法律纠纷，与铁臂维修无关。
	  </div>
