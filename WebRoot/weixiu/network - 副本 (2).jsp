...
<div class="contain980 mb10">
  <h3 class="breadCrumbs"><a href="http://weixiu.21-sun.com/">铁臂维修</a>
   ...
   <%="".equals(factoryid)?"":" &gt;&gt; <a href='/weixiu/0-0-"+factoryid+"-0.htm'>"+("".equals(factoryid)?"":CommonString.getFormatPara(brandMap.get(factoryid))) +"</a>"%>
   <%="".equals(catalog)?"":" &gt;&gt; <a href='/weixiu/0-0-0-"+catalog+".htm'>"+("".equals(catalog)?"":CommonString.getFormatPara(catalogMap.get(catalog))) +"</a>"%>
  ...
  <div class="nre_right">
    ...
    <form name="theform" id="theform">
      <div class="border03 l Condition mb10 nwx_net_sea">
        <div class="ConditionTop">
          <div class="l">
            ...
            <a href="/weixiu/0-0-0-0.htm"> <span class="reset">重置</span></a>
            <%}%>
			...
			<a href="/weixiu/<%="".equals(province)?"0":province%>-<%="".equals(city)?"0":city%>-<%="".equals(factoryid)?"0":factoryid%>-0.htm" title="取消" class="delete" ></a> </span>
            <%}%>
            <%if (!"".equals(factoryid) && !"".equals(factoryid)) {%>
            <span><em><%="".equals(factoryid) ? "" : CommonString.getFormatPara(brandMap.get(factoryid))%></em>
			<a href="/weixiu/<%="".equals(province)?"0":province%>-<%="".equals(city)?"0":city%>-0-<%="".equals(catalog)?"0":catalog%>.htm" title="取消" class="delete"></a> </span>
            <%}%>
			
			<%if (!"".equals(city) && !"".equals(city)) {%>
            <span><em><%=show_city%></em> <a href="/weixiu/<%="".equals(province)?"0":province%>-0-<%="".equals(factoryid)?"0":factoryid%>-<%="".equals(catalog)?"0":catalog%>.htm" title="取消" class="delete"></a> </span>
            <%}%>
			<%if (!"".equals(province) && !"".equals(province)) {%>
            <span><em><%=show_province%></em> <a href="/weixiu/0-0-<%="".equals(factoryid)?"0":factoryid%>-<%="".equals(catalog)?"0":catalog%>.htm" title="取消" class="delete"></a> </span>
            <%}%>
	  ...
	  jQuery(document).ready(function() {
          ...
          <li class="cblist">
            <dt> 省份 </dt>
            <dd> <a href="/weixiu/0-0-<%=factoryid.equals("")?"0":factoryid%>-<%=catalog.equals("")?"0":catalog%>.htm" <%if("0".equals(province)||"".equals(province)){%> class='select' <%} %>>不限</a>
