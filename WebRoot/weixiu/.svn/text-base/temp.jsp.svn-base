<%@ page contentType="text/html; charset=utf-8" language="java" import="java.sql.*,java.util.*,com.jerehnet.util.dbutil.DBHelper,com.jerehnet.util.common.*" errorPage="" %><%
	...
	try{
		...
		List<Map> newsList = dbHelper.getMapList(newsQuery);
		...
		List<Map> companyList = dbHelper.getMapList(companyQuery);
%>
  ...
  <div class="reMid">
    ...
    <div class="rp clearfix mb10 productList">
      <div class="title">
        ...
        <span class="more"><a target="_blank" href="http://product.21-sun.com<%=url%>?factory=<%=factory %>&catalog=<%=catalog %>">更多&gt;&gt;</a></span>
      </div>
      <div class="content">
        <ul class="list blue">
        	<%
        	...
        	<li>
        		...
        		<div class="w328 r">
	        		...
	        		<span class="info">
	        			<a target="_blank" href="http://product.21-sun.com<%=url%>?factory=<%=CommonString.getFormatPara(product.get("factoryid"))%>&catalog=<%=CommonString.getFormatPara(product.get("catalognum")).length() >= 6 ? CommonString.getFormatPara(product.get("catalognum")).substring(0, 6) : CommonString.getFormatPara(product.get("catalognum"))%>">
