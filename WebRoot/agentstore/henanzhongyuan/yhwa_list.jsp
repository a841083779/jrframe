...
<%
	...
	List<Map> hProductsList = new ArrayList() ;
	String hwa_sql = " select top 3 * from pro_agent_product_vi where factoryid=16373 and catalognum LIKE '101001%' and zoomlion_hot=11 AND agentid=1202 and is_show=1" ; 
	String ywa_sql = " select top 3 * from pro_agent_product_vi where factoryid=183 and catalognum LIKE '101001%' and zoomlion_hot=11 AND agentid=1202 and is_show=1" ; 
	waProductsList = dbHelper.getMapList(ywa_sql+"order by order_no");
	hProductsList = dbHelper.getMapList(hwa_sql+"order by order_no");
%>
...
-->
    ...
    <% 
    	if(hProductsList.size()>0 && hProductsList!=null){
    		for(Map oneMap:hProductsList){
    			%>
