...
<%
	...
	String catanum = CommonString.getFilterStr(request.getParameter("catanum")) ;
	DBHelper dbHelper = DBHelper.getInstance() ;
	String sel_sql = "select  * from pro_agent_product_vi where agentid=1661 and is_show=1 and factoryid= "+factoryid ;
	List<Map> products_List = null; 
	Map catalogMap = (HashMap)application.getAttribute("catalogMap") ;
	Map brandMap = (HashMap)application.getAttribute("brandMap") ;
	if("".equals(factoryid)){
		response.sendRedirect("/agentstore/qianlima/") ;
	}
  ...
  <div class="contain980 pt20 clearfix">
    <div class="shopContain">
      ...
      <div class="shopRightPart">
        ...
        <a title="千里马官方旗舰店" href="/agentstore/qianlima"><img src="images/listbanner.gif" alt="做世界级的工程机械绿色连锁服务商" width="700" height="183"></a>
        </div>
		<%
            ...
            <ul class="list">              
              ...
              if(null!=products_List && products_List.size()>0){
               ...
               <li>
                ...
                <h4><a href="/proDetail/<%=CommonString.getFormatPara(oneMap.get("file_name")) %>" target="_blank" title="<%=CommonString.getFormatPara(oneMap.get("factoryname")) +CommonString.getFormatPara(oneMap.get("name"))+CommonString.getFormatPara(oneMap.get("catalogname"))%>"><%=CommonString.getFormatPara(oneMap.get("factoryname")) +CommonString.getFormatPara(oneMap.get("name"))+CommonString.getFormatPara(oneMap.get("catalogname"))%></a></h4>
...
jQuery("#leftNav li ul li").hover(function(){
	...
	if(jQuery(this).find("a").attr("href").indexOf('<%=catanum%>')!=-1&&'<%=catanum%>'!=''){
	  jQuery(this).addClass("select") ;
	}
}) ;
</script>
