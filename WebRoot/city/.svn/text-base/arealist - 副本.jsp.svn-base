...
<%
	...
	List<Map> hotcitypyList3 = (List<Map>)dbHelper.getMapList("select name,num,parent_num from pro_agent_province_city where parent_num <> '0' and parent_num in("+sqlnei3+") ") ;
	...
	List<Map> hotcitypyList4 = (List<Map>)dbHelper.getMapList("select name,num,parent_num from pro_agent_province_city where parent_num <> '0' and parent_num in("+sqlnei4+")") ;
	...
	List<Map> hotcitypyList5 = (List<Map>)dbHelper.getMapList("select name,num,parent_num from pro_agent_province_city where parent_num <> '0' and parent_num in("+sqlnei5+")") ;
	...
	List<Map> hotcitypyList6 = (List<Map>)dbHelper.getMapList("select name,num,parent_num from pro_agent_province_city where parent_num <> '0' and parent_num in("+sqlnei6+")") ;
	...
	List<Map> hotcitypyList8 = (List<Map>)dbHelper.getMapList("select name,num,parent_num from pro_agent_province_city where parent_num <> '0' and num in("+sqlnei8+") or parent_num in("+sqlnei8+")") ;
%>
    ...
    <div class="bd">
      <div class="allcity" id="area01">
        ...
        <%for(int j=0 ;j<pinyin_indexArr.length;j++){ %>
          <dl>
            <dt>
              <label id="a"><%=pinyin_indexArr[j]%></label>
            </dt>
            <dd>
              <ul>
		<%
		...
		%>
				<li><a href="http://city.21-sun.com/<%=parent_areapy%>/" target="_blank" title="<%=name%>"><%=name%></a></li>
		<%}}}%>
              </ul>
            </dd>
          </dl>
    <%}%>    
      ...
      <div class="areabox" id="area02" >
       ...
       <div class="onearea lastarea">
        ...
        <h5><span><a href="http://<%=provience_parent_areapy%>.city.21-sun.com" target="_blank" title="<%=name%>"><%=name%></a></span><i></i></h5>
		<%} %>
		<dl>
        <%
		if (null != hotcitypyList2 && hotcitypyList2.size() > 0) {
        	...
        	citynum = parent_num;
        %>
			<dt><a href="http://<%=provience_parent_areapy%>.city.21-sun.com" target="_blank" title="<%=cityname%>"><%=cityname%></a></dt>
        <%}}}%>	
        </dl>
        </div>
		<%}}%>	
 ...
 <div class="areabox" id="area03" >
       ...
       <div class="onearea lastarea">
		<%if (provience_parent_num.equals("0")) { %>
        <h5><span><a href="http://product.21-sun.com/agent/<%=num%>_0_0_0.shtm" target="_blank" title="<%=name%>"><%=name%></a></span><i></i></h5>
		<%} %>
		<dl>
        <%
