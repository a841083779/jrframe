...
<%@page import="java.net.InetAddress"%><%
	...
	try{
		...
		if (null != ids && ids.length() > 0) {// 删除
			...
			if (null != idStr && !"".equals(idStr)) {// 修改
				...
				//口碑上传图片
				if(eventAfter.contains("insertImages")){
					String username = "";
					if(memberInfo != null){
					...
					if(!username.equals("")){
						System.out.println("usernam1="+username);
						...
						String [] imgs = request.getParameterValues("img");
							System.out.println("usernam2="+imgs);
	...
	if(!"".equals(eventAfter)){
		Common.doPostHttpAsync(Common.getDomain(request)+"/action/after.jsp",postMap);
	}
	out.print(rs); 
%>