<%@ page language="java" import="org.apache.commons.httpclient.params.*,org.apache.commons.httpclient.*,org.apache.commons.httpclient.methods.*,com.jerehnet.webservice.*,java.util.regex.*,org.json.*,java.util.*,com.jerehnet.util.common.*,java.sql.*,com.jerehnet.util.dbutil.*" pageEncoding="UTF-8"%><%
	...
	try{
		...
		if("new_save_photo".equals(flag)){
			...
			String sql = " insert into pro_photo_group(title,index_title,sort_num,image,is_recommend,is_show,add_date,add_ip,usern,pub_date,uuid,keywords,brand_keywords,zhanhui_num)";
			sql += " values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			...
			String pro_id = CommonString.getFormatPara(request.getParameter("pro_id")) ; // sort_num
		    ...
		    int keyId = dbHelper.executeUpdate(sql,new Object []{zd_title,zd_type,sort_num,img,1,0,CommonDate.getToday("yyyy-MM-dd HH:mm:ss"),Common.getIp(request),"youke",CommonDate.getToday("yyyy-MM-dd HH:mm:ss"),UUID.randomUUID().toString(),zd_pro_cataname,CommonString.getFormatPara(request.getParameter("zd_factoryname")),zd_zhanhui_num},connection);
			 connection.setAutoCommit(false);
			 	
			if(null!=imgs&&imgs.length>0){
				for(int i=0;i<imgs.length;i++){
			String sqlStr = " insert into pro_photo ( title,group_id,image,author,pub_date,is_show,add_ip,add_date,add_user,uuid) ";
			sqlStr += " values ( ? , ? , ? , ? , ? , ? , ? ,?,?,?)";
			int rb = dbHelper.execute(sqlStr,new Object [] {CommonString.getFormatPara(request.getParameter("zd_title")),keyId,Common.getSmallImg(imgs[i]),"youke",CommonDate.getToday("yyyy-MM-dd HH:mm:ss"),0,Common.getIp(request),CommonDate.getToday("yyyy-MM-dd HH:mm:ss"),"author",UUID.randomUUID().toString()},connection);
		    }}
