...
try{
	...
	pageBean.setOrderBy(" order_no asc ") ;  // 默认排序
	...
	pageBean.setFields(" id,img2,factoryid,factoryname,price,worktime,old,name,place,catalognum,catalogname,price") ;
	pageBean.setTableName("pro_products_used") ;
     ...
     <jsp:include page="agent_banners.jsp" flush="true">
      ...
      <div class="sRightPart">
        ...
        <div class="sContent">
          <ul class="slist">  
            ...
            String old=CommonString.getFormatPara(list.get(i).get("old"));
            String medel=CommonString.getFormatPara(list.get(i).get("factoryname"))+CommonString.getFormatPara(list.get(i).get("name"));
            String worktime=CommonString.getFormatPara(list.get(i).get("worktime"));
            String price=CommonString.getFormatPara(list.get(i).get("price"));
            String names=CommonString.getFormatPara(list.get(i).get("factoryname"))+CommonString.getFormatPara(list.get(i).get("name"))+old+CommonString.getFormatPara(list.get(i).get("catalogname"));
            %>
            <li>
              <div class="img"><a href="/agentshop02/used_detail.jsp?usern=<%=usern %>&id=<%=CommonString.getFormatPara(list.get(i).get("id"))%>" target="_blank" title="<%=CommonString.getFormatPara(list.get(i).get("factoryname"))%><%=CommonString.getFormatPara(list.get(i).get("name"))%><%=old %><%=CommonString.getFormatPara(list.get(i).get("catalogname"))%>">
              <img src="http://product.21-sun.com/uploadfiles/<%=CommonString.getFormatPara(list.get(i).get("img2")) %>" onerror="showImgDelay(this,'/uploadfiles/no_small.gif',2);" width="176px" height="147px" alt="<%=CommonString.getFormatPara(list.get(i).get("factoryname"))%><%=CommonString.getFormatPara(list.get(i).get("name"))+old%><%=old %><%=CommonString.getFormatPara(list.get(i).get("catalogname"))%>" /></a></div>
              	<p>&nbsp;&nbsp;&nbsp;型号：<a href="/agentshop02/used_detail.jsp?usern=<%=usern %>&id=<%=CommonString.getFormatPara(list.get(i).get("id"))%>" target="_blank" title="<%=CommonString.getFormatPara(list.get(i).get("factoryname"))%><%=CommonString.getFormatPara(list.get(i).get("name"))%><%=old%><%=CommonString.getFormatPara(list.get(i).get("catalogname"))%>">
              	<%=names.length()>12?names.substring(0,12)+"..":names %></a></p>
             	<p style="margin-left:7px">工作小时：<%=worktime.equals("0")?"暂无":worktime+"小时" %></p>
             	<p style="margin-left:7px">价格：￥<%=price.equals("0")?"暂无":price+"万" %>（可议）&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <b><a href="/agentshop02/used_detail.jsp?usern=<%=usern %>&id=<%=CommonString.getFormatPara(list.get(i).get("id"))%>" target="_blank" title="<%=CommonString.getFormatPara(list.get(i).get("factoryname"))%><%=CommonString.getFormatPara(list.get(i).get("name"))%><%=old%><%=CommonString.getFormatPara(list.get(i).get("catalogname"))%>"><input type="button" name="detail" value="详细》" /></a>
             </b> 
             	</p>
            </li>
            <%
      ...
      <div class="wfull l">
        <div id="pagination" class="page" style="margin-right:10px">
          <tags:page pageBean="<%=pageBean %>" url="/agentshop02/used_agent_products.jsp"/>
        </div>
      </div>
      <%
		}
