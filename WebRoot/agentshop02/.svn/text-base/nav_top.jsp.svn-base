...
<div class="sTop02">
  ...
  %>
     ...
     %>
      <li><a href="/agentshop02/<%=usern%>/used_agent_products.htm" <%if("7".equals(flag)){out.println("class='selected'");} %>><span>二手机展示</span></a></li>      	 
   	 <%
     }
     %>
                        <%
...
<script type="text/javascript">
	...
	jQuery.ajax({
		...
		type:"post",
		data:{"flag":"brandAgentCatalogListShop2",shop_url:'agentshop02',"agentid":"<%=CommonString.getFormatPara(map.get("id"))%>","agent_fac":"<%=CommonString.getFormatPara(map.get("agent_fac"))%>","agent_fac_name":"<%=CommonString.getFormatPara(map.get("agent_fac_name"))%>","usern":"<%=usern%>"},
