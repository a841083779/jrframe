<%@ page contentType="text/html; charset=utf-8" language="java" import="java.sql.*,com.jerehnet.util.common.CommonString,com.jerehnet.util.dbutil.DBHelper,java.util.Map,com.jerehnet.util.common.Common,java.util.List,java.util.LinkedHashMap,com.jerehnet.util.dbutil.PageBean,com.jerehnet.util.common.CommonHtml,com.jerehnet.util.common.CommonApplication" errorPage="" %><%@ taglib tagdir="/WEB-INF/tags" prefix="tags" %><%
	...
	try{
		...
		if(map != null && !CommonString.getFormatPara(map.get("is_qijiandian")).equals("2")){ // 使用模板二
     ...
     <jsp:include page="agent_banners.jsp" flush="true">
      ...
      <div class="sRightPart">
        <div class="sTitle" id="recommendProT">
          <h2>推荐产品</h2>
          <span class="more"><a href="/agentshop02/<%=usern%>/agent_products.htm">查看更多产品</a></span>
        </div>
      ...
      <div class="sRightPart">
        <div class="sTitle">
          <h2>主营产品</h2>
          <span class="more"><a href="/agentshop02/<%=usern%>/agent_products.htm">查看更多产品</a></span> </div>
        ...
        <jsp:param name="flag" value='agentshop02'></jsp:param>
        </jsp:include>
      </div>
      <!--公司简介-->
      <div class="sRightPart">
        <div class="sTitle">
          <h2>公司简介</h2>
          <span class="more"><a href="/agentshop02/<%=usern%>/introduce.htm">查看更多详情</a></span> </div>
        <div class="sContent companyInfo">
          <div class="img"><a href="/agentshop02/hfruntong/introduce.htm"><img src="/uploadfiles/<%=CommonString.getFormatPara(map.get("img1"))%>" onerror="showImgDelay(this,'/uploadfiles/no_big.gif',2);" width="246" height="168" /></a></div>
          <div class="intro">
            <p class="text"><%=(CommonHtml.filterHTML(CommonString.getFormatPara(map.get("introduce"))).length()>=220)?CommonHtml.filterHTML(CommonString.getFormatPara(map.get("introduce"))).substring(0,220)+"...":CommonHtml.filterHTML(CommonString.getFormatPara(map.get("introduce")))%><a target="_blank" href="/agentshop02/<%=usern%>/introduce.htm" title="查看详情" class="ablue"><b>[查看详情]</b></a></p>
      ...
      <div class="sRightPart">
    	...
    	type:'post',
    	url:'/action/ajax.jsp',
    	data:{flag:'getAgentViewCount',agentId:'<%=factoryid%>'} ,
	...
	%>
      ...
      <div class="wfull l">
        <div id="pagination" class="page" style="<%=pageBean.getPageCount()>1?"margin-right:10px;":"display:none;" %>">
          <tags:page pageBean="<%=pageBean %>" url="/agentshop02/detail.jsp"/>
        </div>
      </div>
      <%
		}
...
<script type="text/javascript">	
	jQuery("#zd_message").text("您好，我对<%=CommonString.getFormatPara(map.get("full_name")).replace("<br>","")%>的产品感兴趣，想要咨询，请您给我回复，谢谢。");
