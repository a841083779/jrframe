            ...
            //支付成功的业务逻辑
			   ...
			   Connection conn = null;
				...
				try{
					...
					Map m=dbHelper.getMap(" select * from activity_sany_pay where order_no='"+out_trade_no+"' ",conn);
					if(m!=null&&CommonString.getFormatInt(CommonString.getFormatPara(m.get("order_state")))==0){
						...
						dbHelper.execute("insert into activity_sany_pay_log (parent_id,source,add_date,product_name,price,order_state,open_id) "
								+" values ("+CommonString.getFormatInt(CommonString.getFormatPara(m.get("id")))+",'"+CommonString.getFormatPara(m.get("source"))+"',getDate(),'"+CommonString.getFormatPara(m.get("product_name"))+"',"+CommonString.getFormatPara(m.get("price"))+",1,'"+CommonString.getFormatPara(m.get("open_id"))+"') ");
					}
