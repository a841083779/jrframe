...
<%
	...
	try{
		...
		String title = "企业动态_"+CommonString.getFormatPara(map.get("full_name"))+"-中国工程机械商贸网"  ;
		String keywords = "企业动态" ;
		String description = "中国工程机械商贸网提供"+CommonString.getFormatPara(map.get("full_name"))+"企业动态，帮您了解企业动态";
        ...
        <div class="sContent">
        	...
        	if(list != null && list.size() > 0){
				...
				for(int i = 0;list != null && i < list.size();i++){
    				...
    				%>
    				<tr>
    					<td style="height: 30px; line-height: 30px;"><a target="_blank" href="http://product.21-sun.com/news/detail.jsp?id=<%=CommonString.getFormatPara(m.get("id")) %>&t_flag=product"><%=CommonString.getFormatPara(m.get("title")) %></a></td>
