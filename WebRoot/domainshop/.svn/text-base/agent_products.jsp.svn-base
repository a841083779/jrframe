<%@ page language="java" import="java.util.*,com.jerehnet.util.common.*,com.jerehnet.util.dbutil.*,java.sql.*" pageEncoding="UTF-8"%><%@page import="com.jerehnet.util.dbutil.PageBean"%><%@ taglib tagdir="/WEB-INF/tags" prefix="tags" %><%@ taglib uri="/WEB-INF/classes/oscache.tld" prefix="cache"%><%
    ...
    String  realPath1  =  "http://"  +  request.getServerName()    +  request.getContextPath()+request.getServletPath()+"?"+request.getQueryString(); 
    if(realPath1.indexOf("dingfengjixie.")!=-1){
    	realPath1 = realPath1.replaceFirst("dingfengjixie.","") ;
    	// request.getRequestDispatcher(realPath1.replaceFirst("dingfengjixie.","")).forward(request, response);
    // 	response.sendRedirect(realPath1) ;
    }
	...
	try{
		...
		String keywords = "代理机型" ;
		String description = "中国工程机械商贸网提供"+CommonString.getFormatPara(map.get("full_name"))+"代理机型，帮您了解代理机型";
		String catalogname = "" ;
...
<body class="style01">
<script type="text/javascript">
	console.log('<%=realPath1.replace("dingfengjixie.","")%>') ;
</script>
<!--top-->
    ...
    %>
      ...
      <div class="wfull l">
        <div id="pagination" class="page" style="margin-right:10px">
          <tags:domain_page pageBean="<%=pageBean %>" url="agent_products.htm"/>
        </div>
      </div>
      <%
		}
