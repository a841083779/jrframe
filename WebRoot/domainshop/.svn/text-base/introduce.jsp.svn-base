<%@ page language="java" import="java.util.*,com.jerehnet.util.common.*,com.jerehnet.util.dbutil.*,java.sql.*" pageEncoding="UTF-8"%><%
	...
	try{
		...
		String keywords = "公司简介"; 
		String description = "中国工程机械商贸网提供"+CommonString.getFormatPara(map.get("full_name"))+"公司简介，帮您了解公司简介"; 
		// 自定义 seo
    ...
    <div class="shopRight"> 
      ...
      <div class="sRightPart">
        ...
        <div class="sContent companyInfo">
          <div class="img"><img src="http://product.21-sun.com/uploadfiles/<%=CommonString.getFormatPara(map.get("img1"))%>" onerror="showImgDelay(this,'http://product.21-sun.com/uploadfiles/no_big.gif',2);" width="246" height="168" /></div>
          <div class="intro"><%=CommonString.getFormatPara(map.get("introduce"))%>
          <br/><br/>
          <a href="javascript:void(0)" title="免费询价" class="l mr20" onclick="showinquery('','<%=CommonString.getFormatPara(map.get("id"))%>','')"><img src="http://product.21-sun.com/images/simg_btn01.gif" alt="免费询价" width="188" height="45" /></a> <a href="http://product.21-sun.com/admin/login.jsp" target="_blank" title="代理商登录" class="l comlogin">代理商登录</a></div>
        </div>
      </div>
      <!--在线询价--></div>
    <!--right end-->
