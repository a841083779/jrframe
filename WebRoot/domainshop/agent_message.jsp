<%@ page language="java" import="java.util.*,com.jerehnet.util.common.*,com.jerehnet.util.dbutil.*,java.sql.*" pageEncoding="UTF-8"%><%@page import="com.jerehnet.util.dbutil.PageBean"%><%@ taglib tagdir="/WEB-INF/tags" prefix="tags" %><%
	...
	try{
		...
		String keywords = "在线询价" ;
		String description = "中国工程机械商贸网提供"+CommonString.getFormatPara(map.get("full_name"))+"在线询价，帮您了解在线询价";
		// 自定义 seo
...
<script type="text/javascript">
	...
	jQuery("#zd_message").text("您好，我对<%=CommonString.getFormatPara(map.get("name"))%>的产品感兴趣，想要咨询，请您给我回复，谢谢。");
...
jQuery("#zd_company").focus(function(){
	...
	function set_my_city(o,cked){
		...
		jQuery.ajax({
			url : 'http://product.21-sun.com/tools/ajax.jsp',
			async : false,
			type : 'post',
			data : {
				flag : 'get_city',
