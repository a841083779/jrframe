...
define(function(require, exports, module) {
		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
						...
						var settings = $.extend({
							...
							uploader : require.resolve('./uploadify.php#'), 
							// Options
							auto : false, 
							...
							fileTypeExts : '*.gif; *.jpg; *.png', // 格式筛选，如： '*.gif; *.jpg; *.png'
							height : 30, 
							itemTemplate : false, 
							method : 'post', 
							multi : true, // 多选
	...
	*/
	$.fn.h5upload = function(conf){
		 var opt = {
			 	auto     : false,
				uploader : "*.gif; *.jpg; *.png",
				button   : ".btn-upload",
				width    : 120,
				height   : 30,
				buttonText: '选择',
				fileTypeExts:"*.bmp;*.gif;*.jpeg;*.png;*.tiff",
				onUploadSuccess : function(){}
			},
			$this = $(this),
			fileToUpload = $this.find('input:file').get(0),
			$fileList, files,$barList,$state,index,resText;
			
		 $.extend(opt,conf);
		 
		$this.after($('<div class="file-list" />'));
		$fileList = $this.next('.file-list');
		$this.addClass('uploadify').css({
			width : opt.width + 'px',
		...
		fileToUpload.style.position = 'absolute';
		fileToUpload.style.height = '500px';
		fileToUpload.style.width = '220px';
		fileToUpload.style.top = '-260px';
		fileToUpload.style.left = '0';
		 ...
		 function fileSelected() {
			  ...
			  for(var i = 0; i < fs.length; i++) {
					var file = fs[i], _arr, _exp, rFilter;
			  ...
			  $barList = $('.uploadify-progress-bar');
			  $state = $('.uploadify-data');
			  //自动
			  if(opt.auto){
					uploadFile()  
				}
		 ...
		 function uploadProgress(evt) {
			  if (evt.lengthComputable) {
					...
					if(percentComplete == '100%'){
						$state.get(index).innerHTML = ' Complete!';
						$state.eq(index).parents('.uploadify-queue-item').delay(500).fadeOut(500,function(){
								$(this).remove();
							})
					}
			  }
