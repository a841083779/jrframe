...
<div class="fix kb_val">
  ...
  %>
    	<li>
        	...
        	<div class="_name"><a href="/web_kb_detail.jsp?id=<%=product_id%>" ><%=factoryname%><%=name1%><%=catalogname%></a></div>
            <div class="_canshu fix">
            	<p class="c_namedate"><span class="_t1"><%=username%></span><span class="_t2"><%=add_date%></span></p>
				<%
					model = dbHelper.getMap(" select count(*) as counts from pro_mobile_koubei_list where product_id = ? ",new Object [] {product_id} , connection);
				%>
