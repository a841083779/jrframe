...
var isFirefox = typeof document.body.style.MozUserSelect != 'undefined',
	addEvent = function (obj, type, fn) {if (obj.addEventListener) {obj.addEventListener(isFirefox ? 'DOMMouseScroll' : type, fn, false)}else{obj.attachEvent('on' + type, fn)};return fn},
	delEvent = function (obj, type, fn) {if (obj.removeEventListener) {obj.removeEventListener(isFirefox ? 'DOMMouseScroll' : type, fn, false)}else{obj.detachEvent('on' + type, fn)}};

$.fn.scrollBar=function(config){
	var this_=$(this),sC,sB,sU,sD,sS,sW,sAH,sAW,sSH,sSW,prop,keyway,ssTop=0,dataOpt,opts,testVal=false,testID,testMe,testHandler,
	_default={
		wrap:null,
		monitor:false,
		keyway:30,
	...
	opts=$.extend(_default, config || {}, this_.data('config')||{});
	if(opts.wrap){
		sC=$(''+opts.wrap)
		}else{
	...
	function testheight(fromMonitor){
		...
		if(sC.find('img').length && !fromMonitor){
			//如果有图片而且是初次执行，等待load后再计算prop以得到正确的结果
			$(window).on('load',function(){
				//滑块高度
				sSH=!!sS.height() ? sS.height() : Math.round(this_.height()/sC.height()*100)/100*this_.height() ;
				sS.height(sSH);
				//计算prop
			...
			//没有图片或是有图片但并非初次执行不需要等待load，可以立即计算prop	
				sSH=!!sS.height() ? sS.height() : Math.round(this_.height()/sC.height()*100)/100*this_.height() ;
				sS.height(sSH);
				prop=(sC.height()-this_.height())<0 ? 0 : (sC.height()-this_.height())/(this_.height()-sAH*2-sSH);
				setTop();
