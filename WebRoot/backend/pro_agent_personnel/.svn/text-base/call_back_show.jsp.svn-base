...
<%
		...
		try {
				...
				Map formMap = dbHelper.getMap(" select call_result1,call_result2,follow_content from pro_product_form where uuid = ? ",new Object [] {uuid},connection);
%>
...
<form name="theform" id="theform" action="/backend/action/after.jsp">
  ...
  <div class="formDiv">
    ...
    <div class="formRight" style="width: 300px">
      <input style="width: 300px" type="text" class="jr_text" name="company_name" id="company_name" value="<%=CommonString.getFormatPara(formMap.get("call_result2")) %>" />
    </div>
    <div style="clear: left;"></div>
