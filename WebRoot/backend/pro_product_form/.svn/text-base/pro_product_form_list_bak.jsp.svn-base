...
<%@page import="com.jerehnet.util.dbutil.DBHelper"%><%
	...
	String fileds = "id,add_date,factoryname,cataname,name,product_name,contact_address,contact_tel,state,mobile_phone,product_id,city,is_send";
  	...
  	<div class="searchForm">
  		<div class="searchCondition">
  		  	<div class="searchLeft">订单人</div>
		  		...
		  		<div class="searchRight">
		  			<%=commonForm.createRadio(stateMap,"-1","state") %>
		  		</div>
		  		...
		  		<div style="clear:left"></div>
		  	<div class="searchLeft">成交</div>
	  		<div class="searchRight">
	  			<%=CommonForm.createRadio(stateMap,"-1","is_deal") %>
	  		</div>
  		</div>
  		<div class="searchBtn">
...
<script type="text/javascript">
	...
	{ display : '短信发送', name : '', align : 'center' , type : 'string' , width : 50,render:function(row){
	  		return "<a class='link' href='javascript:void(0);' onclick='sendSms("+row.__index+");'>"+(row.is_send==1?"<font color='green'>已发送</font>":"<font color='red'>未发送</font>")+"</a>";
    	} 
	} 
	];
	...
	function doSearch(){
		...
		{
		  conditionStr += " and to_brands like '%,"+factoryid+",%'";
		}
		...
		var is_deal = jQuery("input[name='is_deal']:checked").val();
	   if(''!=is_deal && '-1'!=is_deal){
