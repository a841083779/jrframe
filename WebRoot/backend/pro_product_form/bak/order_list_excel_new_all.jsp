...
String yesday = new SimpleDateFormat("yyyy-MM-dd").format(calendar.getTime()) +" 16:00:00";
	...
	if("1".equals(if_tag))
	{
		...
		String tonandmeter = CommonString.getFormatPara(request.getParameter("zd_tonandmeter")) ; 
		String buy_type = CommonString.getFormatPara(request.getParameter("buy_type")) ; 
         
		 if(tonandmeter.equals("0")){
            whereStr.append( " and product_id in(select id from pro_products where tonandmeter <3 and catalognum like '101002%') ") ;
        }else if(tonandmeter.equals("1")){
            whereStr.append( " and product_id in(select id from pro_products where tonandmeter =3 and catalognum like '101002%')") ;
       }else if(tonandmeter.equals("2")){
	      	whereStr.append( " and product_id in(select id from pro_products where tonandmeter > 3 and tonandmeter <5 and catalognum like '101002%') ") ;
       }else if(tonandmeter.equals("3")){
	      	whereStr.append(" and product_id in(select id from pro_products where tonandmeter > 5 and catalognum like '101002%')") ;
       }else if(tonandmeter.equals("4")){
	       whereStr.append( " and product_id in(select id from pro_products where tonandmeter > 1 and tonandmeter <13 and catalognum like '101001%')") ;
       }else if(tonandmeter.equals("5")){
	        whereStr.append( " and  product_id in(select id from pro_products where tonandmeter > 14 and tonandmeter <30 and catalognum like '101001%') ") ;
       }else if(tonandmeter.equals("6")){
	      whereStr.append( " and  product_id in(select id from pro_products where tonandmeter >30 and catalognum like '101001%') ") ;
       }          		
		...
		if(!"".equals(buy_type)) 
		{
			whereStr.append(" and buy_type like '%"+buy_type+"%'") ;
		}
		...
		if(!"".equals(contact_address))
		{
		    ...
		    whereStr.append(" and province like '%"+contact_address+"%'") ;
		}
		...
		if(!"".equals(call_rs2)) 
		{
		 ...
		 if(!"".equals(is_success)&&!"-1".equals(is_success)&&!"0".equals(is_success)) 
		{
			whereStr.append(" and is_success = "+is_success+"") ;
		}
			 if("0".equals(is_success)) 
		{
			whereStr.append(" and is_success != 1") ;
		}
...
<%
	...
	ws.addCell(label);
    label = new Label(5,0,"购买性质");
	ws.addCell(label);
	
	label = new Label(6,0,"所属厂家");
	ws.addCell(label);
	label = new Label(7,0,"询价产品");
	ws.addCell(label);
	label = new Label(8,0,"所属分类");
	ws.addCell(label);
	label = new Label(9,0,"付款方式");
	ws.addCell(label);
	label = new Label(10,0,"预计购买时间");
	ws.addCell(label);
	label = new Label(11,0,"询价时间");
	ws.addCell(label);
	...
	if(null!=pro_List && pro_List.size()>0){
		...
		for(int i=0;i<pro_List.size();i++){
			...
			String buyType="个人";
			if(!CommonString.getFormatPara(oneMap.get("ifgroup")).equals("个人")&&!CommonString.getFormatPara(oneMap.get("ifgroup")).equals("")){
			   if(!CommonString.getFormatPara(oneMap.get("company")).equals("")){
			      buyType="公司:"+CommonString.getFormatPara(oneMap.get("company"));
			   }else{
			      buyType="公司";
			   }
			}
			label = new Label(5,(i+1),buyType) ;
			ws.addCell(label);
			
			label = new Label(6,(i+1),CommonString.getFormatPara(oneMap.get("factoryname"))) ;
			ws.addCell(label);
			label = new Label(7,(i+1),CommonString.getFormatPara(oneMap.get("product_name"))) ;
			ws.addCell(label);
			label = new Label(8,(i+1), CommonString.getFormatPara(oneMap.get("cataname"))) ;
			ws.addCell(label);
			label = new Label(9,(i+1),CommonString.getFormatPara(oneMap.get("buy_type"))) ;
			ws.addCell(label);
			label = new Label(10,(i+1),CommonString.getFormatPara(buyTime.get(CommonString.getFormatPara(oneMap.get("buy_time")))) ) ;
			ws.addCell(label);
			label = new Label(11,(i+1),CommonString.getFormatPara(oneMap.get("add_date"))) ;
			ws.addCell(label);
