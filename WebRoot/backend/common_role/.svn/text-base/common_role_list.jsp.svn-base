<%@ page language="java" import="java.util.*,com.jerehnet.util.common.*" pageEncoding="UTF-8"%><%
	...
	String fileds = "id,uuid,role_no,role_name";
	String whereStr = "";
  ...
  <body>
  	<div class="searchForm">
  		<div class="searchCondition">
	  		<div class="searchLeft">角色名称</div>
	  		<div class="searchRight">
	  			<input type="text" style="width: 150px;" name="role_name" class="jr_text" id="role_name" />
	  		</div>
  		</div>
  		<div class="searchBtn">
...
<script type="text/javascript">
	...
	//列数组
	columnsArr =  [ 
		{ display : '角色名称', name : 'role_name', align : 'left' , type : 'string' , editor: { type: 'text' } , width : 200 , render : function(row){
				return "<a class='link' href='javascript:void(0);' onclick='addAndEdit("+row.__index+");'>"+row.role_name+"</a>";
			}
		} ,
		{ display : '权限设置', align : 'center' , type : 'string' , render : function(row){
				return "<a class='link' href='javascript:void(0);' onclick='setPower(\""+row.uuid+"\");'>权限设置</a>";
			}
		} 
	];
	...
	function setPower(id){
		if(id!=undefined){
			jrWindow("/backend/common_role/common_power.jsp?id="+id,"权限设置",300,400);
		}
	}
	...
	function doSearch(){
		...
		if(''!=jQuery.trim(role_name)){
			conditionStr += " and role_name like '%"+role_name+"%' ";
		}
		params.push({'name':'tableName','value':tableName});
		params.push({'name':'fileds','value':fields});
