...
<%
		...
		try {
				...
				List<Map> brandList=(ArrayList)(dbHelper.getMapList(" select id,name from pro_agent_factory where  is_show=1 and flag =1 and id in(select distinct(factoryid)  from pro_products where is_show =1 and catalognum like '%101002%' ) order by view_count desc",connection));
                Map formMap=new HashMap();
		     	List<Map> proList = (ArrayList)application.getAttribute("brandList") ;
				proList=new ArrayList();

				List <Map>plist=(List <Map>)(dbHelper.getMapList(" select top 15 id,name,factoryid from pro_products where  is_show=1 and  catalognum like '%101002%'  order by orderno desc,view_count desc",connection));
...
<form name="theform" id="theform" action="/backend/action/after_own.jsp">
  ...
  <input type="hidden" name="eventAfter" id="eventAfter" value="zzjOrder" />
</form>
</body>
</html>
...
<script type="text/javascript" language="javascript">
	...
	jQuery(function(){
		jQuery("#brand_id").change(function(){
			 ...
			 jQuery.post("/backend/action/ajax_r.jsp",{'brandId':brandId,'flag':'getProZzj'},function(data){
