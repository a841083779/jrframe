<%@ page language="java" import="java.util.*,com.jerehnet.util.common.*" pageEncoding="UTF-8"%><%
	...
	String fileds = "id,uuid,parent_no,no,name,type,enum_no,is_search,is_key,is_show,order_no";
...
<script type="text/javascript">
	...
	{ display : '参数类型', name : 'type'  , id : 'type', align : 'center' , type : 'string' , width : 80 ,render : function(row){
			if(row.type==2){
				return "枚举";
			}
			return "文本";
	...
	{ display : '搜索参数', name : 'is_search', align : 'center' , type : 'string' , width : 70 ,render : function(row){
			if(row.is_search==1){
				return "是";
			}
			return "否";
	...
	{ display : '关键参数', name : 'is_key', align : 'center' , type : 'string' , width : 70 ,render : function(row){
			if(row.is_key==1){
				return "是";
			}
			return "否";
