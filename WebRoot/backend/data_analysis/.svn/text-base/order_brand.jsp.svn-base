...
<%@page import="java.net.URLDecoder"%><%
	...
	if(!"".equals(factoryid)){
	 ...
	 if(tonandmeter.equals("0")){
		 whereStr += " and product_id in(select id from pro_products where tonandmeter <3 and catalognum like '101002%') ";
	}else if(tonandmeter.equals("1")){
		whereStr += " and product_id in(select id from pro_products where tonandmeter =3 and catalognum like '101002%')";
   }else if(tonandmeter.equals("2")){
		whereStr += " and product_id in(select id from pro_products where tonandmeter > 3 and tonandmeter <5 and catalognum like '101002%') ";
   }else if(tonandmeter.equals("3")){
		whereStr += " and product_id in(select id from pro_products where tonandmeter > 5 and catalognum like '101002%')";
   }else if(tonandmeter.equals("4")){
		whereStr += " and product_id in(select id from pro_products where tonandmeter > 1 and tonandmeter <13 and catalognum like '101001%')";
   }else if(tonandmeter.equals("5")){
		whereStr += " and  product_id in(select id from pro_products where tonandmeter > 14 and tonandmeter <30 and catalognum like '101001%') ";
   }else if(tonandmeter.equals("6")){
	   whereStr += " and  product_id in(select id from pro_products where tonandmeter >30 and catalognum like '101001%') ";
   } 
 ...
 <form id="theform" name="theform" action="">
 	<input type="hidden" name="t_flag" id="t_flag" value="<%=flag %>"/>
      <table width="100%" cellspacing="0" cellpadding="0" border="0" class="orderSelect">
        ...
        <tr>
        	<td>
        	    <div class="menuLinks" style="float:left;width: 230px;">品<span class="span2">&nbsp;</span>牌:
        		...
        		<div class="menuLinks" style="float:left;width: 230px;">机<span class="span2">&nbsp;</span>型:
				...
				<div class="menuLinks" style="float:left;width: 230px;">吨<span class="span2">&nbsp;</span>位:
        		<select class="jr_select" id="tonandmeter" name="tonandmeter">
        		<option value="">--请选择--</option>
        		...
        		<div class="menuLinks" style="float:left;width: 230px;">所在地区:
...
jQuery(function(){
	...
	jQuery("#tonandmeter option").each(function(){
	      if(jQuery(this).val()=='<%=tonandmeter%>'){
	      	jQuery(this).attr("selected","selected") ;
	      	return  ;
	      }
	}) ;
...
function checkInput(){
	...
	var tonandmeter = jQuery("#tonandmeter").val() ;
	var province = jQuery("#zd_province").val() ;
	var catalognum = jQuery("#catalognum").val() ;
	if(ts>te){
		 jQuery("#te").select() ;
	}else{
	     window.location.href="?flag=tt&ts="+ts+"&te="+te+"&tonandmeter="+tonandmeter+"&factoryid="+factoryid+"&catalognum="+catalognum+"&province="+province ;
	}
...
jQuery("#catalognum").change(function(){
	...
	var province = jQuery("#zd_province").val() ;
	var factoryid = jQuery("#factoryid").val() ;
	var tonandmeter = jQuery("#tonandmeter").val() ;s
	window.location.href="?flag="+flag+"&ts="+ts+"&te="+te+"&catalognum="+catalognum+"&province="+province+"&factoryid="+factoryid+"&tonandmeter="+tonandmeter ;
}) ;
...
/*jQuery("#tonandmeter").change(function(){
	var flag = jQuery("#t_flag").val() ;
	var ts = jQuery("#ts").val() ;
	var te = jQuery("#te").val() ;
	var catalognum = jQuery("#catalognum").val() ;
