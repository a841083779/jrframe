<%@ page language="java" import="java.util.*,java.sql.*,com.jerehnet.util.dbutil.*,com.jerehnet.util.common.*" pageEncoding="UTF-8"%><%
	...
	try{
		...
		Map user = dbHelper.getMap(" select usern from "+tablePrefix+"common_user where uuid = ? ",new Object [] {CommonString.getFormatPara(request.getParameter("id"))},conn);
		usern = (String)user.get("usern");
		...
		List<Map> roleList = dbHelper.getMapList(" select resource_uuid from "+tablePrefix+"common_role_resource where usern = ? ",new Object [] {usern},conn);
...
function doSub(dialog){
	...
	jQuery.ajax({
		...
		type:'post',
		data:{'flag':'set_power_user','power':powers,'usern':'<%=usern %>'},
		async:false,
