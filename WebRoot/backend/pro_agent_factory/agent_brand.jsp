...
<%
		...
		int rId6=0;String brandId6="",brandName6="",catalogNum6="",catalogName6="";
		try {
				Map model = null;
				...
				if(relateList!=null && relateList.size()>0){
					for(Map m:relateList){
						...
						if(num==6){
							rId6=CommonString.getFormatInt(CommonString.getFormatPara(m.get("id")));
							brandId6=CommonString.getFormatPara(m.get("brand_id"));
							brandName6=CommonString.getFormatPara(m.get("brand_name"));
							catalogNum6=CommonString.getFormatPara(m.get("catalog_num"));
							catalogName6=CommonString.getFormatPara(m.get("catalog_name"));
						}
					}
				}
    ...
    <script type="text/javascript">
        $(function ()
        {
            ...
            var combo1 = $("#catalog_num1").ligerComboBox({
                width: 300,
                selectBoxWidth: 300,
                selectBoxHeight: 400, valueField: 'id',textField: 'id',
			...
			var combo5 = $("#catalog_num5").ligerComboBox({
                width: 300,
                selectBoxWidth: 300,
                selectBoxHeight: 360, valueField: 'id',textField: 'id',
                tree: { url: 'tree/json.txt', ajaxType: 'get', idFieldName: 'id' }
            });
			var combo6 = $("#catalog_num6").ligerComboBox({
                width: 300,
                selectBoxWidth: 300,
                selectBoxHeight: 330, valueField: 'id',textField: 'id',
                tree: { url: 'tree/json.txt', ajaxType: 'get', idFieldName: 'id' }
            });
			...
			%>
			combo1.selectValue("<%=catalogNum1%>","<%=catalogName1%>");
			<%
			}
			%>
			<%
			...
			%>
			combo2.selectValue("<%=catalogNum2%>","<%=catalogName2%>");
			<%
			}
			%>
			<%
			...
			%>
			combo3.selectValue("<%=catalogNum3%>","<%=catalogName3%>");
			<%
			}
			%>
			<%
			...
			%>
			combo4.selectValue("<%=catalogNum4%>","<%=catalogName4%>");
			<%
			}
			%>
			<%
			...
			%>
			combo5.selectValue("<%=catalogNum5%>","<%=catalogName5%>");
			<%
			}
			%>
			<%
			if(!"".equals(catalogNum6)){
			%>
			combo6.selectValue("<%=catalogNum6%>","<%=catalogName6%>");
			<%
			}
			%>
        });
...
<form name="theform" id="theform" action="/backend/action/after_own.jsp">
  <div class="formDiv">
	...
	<!--第六个-->
    <div class="formLeft" style="width:60px;"> 代理品牌6 </div>
    <div class="formRight" style="width:140px; ">
      <select name="select_brand6" id="select_brand6" class="jr_select" style="width: 140px;" onchange="changeBrand6();">
        <option value="">-请选择-</option>
        <%if(brandList != null){for(Map m : brandList){%>
        <option value="<%=m.get("id")%>" <%if(brandId6.equals(CommonString.getFormatPara(m.get("id")))){ out.print("selected"); }%>><%=CnToFullSpell.getFullSpell(CommonString.getFormatPara(m.get("name")).substring(0,1)).toUpperCase().substring(0,1)%>-<%=m.get("name")%></option>
        <%}}%>
      </select>
    </div>
    <div class="formLeft" style="width:5px;"> </div>
    <div class="formRight" style="width: 0px">
	  <input style="width: 0px" type="hidden"  class="jr_text" name="brand_id6" id="brand_id6" value="<%=brandId6%>" />
      <input style="width: 0px" type="hidden"  class="jr_text" name="brand_name6" id="brand_name6" value="<%=brandName6%>" />
    </div>
	<div class="formLeft"  style="width:60px;"> 代理类别</div>
    <div class="formRight" style="width: 200px">
      <input type="text"  class="jr_text" name="catalog_num6" id="catalog_num6" value="<%=catalogNum6%>" />
    </div>
    <div class="formLeft" style="width:5px;"> </div>
    <div class="formRight" style="width: 300px">
	  <input name="relate_id6" id="relate_id6" type="hidden" value="<%=rId6%>"/>
...
<script type="text/javascript">
	...
	function changeBrand6(){
		$('#brand_id6').val($('#select_brand6').val());
		$('#brand_name6').val($('#select_brand6').find('option:selected').text().substring($('#select_brand6').find('option:selected').text().indexOf('-')+1));
