...
<%@page import="com.jerehnet.util.dbutil.DBHelper"%><%
	...
	String whereStr = " and  is_show=1   and id not in (select product_id FROM pro_agent_products where agent_id='"+agentid+"') "; // 除去已经代理的产品id
	if(!"".equals(factoryid)){
	...
	DBHelper dbHelper = DBHelper.getInstance() ;
		List <Map> brandList = dbHelper.getMapList(" select id,name,upper_index from pro_agent_factory where  flag=1 and is_show=1 order by  upper_index asc ");
...
<script type="text/javascript">
	...
	function doSearch(){
		params = [];
		//date: 20140928  author:sunyanbin  修改查询条件bug 
		conditionStr = "";
		...
		whereStr = " and is_show=1 and id not in (select product_id FROM pro_agent_products where agent_id='"+factoryid+"')"; 
		params.push({'name':'tableName','value':tableName});
		params.push({'name':'fileds','value':fields});
