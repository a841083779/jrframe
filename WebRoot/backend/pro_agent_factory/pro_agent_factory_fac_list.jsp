...
<script type="text/javascript">
	...
	{ display : '厂商名称', name : 'name', align : 'left' , type : 'string' , width : 130 ,render:function(row){
	...
	{ display : '品牌排序', align : 'center' , type : 'string' , width : 80,render:function(row){
	...
	{ display : '产品列表推荐', name : 'view_count', align : 'center' , type : 'string' , width :80,render:function(row){
		return "<a class='link' href='javascript:;' onclick='catalogrecommend(\""+row.id+"\");'>操作</a>" ;
	} } ,
	
	
	{ display : '销售政策', name : 'view_count', align : 'center' , type : 'string' , width :60,render:function(row){
		return "<a class='link' href='javascript:;' onclick='salePolicy(\""+row.id+"\");'>操作</a>" ;
	} } ,
	
	
	{ display : '合同时间', name : 'order_end_time', align : 'center' , type : 'string' , width : 110 ,render:function(row){
	...
	function doSearch(){
		...
		if(''!=jQuery.trim(full_name)){
			conditionStr += " and  (full_name like '%"+jQuery.trim(full_name)+"%' or  name like '%"+jQuery.trim(full_name)+"%') ";
		}
	...
	function salePolicy(id){
		if(id.length>0){
            jrWindow("/backend/pro_agent_factory/pro_agent_sale_policy_list.jsp?facId="+id,"厂家政策",800,400);
        }
