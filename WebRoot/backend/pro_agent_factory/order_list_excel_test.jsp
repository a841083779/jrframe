...
if_tag="1";
	if("1".equals(if_tag))
	{
		...
		{
			whereStr.append("and (province like '%云南%' or province like'%贵州%' or province like '%四川%' or province like '%重庆%' or province like '%西藏%' or province like '%新疆%' or province like '%青海%' or province like '%甘肃%' or province like '%宁夏%') and add_date >='2019-07-15 00:00:00' and add_date<'2019-07-22 00:00:00'") ;
		}
...
<%
	...
	if(null!=pro_List && pro_List.size()>0){
		...
		for(int i=0;i<pro_List.size();i++){
			...
			ws.addCell(label);
			String pc_state="未处理";
			if(CommonString.getFormatPara(oneMap.get("follow_state")).equals("1")){
				 pc_state="跟踪中";
			}
			if(CommonString.getFormatPara(oneMap.get("follow_state")).equals("3")){
				pc_state="已成交";
			}
			if(CommonString.getFormatPara(oneMap.get("follow_state")).equals("4")){
								pc_state="流失";

			}
			label = new Label(21,(i+1),pc_state) ;
			ws.addCell(label);
		}
		}else if("2".equals(if_tag)){
		    Map oneMap = null ;
