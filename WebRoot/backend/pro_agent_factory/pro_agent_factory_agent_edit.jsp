<%@ page language="java" import="java.util.*,java.sql.*,com.jerehnet.util.dbutil.*,com.jerehnet.util.common.*" pageEncoding="UTF-8"%><%
	...
	try{
		...
		List <Map> brandList = dbHelper.getMapList(" select id,name,upper_index from pro_agent_factory where flag = 1 and is_show=1 order by upper_index asc ",connection);
	...
	<form name="theform" id="theform" action="/backend/action/crud.jsp">
	    ...
	    <div class="formRight" style="height: 220px; text-align: left;"> 
	      ...
	      <input type="hidden" name="zd_introduce" id="zd_introduce" value='' />
				...
				<div style="clear:left;"></div>
				<div class="formLeft">推送是否显示</div>
				<div class="formRight" style="width: 80px;">
					<%=CommonForm.createIsRadio(CommonString.getFormatPara(model.get("push_show")).equals("")?"0":CommonString.getFormatPara(model.get("push_show")),"zd_push_show") %>
				</div>
