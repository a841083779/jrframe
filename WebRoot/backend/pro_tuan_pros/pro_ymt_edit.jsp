  	...
  	<div class="searchForm">
		...
		<div class="searchLeft">产品名称</div>
		  		<div class="searchRight">
		  			<input type="text" id="productname" name="productname" style="width: 150px;" class="jr_text" />
		  		</div>
  		<div class="searchLeft">品牌</div>
		  		<div class="searchRight">
		  			<select class="jr_select" id="factoryid" name="factoryid">
                    	<option value="">--请选择品牌--</option>
		  				<%if(brandList != null){for(Map brand : brandList){%>
                        	<option value="<%=brand.get("id")%>"><%=CnToFullSpell.getFullSpell(CommonString.getFormatPara(brand.get("name")).substring(0,1)).toUpperCase().substring(0,1)%>-<%=brand.get("name")%></option>
                        <%}}%>
		  			</select>
		  		</div>
				
				<!--<div class="searchLeft">代理商</div>
		  		<div class="searchRight">
	  				<select class="jr_select" id="agent" name="agent">
                   	<option value="">--请选择代理商--</option>
	  				<%if(daili != null){for(Map dai : daili){%>
                       	<option value="<%=dai.get("name")%>" <%=dai.get("name").equals(catalognum)?"selected=\"selected\"":""%>><%=CnToFullSpell.getFullSpell(CommonString.getFormatPara(dai.get("name")).substring(0,1)).toUpperCase().substring(0,1)%>-<%=dai.get("name")%></option>
                       <%}}%>
	  			</select>
		  		...
		  		<div class="searchRight">
	  				...
	  				<%if(catalogList != null){for(Map m : catalogList){%>
                       	<option value="<%=m.get("num")%>" <%=m.get("num").equals(catalognum)?"selected=\"selected\"":""%>><%=CommonForm.getSpace(m.get("num").toString(),1)%><%=CnToFullSpell.getFullSpell(CommonString.getFormatPara(m.get("name")).substring(0,1)).toUpperCase().substring(0,1)%>-<%=m.get("name")%></option>
                       <%}}%>
	  			</select>
...
function getShowName(row){
   ...
   var ids="";
	...
	function doSearch(){
		...
		if(''!=jQuery.trim(agent)){
		conditionStr += " and factoryname  like '"+agent+"%'" ;
		}
		
		var factoryid = jQuery("#factoryid").val() ;

		if(''!=jQuery.trim(factoryid)){
		conditionStr += " and factoryid  like '"+factoryid+"%'" ;
		}
