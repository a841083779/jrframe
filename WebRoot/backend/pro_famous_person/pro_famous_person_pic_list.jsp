...
<%
	String tableName = "pro_famous_person_pic";
...
<script type="text/javascript">
	...
	function doSort(){
		...
		jQuery.ajax({
			...
			type:"post",
			data:{'flag':'saveSort',"tableName":"pro_famous_person_pic","sortId":sortHtml},
			async:false,
			cache:false
		});
	}
	...
	function saveImgToDb(){
		...
		var data = {
			flag : 'saveSubImg',
			table_name : 'pro_famous_person_pic',
			pro_id : '<%=id %>',
			channel_no : '',
			imgStr : str
		};
	...
	jQuery(".miaoshu").blur(function(){
		...
		if(oldval!=selfval){
			jQuery.post("/backend/action/ajax_cud.jsp",{
				'flag':'commonUpdate',
				'tableName':'pro_famous_person_pic',
				'id':id,
				'key':'title',
				'value':selfval
			});
	...
	jQuery("#del_img").click(function(){
		...
		jrConfirm("确定要删除选中的"+blue.length+"张图片吗？",function(v){
			if(v){
				...
				jQuery.ajax({
					url:'/backend/action/ajax_cud.jsp',
					data:{'flag':'groupDel','ids':ids,'tableName':'pro_famous_person_pic'},
					type:'post',
					async:false,
