...
<script type="text/javascript">
   ...
   { display : 'ID', name : 'id', align : 'left' , type : 'string' , width : 50 } ,
	{ display : '', name : 'img', align : 'left' , type : 'string' , width : 40,render:function(row){
		var img = row.img;
		if(img == '' || img == 'null' || img == null){
			img = 'no_small.gif';	
		}
		return "<img src='/uploadfiles/"+img+"' width='35'  height='24' />";
		} } ,
	{ display : '姓名', name : 'name', align : 'left' , type : 'string' , width : 120 ,render:function(row){
		return "<a class='link' href='javascript:void(0);' onclick='addAndEdit("+row.__index+");'>"+row.name+"</a>";
	}} ,
	{ display : '厂家', name : 'factoryname', align : 'left' , type : 'string' , width : 120 } ,
	{ display : '点击量', name : 'view_count', align : 'center' , type : 'string' , width : 100 } ,
	{ display : '是否显示', name : 'is_show', align : 'center' , type : 'is' , width : 100 } , 
	{ display : '最后修改日期', name : 'last_modify_date', align : 'center' , type : 'string' , width : 120 } ,
	{ display : '操作', align : 'center' , type : 'is' , width : 250 ,render : function(row){
	...
	function openWin(id,name){
		jrWindow("/backend/pro_famous_person/ipt_product_temp_products_list.jsp?pro_name="+name+"&pid="+id,"导入产品",900,600);	
	}
	function showPicList(pro_id){
		jrWindow("/backend/pro_famous_person/pro_famous_person_pic_list.jsp?id="+pro_id+"&channel_no=","产品图片",800,600);
	}
