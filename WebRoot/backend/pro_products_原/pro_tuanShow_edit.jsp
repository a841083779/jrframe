   ...
   <input type="hidden" class="jr_text" name="fa_id" id="fa_id"   style="width:270px"/>
    ...
    <div class="formRight">
      ...
      <%if(catalogList != null){for(Map m : catalogList){%>
       <option value="<%=m.get("num")%>" <%if(CommonString.getFormatPara(model.get("pro_catanum")).equals(m.get("num").toString())){%> selected="selected"<%}%>><%=CommonForm.getSpace(m.get("num").toString(),1)%><%=m.get("name")%></option>
         <%}}%>
      </select>
    </div>
    <div class="formRight">
    ...
    <div class="formRight">
	 <select class="jr_select" id="zd_pro_name" name="zd_pro_name" >
        <option value="<%=CommonString.getFormatPara(model.get("pro_name")) %>"><%=CommonString.getFormatPara(model.get("pro_name")) %></option>
      </select>
    </div>
	<div style="clear:left;"></div>
    <div style="clear: left;"></div>
