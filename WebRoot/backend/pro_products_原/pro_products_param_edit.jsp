...
<%
	...
	try{
	    ...
	    isCheckMap.put("0","否") ;
	    isCheckMap.put("1","是") ;
	    //if(no.equals("101002008")){no="101002";}
...
<form name="theform" id="theform" action="/backend/action/crud.jsp">
  <div class="formDiv">
    <div id="paramId">
      ...
      <%
			for(Map m : categoryParamsList){	
			  ...
			  if(!CommonString.getFormatPara(model.get("param_update")).equals("")){
				  param=CommonString.getFormatPara(model.get("param_update"));
			  }
	...
	%>
      ...
      <div class="formRight" style="width:190px">
        <input style="width:190px" type="text" class="jr_text" name="param" id="<%=CommonString.getFormatPara(model.get("id"))%>" lang="'product_no':'<%=no%>' ,'no':'<%=cataolgryParamId%>' , 'name':'<%=m.get("name")%>' , 'type':'1', 'order_no':'0' , 'enum_no':'0' " value="<%=param_value%>" maxlength="50"/>
      </div>
  ...
  <%if(adminName.equals("mujie")||adminName.equals("admin")||adminName.equals("zhangxuan")||adminName.equals("zhangjx")){%>
  <div class="formLeft">是否审核</div>
  <div class="formRight" style="width:100px;"> <%=CommonForm.createRadio(isCheckMap,CommonString.getFormatPara(model.get("is_check")).equals("")?"0":CommonString.getFormatPara(model.get("is_check")),"zd_is_check") %> </div>
  <%}else{%>
    <input type="hidden" name="zd_is_check" id="zd_is_check" value="0" />
  <%}%>
  <div style="clear: left;"></div>
  ...
  <input type="hidden" name="zd_param_update" id="zd_param_update" value="" />
