<%@ page language="java" import="java.util.*,com.jerehnet.util.common.*" pageEncoding="UTF-8"%><%
	...
	String fileds = "id,add_date,factoryname,pro_catanum,catalogname,name,img2,file_name,view_count,is_show,title";
...
<script type="text/javascript">
	...
	{ display : '活动类型', name : 'pro_type', align : 'left' , type : 'pro_type' , width : 100 ,render:function(row){
	if(row.pro_type==0){
		return "<a class='link' href='javascript:void(0);' '>多产品</a> ";
		}else{
		return "<a class='link' href='javascript:void(0);' '>单产品</a> ";
		}
	} },
	{ display : '品 牌', name : 'factoryname', align : 'left' , type : 'string' , width : 100 } ,
	...
	if(row.pro_type==0){
		...
		return "<a class='link' href='javascript:void(0);' onclick='subject_edit("+row.id+");'>操作</a>      <a class='link' href='javascript:void(0);' onclick='subject_tuji("+row.id+");'>[图集]</a>";
		}
	} },
	{ display : '是否显示', name : 'is_show', align : 'left' , type : 'is' , width : 110 } ,
	{ display : '排序', align : 'center' , width : 100 , render : function(row){
				return "<a class='link' href='javascript:void(0);' onclick=newsSort({parentId:\""+row.id+"\"});>排序</a>";
		    }},
	{ display : '同类型排序', align : 'center' , width : 100 , render : function(row){
				return "<a class='link' href='javascript:void(0);' onclick=leiSort({parentId:\""+row.pro_catanum+"\"});>同类型排序</a>";
		    }}		

	];
	//排序
	function newsSort(arg){
		var url = "/backend/pro_tuan/sort_subject.jsp";
		url += "?parentId=" + arg.parentId;
		jrWindow(url, "排序", 500, 400);
	}
	//排序
	function leiSort(arg){
		var url = "/backend/pro_tuan/leisort_subject.jsp";
		url += "?parentId=" + arg.parentId;
		jrWindow(url, "排序", 500, 400);
	}
	//增加和编辑的方法
	...
	//图集
/*	function subject_tuji(no) {
		var _url = "/backend/pro_tuan_pros/pro_tuan_tuji_list.jsp?id="+no;
		var _title = "图片";
		var iWidth = 800;
		var iHeight = 600;
		var iTop = (window.screen.height - 30 - iHeight) / 2;
...
///图集
	function subject_tuji(no) {
		var _url = "/backend/pro_tuan_pros/pro_photo_list.jsp?id="+no;
		var _title = "图片";
		var iWidth = 800;
		var iHeight = 600;
		var iTop = (window.screen.height - 30 - iHeight) / 2;
