...
<%
	...
	List <Map> procatalogList = null ;
	 ...
	 List<Map> daili = dbHelper.getMapList(" select  name,city from pro_agent_factory where is_cooperate=?",new Object[]{1}) ;//代理商
  ...
  <div style="clear:left;"></div>
    ...
    <div class="formRight">
      <select class="jr_select" id="zd_pro_factoryname" name="zd_pro_factoryname" onchange="getarea(this.options[this.selectedIndex].value);">
        ...
        <option value="<%=dai.get("name")%>"><%=dai.get("name")%></option>
        <%}}%>
      </select>
    </div>
    ...
    <div class="formRight" style="width:250px; text-align:left" id="pr_place">
     <input type="text" class="jr_text" name="zd_agent_area" id="zd_agent_area" value="" style="width:250px" />
      </div>
	  <div style="clear:left;"></div>
    ...
    <input type="hidden" class="jr_text" name="zd_factoryname" id="zd_factoryname" value=""  style="width:270px"/>
    ...
    <div class="formRight">
      ...
      <%if(catalogList != null){for(Map m : catalogList){%>
       <option value="<%=m.get("num")%>" <%if(CommonString.getFormatPara(model.get("catalognum")).equals(m.get("num").toString())){%> selected="selected"<%}%>><%=CommonForm.getSpace(m.get("num").toString(),1)%><%=CnToFullSpell.getFullSpell(CommonString.getFormatPara(m.get("name")).substring(0,1)).toUpperCase().substring(0,1)%>-<%=m.get("name")%></option>
         <%}}%>
      </select>
    ...
    <div class="formRight" >
	 <select class="jr_select" id="zd_pro_name" name="zd_pro_name"  onchange="jQuery('').val(jQuery('#zd_pro_name').find('option:selected').text());">
        <option value="">--请选择产品型号--</option>
      </select>
    </div>
     <!--  <input type="hidden" class="jr_text" name="zd_catalognum_spe" id="zd_catalognum_spe" value="" style="width:270px" />-->
