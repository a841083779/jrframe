	...
	try{
		...
		if(eventList.contains("updateFormTask2Count")){
			...
			if("103004".equals(zd_call_rs_task2)){
				//结束订单，并添加任务2数量
				sql = " update pro_product_form set task2_count = isnull(task2_count,0)+1 , task2_over = 1 , call_rs_task2 = ?,call_result2=?,is_high_quality=?,is_star=?,factoryname=?,cataname=?,product_name=? where uuid = ?  ";
				dbHelper.execute(sql,new Object [] {zd_call_rs_task2,contents,is_high_quality,is_push,main_brand,main_category,main_model,formUUID},connection);
				...
				if(isUpdateForm){
					...
					sql += " , task2_count = isnull(task2_count,0)+1 , task2_over = ? , call_rs_task2 = ?,call_result2=?,is_high_quality=?,is_star=?,factoryname=?,cataname=?,product_name=? ";
					sql += " where uuid = ? ";
					Float main_price = 0f;
					try{
					...
					dbHelper.execute(sql,new Object [] {
							...
							is_high_quality,
							is_push,
							main_brand,main_category,main_model,
							formUUID
					},connection);
				}else{
					//更新当前订单的呼叫状态
					dbHelper.execute(" update pro_product_form set call_rs_task2 = ? , task2_count = isnull(task2_count,0)+1 ,call_result2=?,is_high_quality=?,is_star=?,factoryname=?,cataname=?,product_name=?  where uuid = ? ",new Object [] {zd_call_rs_task2,contents,is_high_quality,is_push,main_brand,main_category,main_model,formUUID},connection);
				}
			}else{
				//更新当前订单的呼叫状态
				dbHelper.execute(" update pro_product_form set call_rs_task2 = ? , task2_count = isnull(task2_count,0)+1 ,call_result2=?,is_high_quality=?,is_star=?,factoryname=?,cataname=?,product_name=? where uuid = ? ",new Object [] {zd_call_rs_task2,contents,is_high_quality,is_push,main_brand,main_category,main_model,formUUID},connection);
		...
		if(eventList.contains("updateAgentFormsss")){
			...
			String jr_remark = CommonString.getFormatPara(request.getParameter("jr_remark"));
			//获取合作客户是厂家还是代理商
			...
			sqlString += "add_ip,add_date,uuid,factory_id,factory_name,agent_id,agent_name,cata_num,cata_name,product_id,product_name,contact_address,contact_tel,province,city,state,form_date,message,company_name,user_name,push_order,source_id,call_result1";
			sqlString += ") select add_ip,'"+CommonDate.getToday("yyyy-MM-dd HH:mm:ss")+"','"+CommonString.getUUID()+"',"+brand_id+",'"+company_name+"',"+factory_id+",agentname,catanum,'"+cata_name+"',"+pro_id+",'"+pro_name+"',contact_address,mobile_phone,province,city,state,'"+CommonDate.getToday("yyyy-MM-dd HH:mm:ss")+"','"+jr_message+"',company,name,"+1+",id,call_result1";
			...
			sqlCopyString += "add_ip,add_user,add_date,channel_uuid,factoryid,factoryname,agentid,agentname,catanum,cataname,name,email,company,product_id,product_name,contact_address,contact_tel,message,disposal_content,state,province,city,mobile_phone,view_count,uuid,is_send,buy_time,ifgroup,is_success,call_rs,call_rs_task2,is_high_quality,is_push,task1_count,task2_count,task1_over,task2_over,from_factoryid,from_factoryname,rebate,is_link,is_buy,is_buy_self,not_buy_reson1,not_buy_reson2,brand,category,model,price,is_intention,plan_to_buy,intention_brand,intention_category,intention_model,intention_price,other_brand,other_category,other_model,other_price,is_lock_for_agent,agent_can_view,dispose_ps,referer,to_brands,is_deal,deal_desc,is_first,push_order";
			sqlCopyString += ") select add_ip,add_user,'"+CommonDate.getToday("yyyy-MM-dd HH:mm:ss")+"',channel_uuid,"+brand_id+",'"+company_name+"',"+factory_id+",agentname,catanum,'"+cata_name+"',name,email,company,"+pro_id+",'"+pro_name+"',contact_address,contact_tel,'"+jr_message+"',disposal_content,state,province,city,mobile_phone,view_count,'"+CommonString.getUUID()+"',is_send,buy_time,ifgroup,is_success,call_rs,call_rs_task2,is_high_quality,is_push,task1_count,task2_count,task1_over,task2_over,from_factoryid,from_factoryname,rebate,is_link,is_buy,is_buy_self,not_buy_reson1,not_buy_reson2,brand,category,model,price,is_intention,plan_to_buy,intention_brand,intention_category,intention_model,intention_price,other_brand,other_category,other_model,other_price,is_lock_for_agent,agent_can_view,dispose_ps,referer,to_brands,is_deal,deal_desc,"+0+","+1+"";
		...
		if(eventList.contains("ymtActivity")){
			...
			sql+= "add_ip,add_user,add_date,uuid,is_show,description,img,factoryname,factorylogo,factoryid,agent_fac,agent_area,flag,pro_name,pro_catanum,pro_cataname,pro_factoryid,pro_factoryname,pro_start_price,pro_end_price,pro_uuid,is_show,tonandmeter,pro_id)";
            sql+="values('"+Common.getIp(request)+"','"+add_user+"','"+CommonDate.getToday("yyyy-MM-dd HH:mm:ss")+"','"+CommonString.getUUID()+"',"+is_show+",'"+jr_textarea+"','"+jr_activity_img+"','"+factoryname+"','"+factorylogo+"',"+factoryid+",'"+agent_fac+"','"+agent_area+"',"+jf_flag+",'"+pro_name+"','"+pro_catanum+"','"+pro_cataname+"',"+pro_factoryid+",'"+pro_factoryname+"',"+pro_start_price+","+pro_end_price+",'"+pro_uuid+"',1,"+tonandmeter+","+pro_id+")";
