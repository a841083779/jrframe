...
<script type="text/javascript">
	...
	columnsArr =  [ 
		...
		{ display : '图库标题', name : 'title', isSort : false ,  id : 'no' , align : 'left' , type : 'string' , width : 250 , render : function(row){
				return "<a class='link' href='javascript:void(0);' onclick='addAndEdit("+row.__index+");'>"+row.title+"</a>";
		}},
		{ display : '是否有图片', isSort : false , name : 'is_display', align : 'center' , type : 'is' , width : 60 , render:function(row){
			if((jQuery.trim(row.is_display)==0)){
				return ("纯文字" ) ;
			}else if((jQuery.trim(row.is_display)==1)){
				return ("有图片" ) ;
			}
		}},
	...
	function addAndEdit(index){
    	if(undefined==index){//如果是增加
    	    toolbarAdd({
	    		...
	    		height:500,
	    		title:'增加',
				fileName:"pro_wajueji/pro_wajueji_pic/pro_wajueji_pic",
