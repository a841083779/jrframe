...
function setProductShow(arg) {
	...
	jrConfirm(((typeof(arg.flag)!='undefined' && arg.flag=='setProductShow')?"确定显示所选产品吗？":"确定隐藏所选产品吗？"), function(v) {
				if (v) {
					var checkedRows = grid.getCheckedRows();
					var idStr = "";
					for (var i = 0; i < checkedRows.length; i++) {
					...
					if(typeof(arg.flag)!='undefined' && arg.flag=='setProductShow'){
					  executeSetProductShow(arg) ;// 显示所选产品
					}else if(typeof(arg.flag)!='undefined' && arg.flag=='setProductHide'){
					executeSetProductShow(arg) ; // 隐藏所选产品
				}
				}
			});
}
function executeSetProductShow(arg){
 	 		jQuery.ajax({
