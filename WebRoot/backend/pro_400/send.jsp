...
<%
	...
	try {
		...
		Map toOrderPhone = new HashMap() ; // 发送订单的电话
		toOrderPhone.put("") ;
		
		String sel_sql = "select to_brands from pro_product_form where id=?" ;  // 该条订单是否已经推荐 
		String to_orders_phone = "" ;
		Map orderInfoMap = null ;
		String to_brands = "" ;// 推荐的品牌
		String[] to_brandS = null ;
		if(!"".equals(productId)){
			orderInfoMap = dbHelper.getMap(sel_sql,new Object[]{productId},connection) ;
		}
		if(null!=orderInfoMap && orderInfoMap.size()>0){
			to_brands = CommonString.getFormatPara(orderInfoMap.get("to_brandS")) ;
		}
		if(!"".equals(to_brands)){
			to_brandS = to_brands.split(",") ;
		}
		if(null!=to_brandS && to_brandS.length>0){ 
			for(int i=0;i<to_brandS.length;i++){
				if(!"".equals(to_brandS[i])){
					System.out.println(to_brandS[i]+"--") ;
				}
			}
		}
		System.out.println(orderInfoMap+"-"+productId) ;
	// 浙江成峰 特殊情况
		...
		String content = "中国工程机械商贸网提醒你：" + CommonString.getFormatPara(request.getParameter("param")) + "[www.21-sun.com]";
