...
<%
	...
	String fileds = "add_user,is_special,catanum,is_send,id,province,city,name,mobile_phone,factoryname,product_name,cataname,add_date,call_rs,factoryid,is_success,is_first,product_id,uuid,contact_address,call_rs_task2,is_interested";
    ...
    <div class="searchCondition">
      ...
      <div class="searchRight" >
        <select name="tonandmeter" id="tonandmeter" class="jr_select" style="width: 100px;"  onchange="jQuery('#zd_tonandmeter').val(jQuery.trim(jQuery('#tonandmeter').find('option:selected').val()));getProName(this.options[this.selectedIndex].value);">
          ...
          <option value="4">0T～4T(微挖)</option>
          <option value="5">4T～8T(小挖)</option>
          <option value="6">8T～15T(中小挖)</option>
		  <option value="7">15T～25T(中挖)</option>
		  <option value="8">25T～35T(中大挖)</option>
		  <option value="9">35T以上(大挖)</option>
        </select>
	  ...
	  <%if(usern.equals("zhangjx")||usern.equals("admin")||usern.equals("mujie")||usern.equals("jiangcc")){%>
	     <div style="clear:left"></div>
	  ...
	  <div class="searchLeft" style="width:70px;">接口分类</div>
      <div class="searchRight">
       <div class="jr_radio_group">
	   <input class="jr_radio" type="radio" name="is_cat_phone" id="is_cat_phone0" value="1"><label for="is_cat_phone0">铁臂帮</label>
	   <input class="jr_radio" type="radio" name="is_cat_phone" id="is_cat_phone2" value="2"><label for="is_cat_phone2">360优利</label>
	   <input class="jr_radio" type="radio" name="is_cat_phone" id="is_cat_phone1" value="0"><label for="is_cat_phone1">正常</label>
	   <input class="jr_radio" type="radio" name="is_cat_phone" id="is_cat_phone3" value="-1"><label for="is_cat_phone3">全部</label>
	   </div>
	  </div>
	  
	  
	  <div class="searchLeft" style="width:70px;">卡特活动</div>
      <div class="searchRight">
       <div class="jr_radio_group">
	   <input class="jr_radio" type="radio" name="is_360" id="is_3600" value="1"><label for="is_3600">是</label>
	   <input class="jr_radio" type="radio" name="is_360" id="is_3602" value="0"><label for="is_3602">否</label>
	   <input class="jr_radio" type="radio" name="is_360" id="is_3603"  ><label for="is_3603">全部</label>
	   </div>
	  </div>
...
<script type="text/javascript">
	...
	//列数组
	columnsArr =  [ 
		{ display : 'ID', name : 'id', align : 'center' , type : 'string' , width : 65 , render : function(row){
		...
		{ display : '联系人', name : 'name', align : 'center' , type : 'string' , width : 100 ,render:function(row){
				if(row.add_user==='2018spring_cat_pc'){
					if(row.is_interested==='1'){
						return row.name+"(360全程安心pc<span style='color:red;'>意向购买</span>)" ;
					}else{
						return row.name+"(360全程安心pc)" ;
					}
				}else if(row.add_user==='2018spring_cat_mobile'){
					if(row.is_interested==='1'){
						return row.name+"(360全程安心mobile<span style='color:red;'>意向购买</span>)" ;
					}else{
						return row.name+"(360全程安心mobile)" ;
					}
				}else{
					if(row.product_id=='0'&&row.factoryid=='174'){
		...
		{ display : '询价产品', align : 'left' , type : 'string' , width : 175 , render : function(row){
		...
		if(row.factoryid=="14238"|| row.factoryid=="16566"||row.factoryid=="154" ||row.factoryid=="1777"||row.factoryid=="151"){
	    ...
	    { display : '是否推送', name : 'is_success', align : 'center' , type : 'string' , width : 90,render:function(row){
			if(row.factoryid=="174"||row.factoryid=="138"||row.factoryid=="133"||row.factoryid=="139" ||row.factoryid=="209"  ||row.factoryid=="192" ){
				if(row.is_success=="1"){
					   if(row.is_special=="1"){
		...
		<%	if("zhangjx".equals(usern)||"admin".equals(usern)){%>
	    { display : '订单推送', name : '', align : 'center' , type : 'string' , width : 320,render:function(row){
			if(row.factoryid=="174"){
				//if(row.add_user==='2018spring_cat_pc'){
				//	return "<a class='link' href='javascript:void(0);' onclick='sendOrder("+row.__index+",1);'>推送</a>"+"&nbsp;&nbsp;||&nbsp;&nbsp;"+"<a class='link' href='javascript:void(0);' onclick='sendOrder("+row.__index+",98);'>360PC</a>";
				//}else if(row.add_user==='2018spring_cat_mobile'){
				//	return "<a class='link' href='javascript:void(0);' onclick='sendOrder("+row.__index+",1);'>推送</a>"+"&nbsp;&nbsp;||&nbsp;&nbsp;"+"<a class='link' href='javascript:void(0);' onclick='sendOrder("+row.__index+",97);'>360MOBILE</a>";
				//}
				if(row.product_id===31913||row.product_id===31918){
					return "<a class='link' href='javascript:void(0);' onclick='sendOrder("+row.__index+",1);'>推送</a>";
				}else if(row.product_id===31828||row.product_id===31829){
					return "<a class='link' href='javascript:void(0);' onclick='sendOrder("+row.__index+",1);'>推送</a>";                        
				}else if(row.product_id===31914){
					return "<a class='link' href='javascript:void(0);' onclick='sendOrder("+row.__index+",1);'>推送</a>";
				}else if(row.product_id===30221||row.product_id===30220||row.product_id===30219){
					return "<a class='link' href='javascript:void(0);' onclick='sendOrder("+row.__index+",1);'>推送</a>";
				}else{
					return "<a class='link' href='javascript:void(0);' onclick='sendOrder("+row.__index+",1);'>推送</a>";
				}
				
			}if(row.factoryid=="209209209"){
				return "<a class='link' href='javascript:void(0);' onclick='sendOrder("+row.__index+",10);'>推送</a>";
			}else if(row.factoryid=="138"){
				return "<a class='link' href='javascript:void(0);' onclick='sendOrder("+row.__index+",2);'>推送</a>";
			}else if(row.factoryid=="192192192"){
				return "<a class='link' href='javascript:void(0);' onclick='sendOrder("+row.__index+",99);'>推送</a>";
	...
	function sendOrder(index,flag){
		...
		if(flag=="1"){
		   ...
		   url = "/tools/order_interface/cat_admin_360.jsp?uuid="+row.uuid;
		}else if(flag=="98"){
		   url = "/tools/order_interface/cat_admin_yl.jsp?flag=pc&uuid="+row.uuid;

		}else if(flag=="97"){
		   url = "/tools/order_interface/cat_admin_yl.jsp?flag=mobile&uuid="+row.uuid;
		}
		else if(flag=="10"){
		   url = "/tools/order_interface/xcmg_admin.jsp?uuid="+row.uuid;
		}else if(flag=="99"){
		   url = "/tools/order_interface/ds_admin.jsp?uuid="+row.uuid;
		}
		else if(flag=="160"){//pc
		   url = "/tools/order_interface/cat_admin_160.jsp?uuid="+row.uuid+"";
		}
		else if(flag=="100"){//pc
		   url = "/tools/order_interface/cat_admin_15.jsp?uuid="+row.uuid+"&flag=pc";
		}else if(flag=="101"){//mobile
		   url = "/tools/order_interface/cat_admin_15.jsp?uuid="+row.uuid+"&flag=mobile";
		}else if(flag=="102"){//mobile
		   url = "/tools/order_interface/cat_admin_huodong.jsp?uuid="+row.uuid+"&flag=mobile";
		}else if(flag=="156"){//mobile
		   url = "/tools/order_interface/cat_admin_bauma.jsp?uuid="+row.uuid+"&flag=m";
		}else if(flag=="157"){//mobile
		   url = "/tools/order_interface/cat_admin_bauma.jsp?uuid="+row.uuid+"&flag=pc";
		}else if(flag=="170"){//mobile
		   url = "/tools/order_interface/cat_admin_anxin.jsp?uuid="+row.uuid;
		}else if(flag=="200"){
		   url = "/tools/order_interface/shangong_admin_bices2019.jsp?uuid="+row.uuid;
		}
		...
		if((flag=="5"||flag=="4") ){
			 ...
			 $.ajax({
		        ...
		        type:"post",
				cache:false,
		        data:{"flag":"checkRepeatSany","mobile_phone":row.mobile_phone},
		        success:function(data){
					if($.trim(data)=='norepeat'){
						    jrWindow(url, "订单推送", 900, 600);
					}else{
	...
	function doSearch(){
		...
		var tonandmeter=jQuery("#zd_tonandmeter").val();
          if(tonandmeter=='0'){
	        ...
	        conditionStr += " and product_id in(select id from pro_products where tonandmeter > 0 and tonandmeter <4 and catalognum like '101001%')";
       }else if(tonandmeter=='5'){
	        conditionStr += " and  product_id in(select id from pro_products where tonandmeter > 4 and tonandmeter <=8 and catalognum like '101001%') ";
       }else if(tonandmeter=='6'){
	       conditionStr += " and  product_id in(select id from pro_products where tonandmeter >8  and tonandmeter <=15 and catalognum like '101001%') ";
       }else if(tonandmeter=='7'){
	        conditionStr += " and product_id in(select id from pro_products where tonandmeter > 15 and tonandmeter <=25 and catalognum like '101001%')";
       }else if(tonandmeter=='8'){
	        conditionStr += " and  product_id in(select id from pro_products where tonandmeter > 25 and tonandmeter <=35 and catalognum like '101001%') ";
       }else if(tonandmeter=='9'){
	       conditionStr += " and  product_id in(select id from pro_products where tonandmeter >35 and catalognum like '101001%') ";
       }          		
		...
		} else if('2'== jQuery.trim(is_cat_phone)){
			conditionStr += " and is_special  = '2' ";
		...
		if('1'== jQuery.trim(from_source)){  //pc端
			...
			if('1'== jQuery.trim(is_first)){  //主动
				...
				conditionStr += " and  ( ((is_first  is null  or  is_first=''  or  is_first='0' or  is_first=0 ) and   add_user  is null  )  or    ((is_first  is null  or  is_first=''  or  is_first='0' or  is_first=0 ) and  (add_user! ='baidu' or add_user  is null  or  add_user=''  )  and add_user not like '%mobile%'  )    or (add_user   like '%mobile%' and push_order=1)  ) ";
			}else{  //其他所有
		...
		var 	is_360 = jQuery("input[name='is_360']:checked").val();
		if(''!=jQuery.trim(is_360)){
			if(is_360=='1'){
				conditionStr += " and  product_id=0 ";
			}
			if(is_360=='0'){
				conditionStr += " and  (product_id!=0 ) ";
			}
		...
		if(''!=jQuery.trim(cat_catalog)){
			if(cat_catalog=='0'){
			    conditionStr += " and product_id in(31918,31914,31913,31829,31828,30221,30220,30219,29179,29162,28480,23921,23907,23906,23905,23371,23369,21959,21524,20735,3535,3505,9503,30219,9554,25480,9552,9551,9550,9548,9547,8351,8337,4615,378,390,400,403,407,445,522,3503,3525,4611,4613,4614,28481,9554,25480,9555,25477,25167,23911,23910,23909,23908,23364,23362,23228,21960,21958,20473,20472,20440,19947,19946,15159,11366,9585,9584,9555,25477,20423,20735,21524,21601,21607,21959,22613,23369,23371,23905,23906,23907,23921,23922,28480,29162,29171,29172,29173,29176,29178,29179,29444,30220,30221) ";
			}

			if(cat_catalog=='2'){
			  	conditionStr += " and product_id in(31921,31920,31919,31807,31806,31805,29165,29164,29163,23917,23915,23912,21525,4561,9553,21525,23912,23915,29163,29164,29165,325,574,588,590,4561,9546,20164,20163,21514,23913,23914,23916,23917,23918,23919,23920) ";
			}
