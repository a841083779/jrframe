...
<%
	...
	try{
		...
		Map root1 = new HashMap();
			...
			list = dbHelper.getMapList(" select  id,catalognum,param  from pro_products where  is_show=1    order by id  asc  ");
			for(Map model:list){
			...
			if(categoryKeyList==null&&catalognum.length()>6){
				categoryKeyList= dbHelper.getMapList(" select  id,name from pro_catalog_param where is_key=1 and parent_no='"+catalognum.substring(0,6)+"'  ");
			}
			if(categoryKeyList==null&&catalognum.length()>3){
				categoryKeyList= dbHelper.getMapList(" select  id,name from pro_catalog_param where is_key=1 and parent_no='"+catalognum.substring(0,3)+"'  ");
			}
