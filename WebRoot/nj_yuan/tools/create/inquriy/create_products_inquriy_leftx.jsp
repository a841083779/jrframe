	...
	try{
		...
		Map root1 = new HashMap();
			...
			if(list != null && list.size() > 0){
				root.put("list",list);
				freemarkerUtils.generate("inquriy/products_inquriy_catalog.htm",root,realPath+"include/inquriy/products_inquriy_catalog.htm");
				resule="true";
			}
			...
			String catalogNum="";
			   catalogNum=CommonString.getFormatPara(brand.get("num"));
				String zmSql=" select paf.upper_index as upper_index from pro_products pp ,pro_agent_factory paf where pp.factoryid = paf.id and pp.catalognum like '"+catalogNum+"%'  and pp.is_show=1 and paf.is_show=1 group by paf.upper_index order by paf.upper_index";
				...
				String sql2 ="select paf.id,paf.upper_index,paf.name,paf.usern from pro_products pp ,pro_agent_factory paf where pp.factoryid = paf.id and pp.catalognum like '"+catalogNum+"%' group by paf.upper_index,paf.name,paf.usern,paf.id order by paf.upper_index,paf.name";
				...
				if(list != null && list.size() > 0&&list1 != null && list1.size() > 0&&list2 != null && list2.size() > 0){
					...
					freemarkerUtils.generate("inquriy/products_inquriy_brand.htm",root,realPath+"include/inquriy/products_inquriy_brand_"+catalogNum+".htm");
					resule="true";
				}
		    }
				...
				if(list != null && list.size() > 0&&list1 != null && list1.size() > 0&&list2 != null && list2.size() > 0){
					...
					root.put("list2",list2);
					freemarkerUtils.generate("inquriy/products_inquriy_brand.htm",root,realPath+"include/inquriy/products_inquriy_brand_0.htm");
					resule="true";
				}
	}catch(Exception e){
		e.printStackTrace();
