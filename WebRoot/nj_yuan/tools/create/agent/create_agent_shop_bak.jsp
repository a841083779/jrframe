...
// 生成所有的品牌详显页
	...
	String agentSql="select   id,usern,name,full_name,city,address,intro,logo,img1,agent_fac,agent_fac_name,parent_id,lng,lat,is_made,is_qijiandian from  pro_agent_factory where is_show=1 and agent_fac!=''  and  flag = 2   order by id desc";
	List<Map> agentlist = dbHelper.getMapList(agentSql) ;//所有代理商
	out.print("count="+agentlist.size());
		...
		List<Map> agentFaList = new ArrayList() ;
			...
			if(null != topMap){
					...
					root.put("agentFaList",agentFaList) ;//右侧品牌类别信息
						freemarkerUtils.generate("agent_page/agent_shop_introduce.htm",root,realPath+"/agent_bak/"+usern+"/introduce.shtm") ;  // 生成所有的品牌详显页
		        }
			
			
			//首页主推产品
			productSql="select top 8 id,name,catalogname,factoryname,factoryid,file_name,img2 from pro_products where is_show=1 and factoryid="+agent_fac+" order by view_count desc ";
			...
			if(null != productList && productList.size()>0){
					...
					root.put("agentFaList",agentFaList) ;//右侧品牌类别信息
						freemarkerUtils.generate("agent_page/agent_shop_index.htm",root,realPath+"/agent_bak/"+usern+"/index.shtm") ;  // 生成所有的品牌详显页
		        }
			...
			if(null != topMap ){
			        ...
			        root.put("topMap",topMap) ;//基本信息
					root.put("agentFaList",agentFaList) ;//右侧品牌类别信息
					freemarkerUtils.generate("agent_page/agent_message.htm",root,realPath+"/agent_bak/"+usern+"/agent_message.shtm") ;  // 生成所有的品牌详显页
		        }
			...
			if(null != topMap ){
			        ...
			        root.put("topMap",topMap) ;//基本信息
					root.put("agentFaList",agentFaList) ;//右侧品牌类别信息
					freemarkerUtils.generate("agent_page/agent_sales.htm",root,realPath+"/agent_bak/"+usern+"/agent_sales.shtm") ;  // 生成所有的品牌详显页
		        }
			...
			if(null != topMap ){
					...
					root.put("agentFaList",agentFaList) ;//右侧品牌类别信息
					freemarkerUtils.generate("agent_page/agent_products.htm",root,realPath+"/agent_bak/"+usern+"/agent_products.shtm") ;  // 生成所有的品牌详显页
		        }
			  ...
			  //企业动态
				...
				if(null != Newslist && Newslist.size()>0){
					...
					root.put("agentFaList",agentFaList) ;//右侧品牌类别信息
					freemarkerUtils.generate("agent_page/agent_news.htm",root,realPath+"/agent_bak/"+usern+"/agent_news.shtm") ;  // 生成所有的品牌详显页
		        }
	     }
	out.println("ok") ;
%>
