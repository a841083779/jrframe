	...
	try{
		...
		Map root1 = new HashMap();
			//根据大类调取二级类别
				...
				sql = "select num from pro_catalog where is_show=1 "+catalogWhere+" order by id asc";//类别
				...
				sql3 = "select  id from pro_agent_factory where flag=1 and is_show=1 and  id in (select  factoryid from pro_products where  is_show=1 GROUP BY factoryid)  order by upper_index asc";//品牌
				...
				for(Map catalogB:list){
					String catalogw=CommonString.getFormatPara(catalogB.get("num"));//类别号
					sql2= "select  id from pro_agent_factory where flag=1 and is_show=1   and  id in (select  factoryid from pro_products where 1=1 and catalognum like '"+catalogw+"%' and is_show=1 GROUP BY factoryid)  order by upper_index asc";//品牌
