...
String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
	...
	try{
		...
		Map root1 = new HashMap();
				sql = "select num from pro_catalog where  is_show=1 and len(num)<=6  order by id asc";
			    list = dbHelper.getMapList(sql);
				if(list != null && list.size() > 0){
					root.put("list",list);
					freemarkerUtils.generate("koubei_page/koubei_index_brand.htm",root,realPath+"include/koubei_page/koubei_index_brand.jsp");
					resule="true";
				}
	}catch(Exception e){
		e.printStackTrace();
