<%@ page language="java" import="java.io.File,java.util.*,com.jerehnet.util.common.*,org.apache.commons.fileupload.servlet.*,com.jerehnet.util.images.*" pageEncoding="UTF-8"%><%@page import="org.apache.commons.fileupload.FileItemFactory"%><%@page import="org.apache.commons.fileupload.disk.DiskFileItemFactory"%><%@page import="java.io.BufferedInputStream"%><%@page import="org.apache.commons.fileupload.FileItem"%>
<%@page import="com.alibaba.fastjson.JSONObject"%><%
    String method = CommonString.getFormatPara(request.getParameter("method"));
    //String method = Common.getString(request.getParameter("method"));
	String path = request.getContextPath();
	if("single_upload".equals(method)){
		String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path;
		//basePath="http://119.254.0.227:7777";
		...
		while (itr.hasNext()) {
			...
			if (!item.isFormField()) {
				...
				String outStr = saveUrl+newFileName; 
				JSONObject obj = new JSONObject();
				obj.put("fullPath",outStr);
				out.println(obj.toJSONString());
			}
		}
	}
	//远程上传
	if("remote_upload".equals(method)){
		Map paramMap = new HashMap();
		//远程生成的文件夹
		paramMap.put("folder",Env.getInstance().getProperty("web.id"));
		out.print(paramMap);
	}
%>