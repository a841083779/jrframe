				...
				if(null != fourids && fourids.length>0 && null!=orderuuid){
					...
					for(int i=0;i<fourids.length;i++){
						...
						if(null != tempproduct && tempproduct.size()>0){
							...
							//2014,11,11
							insertsql = " insert into pro_product_form_activity(add_date,add_ip,name,contact_address,mobile_phone,uuid,product_name,message,province,city) values('"+today+"','"+ip+"','"+username+"','"+contact_addr+"','"+tel+"','"+Common.securityFilter(CommonString.getFormatPara(tempproduct.get("id")))+"','"+orderuuid[i]+"','"+message+"','"+zd_province+"','"+zd_city+"') " ;
