      ...
      <% 
		...
		List <Map>list = dbHelper.getMapList(sql,connection);
	  	if(list != null && list.size() > 0){
			for(Map m : list){
	  %>
      <li><a target="_blank" href="/<%=shop_url %>/agent_products.jsp?usern=<%=usern%>&catalognum=<%=CommonString.getFormatPara(m.get("catalognum"))%>&factory=<%=agent_fac_arr[i] %>"><%=CommonString.getFormatPara(m.get("catalogname"))%></a></li>
      <%	
			}
		}
	  %>
      ...
      <%
		...
		List <Map>list = dbHelper.getMapList(sql,connection);
	  	...
	  	if(list != null && list.size() > 0){
			for(Map m : list){
	  %>
      <li><a href="http://<%=usern%>.product.21-sun.com/<%=file_jsp %>?usern=<%=usern%>&catalognum=<%=CommonString.getFormatPara(m.get("catalognum"))%>"><%=CommonString.getFormatPara(catalogMap.get(CommonString.getFormatPara(m.get("catalognum"))))%></a></li>
      <%	
			}
		}
	  %>
