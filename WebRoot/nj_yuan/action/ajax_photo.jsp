<%@ page language="java" import="org.apache.commons.httpclient.params.*,org.apache.commons.httpclient.*,org.apache.commons.httpclient.methods.*,com.jerehnet.webservice.*,java.util.regex.*,org.json.*,java.util.*,com.jerehnet.util.common.*,java.sql.*,com.jerehnet.util.dbutil.*" pageEncoding="UTF-8"%><%
	...
	if(!CommonString.getFormatPara(request.getParameter("username")).equals("")){
	          username=CommonString.getFormatPara(request.getParameter("username"));
	}
	...
	try{
		...
		if("new_save_photo".equals(flag)){
			...
			String sql = " insert into pro_photo_group(title,index_title,sort_num,image,is_recommend,is_show,add_date,add_ip,usern,pub_date,uuid,keywords,brand_keywords,zhanhui_num,catalognum,factoryid,add_user)";
			sql += " values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		    ...
		    int keyId = dbHelper.executeUpdate(sql,new Object []{zd_title,zd_type,sort_num,img,1,1,CommonDate.getToday("yyyy-MM-dd HH:mm:ss"),Common.getIp(request),"youke",CommonDate.getToday("yyyy-MM-dd HH:mm:ss"),UUID.randomUUID().toString(),zd_pro_cataname,CommonString.getFormatPara(request.getParameter("zd_factoryname")),zd_zhanhui_num,pro_id,fa_id,username},connection);
