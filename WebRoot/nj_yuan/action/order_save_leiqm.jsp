...
<%
	...
	String urlString = "http://virtual.paipai.com/extinfo/GetMobileProductInfo?mobile="+telephone+"&amount=10000&callname=getPhoneNumInfoExtCallback";
	String responses = "";
	...
	try{
       ...
       if(responses.indexOf("city")!=-1&&responses.indexOf("province")!=-1){
        	 province= responses.substring(responses.indexOf("province:")+9,responses.lastIndexOf("isp")).replace(",", "").replace("'", "").trim();
             city= responses.substring(responses.indexOf("cityname:")+9,responses.lastIndexOf("})")).replace(",", "").replace("'", "").trim();
	...
	try{
		...
		String ids = Common.securityFilter(CommonString.getFormatPara(request.getParameter("del_id")));
			...
			StringBuffer sqlques = new StringBuffer("(");
				...
				Integer keyIds = dbHelper.executeUpdate(sql1.toString(), values.toArray(),connection) ;

               //寻单次数+1
				if(null!=keyIds&&keyIds>0){
