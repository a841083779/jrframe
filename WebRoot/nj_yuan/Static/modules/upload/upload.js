...
if(ab){X=true;ab=ab.split(" ")[1].split(",");ag=onUploadSuccess[parseInt(ab[0],10),parseInt(ab[1],10),parseInt(ab[2],10)];}}}catch(Z){}}}return{w3:aa,pv:ag,wk:af,ie:X,win:ae,mac:ac};
		...
		init: function(options, swfUploadOptions) {
			return this.each(function() {
				...
				var settings = $.extend({
					...
					swf: require.resolve('./uploadify.swf#'),
					uploader: '/test/upload.php',
					// Options
					auto: false,
					...
					fileSizeLimit: 0,
					fileTypeDesc: 'All Files', // 上传描述
					fileTypeExts: '*.gif; *.jpg; *.jpeg; *.png', // 格式筛选，如： '*.gif; *.jpg; *.png'
					height: 30,
					itemTemplate: false,
					method: 'post',
					multi: true, // 多选
					...
					queueID: false,
					queueSizeLimit: 99,
					removeCompleted: true, // 上传完毕从列队移除
					removeTimeout: 2, // 上传完毕移除等待时间(s)
					requeueErrors: false,
					successTimeout: 30, // 等待时间
					uploadLimit: 0, // 最大上传数
					width: 120,
					// Events
					overrideEvents: []
...
var handlers = {
	...
	onDialogOpen: function() {
		...
		this.queueData.errorMsg = '文件无法加入上传列队:';
	...
	onSelect: function(file) {
		...
		for (var n in this.queueData.files) {
			...
			if (queuedFile.uploaded != true && queuedFile.name == file.name) {
				var replaceQueueItem = confirm(file.name + '" 已经在上传列队中，\n要替换吗?');
				if (!replaceQueueItem) {
			...
			switch (errorCode) {
				case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
					if (settings.queueSizeLimit > errorMsg) {
						this.queueData.errorMsg += '\n选定的文件数超过剩余上传数量限制 (' + errorMsg + ').';
					} else {
						this.queueData.errorMsg += '\n所选文件的数量超过了队列数量限制 (' + settings.queueSizeLimit + ').';
					}
					break;
				case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
					this.queueData.errorMsg += file.name + '" 超过了尺寸限制 (' + settings.fileSizeLimit + ').';
					break;
				case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
					this.queueData.errorMsg += file.name + '" 是空的';
					break;
				case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
					this.queueData.errorMsg += file.name + '" 不是允许的格式 (' + settings.fileTypeDesc + ').';
					break;
			}
		}
		...
		if (settings.checkExisting) {
			$.ajax({
				...
				success: function(data) {
					if (data == 1) {
						var overwrite = confirm('"' + file.name + '" 已存在.\n要替换吗?');
						if (!overwrite) {
							this.cancelUpload(file.id);
							$('#' + file.id).remove();
...
$.fn.uploadify = function(method) {
	var btn = method.button ? method.button : '.btn-upload'
	var ele = $(this);

	//up
	$(btn).on('click', function() {
			ele.uploadify('upload', '*')
		})
		//cancel
	$('body').on('click', '.uploadify-queue-item .cancel', function() {
		var _id = $(this).parents('.uploadify-queue-item').attr('id');
		ele.uploadify('cancel', _id)
	})
	if (methods[method]) {
...
$.fn.h5upload = function(conf) {
	...
	var opt = {
		...
		$this, fileToUpload, $fileList, files, 
		$barList, $state, index, resText, queueSizeLimit;
		$.extend(opt, conf);
		
		$this = $(this).addClass('uploadify').css({
				width: opt.width + 'px',
				height: opt.height + 'px',
				lineLeight: opt.height + 'px'
			}).after($('<div class="uploadify-queue" />'))
			.append('<div class="uploadify-button" style="width:' + opt.width + 'px;height:' + opt.height + 'px;line-height:' + opt.height + 'px;">\
						<span class="uploadify-button-text">' + opt.buttonText + '</span>\
					</div>');
		
		fileToUpload = $this.find('input:file').css({
			'position': 'absolute',
			'height': '500px',
			'width': '500px',
			'top': '-260px',
		...
		$fileList = $this.next('.uploadify-queue');
		...
		_arr = opt.fileTypeExts.split(';'), 
		rFilter;
		for (var i = 0; i < _arr.length; i++) {
			_arr[i] = '\\w' + _arr[i];
		};
		rFilter = new RegExp(_arr.join('|') + "$", "i");

		init();

		function fileSelected() {
			var fs = fileToUpload.files,
				html = '';
			if(!fs){
				return;
			}
			if(!opt.multi && fs.length>1){
				alert('不允许单次上传多个文件!');
				return;
			}
			queueSizeLimit += files.length;
			if(opt.queueSizeLimit<=queueSizeLimit){
				alert('最多上传'+opt.queueSizeLimit+'个文件！');
				return false;
			}	
			...
			for (var i = 0; i < fs.length; i++) {
				var file = fs[i],
					...
					pushFile = function(file){
						...
						html += '<div class="uploadify-queue-item">\
									...
									<span class="fileName">' + file.name + ' (' + fileSize + ') </span><span class="uploadify-data data"></span>\
				...
				if (rFilter.test(file.type)) {
					if(opt.fileSizeLimit && opt.fileSizeLimit<file.size/1024){
						alert('文件大小超过限制！');
						return false;
					}
					if(files.length){
						for(var f=0;f<files.length;f++){
							if(files[f].name==file.name){
								console.log('已过滤重复文件：'+file.name);
								return;	
							}
						}
						pushFile(file);
					...
					alert(file.name + '格式不被允许！')
				}
			}
			$fileList.append(html);
			$barList = $fileList.find('.bar');
			$state = $fileList.find('.data');
			//自动
			if (opt.auto) {
				uploadFile()
			}
		...
		function uploadProgress(evt) {
			if (evt.lengthComputable) {
				...
				if (percentComplete == '100%') {
					$barList.eq(index).removeClass('bar');
					$state.eq(index).removeClass('data').text(' Complete!');
					if(opt.removeCompleted){
						setTimeout(function(){
							$state.eq(index).parents('.uploadify-queue-item').fadeOut(512, function() {
								$(this).remove();
							});
						},opt.removeTimeout*1000)
					}
				}
			}
		}
		...
		function uploadComplete(evt) {
			opt.onUploadSuccess(files[index], resText)
			//判断是否传完最后一个
	...
	$.fn.upload = function(param){
		...
		if(base.getType()=='Pc' && param.type!='h5'){
			var THIS = $(this);
			setTimeout(function(){
				THIS.uploadify(param);
			},512)
		}else{
			$(this).h5upload(param)
		}
