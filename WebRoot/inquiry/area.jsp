<%@ page language="java" import="java.util.*" pageEncoding="utf-8"%><%@page import="com.jerehnet.util.common.CommonString"%><%@page import="com.jerehnet.util.dbutil.DBHelper"%><%@page import="com.jerehnet.util.common.*"%><%@page import="java.text.SimpleDateFormat"%><%@page import="com.jerehnet.util.dbutil.PageBean"%><%@ taglib tagdir="/WEB-INF/tags" prefix="tags" %><%@ taglib uri="/WEB-INF/classes/oscache.tld" prefix="cache"%><%
	...
	Map oneOrder = dbHelper.getMap(" select id,view_count,factoryname,cataname,name,mobile_phone,contact_tel,contact_address,product_name,add_date,message,product_id,factoryid,catanum from pro_product_form where id=?",new Object[]{orderid}) ;
	if(oneOrder == null){
		response.setStatus(301);
		response.setHeader("Location", "/inquiry/"); 
		return;	
...
<div class="contain980 mb10">
  ...
  <a href="/inquiry/">采购询价</a> &gt;&gt;<a href="/inquiry/<%=(province.equals("")?"0":province)%>_<%=(city.equals("")?"0":city)%>_0_0_0.shtm"><%=search_province%><%=search_city%></a>&gt;&gt;
  ...
  <div class="w757 r">
        ...
        <ul>
          <li> <a href="/proDetail/<%=CommonString.getFormatPara(oneProduct.get("file_name")) %>" target="_blank"><img src="/uploadfiles/<%=CommonString.getFormatPara(oneProduct.get("img2")).toLowerCase() %>" onerror="this.src='/uploadfiles/no_small.gif'" width="130" height="90" class="plimg" alt="<%=CommonString.getFormatPara(oneProduct==null?"":oneProduct.get("factoryname")) %><%=CommonString.getFormatPara(oneProduct.get("name")) %><%=CommonString.getFormatPara(oneProduct.get("catalogname")) %>" title="<%=CommonString.getFormatPara(oneProduct==null?"":oneProduct.get("factoryname")) %><%=CommonString.getFormatPara(oneProduct.get("name")) %><%=CommonString.getFormatPara(oneProduct.get("catalogname")) %>"/></a>
            <div class="w618 r">
              ...
              <span class="info"><a href="/inquiry/index.jsp?proId=<%=CommonString.getFormatPara(oneOrder.get("product_id")) %>" target="_blank">订单</a> <em>|</em> <a href="http://photo.21-sun.com/list.jsp?factory=<%=CommonString.getFormatPara(oneOrder.get("factoryid")) %>&catalog=<%=CommonString.getFormatPara(oneOrder.get("catanum")).length()>=6?CommonString.getFormatPara(oneOrder.get("catanum")).substring(0,6):CommonString.getFormatPara(oneOrder.get("catanum")) %>" target="_blank">图片</a> <a href="javascript:;" onclick="showinquery('<%=CommonString.getFormatPara(oneProduct.get("id")) %>','')" class="plbtn">询价</a></span>
    ...
    <div class="w755 border03 l mb10 list08">
      <div class="title08 listTop">
        ...
        <div class="r" style="padding-top:0px;"> <a title="更多<%=search_province%><%=search_city%><%=CommonString.getFormatPara(oneOrder.get("factoryname")) %><%=CommonString.getFormatPara(oneOrder.get("product_name")) %><%=CommonString.getFormatPara(oneOrder.get("cataname")) %>询价单" href="/inquiry/?" target="_blank">更多</a>  </div>
      </div>
      <ul class="ti">
        <li class="i1">编号</li>
        <li class="i2">询价产品</li>
    ...
    <div class="w755 border03 l mb10 list08">
      <div class="title08 listTop">
        ...
        <div class="r"  style="padding-top:0px;"> <a  title="更多<%=search_province%><%=search_city%><%=CommonString.getFormatPara(oneOrder.get("factoryname")) %><%=CommonString.getFormatPara(oneOrder.get("product_name")) %><%=CommonString.getFormatPara(oneOrder.get("cataname")) %>评论" href="http://koubei.21-sun.com/" target="_blank">更多</a> </div>
      </div>
  ...
  <!--翻页--> 
    ...
    <div class="w755 border03 l mb10 list08">
      <div class="title08 listTop">
        ...
        <div class="r" style="padding-top:0px;"> <a title="更多<%=search_province%><%=search_city%><%=CommonString.getFormatPara(oneOrder.get("cataname")) %>询价单" href="/inquiry/" target="_blank">更多</a> </div>
      </div>
      <ul class="ti">
        <li class="i1">编号</li>
        <li class="i2">询价产品</li>
