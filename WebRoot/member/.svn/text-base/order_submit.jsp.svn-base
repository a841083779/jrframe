...
<%
	...
	Map proMap = dbHelper.getMap("select id,factoryid,factoryname,catalognum,catalogname,name,img2,file_name from pro_products where id="+pro_id);
    if(proMap==null){
		  Common.do302(response,"http://product.21-sun.com/tuan/");
		  return;
	}
...
<div class="wrapper m_contain mb20 fix" style="border:0; background:none">
  <div class="m_part">
    ...
    <div class="cart_products_v2">
      <table width="100%" id="cart_products">
        <tbody>
          ...
          <tr class="cart_item">
            <td align="center"><div style="width:150px;"> <a target="_blank" href="<%=CommonString.getFormatPara(proMap.get("file_name")) %>"> <img style="margin: 5px 5px;" title="<%=CommonString.getFormatPara(proMap.get("factoryname")) %><%=CommonString.getFormatPara(proMap.get("name")) %><%=CommonString.getFormatPara(proMap.get("catalogname")) %>" alt="<%=CommonString.getFormatPara(proMap.get("factoryname")) %><%=CommonString.getFormatPara(proMap.get("name")) %><%=CommonString.getFormatPara(proMap.get("catalogname")) %>" 	src="http://product.21-sun.com/uploadfiles/<%=CommonString.getFormatPara(proMap.get("img2"))%>" onerror="showImgDelay(this,'http://product.21-sun.com/uploadfiles/no_big.gif',2);" width="70" height="70" class="l">
                <p style="text-align:left"><%=CommonString.getFormatPara(proMap.get("factoryname")) %><%=CommonString.getFormatPara(proMap.get("name")) %><%=CommonString.getFormatPara(proMap.get("catalogname")) %></p>
                </a> </div></td>
            <td  style="width:100px;" align="center"> <%=CommonString.getFormatPara(memberInfo.get("username"))%> </td>
            <td align="center"> <%=CommonString.getFormatPara(memberInfo.get("telephone"))%></td>
            <td align="center" > <%=CommonString.getFormatPara(proMap.get("factoryname")) %> </td>
            <td align="center" > <span id="the_area"></span> </td>
            <td align="center" > <%=todayTime%> </td>
            <td align="center" class="dingj"> 2000.00 </td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="tj fix">
      <div class="l tj1"> <span class="ftx-08">留言：</span>
        <textarea id="the_message" class="text inputholder">您好，我对<%=CommonString.getFormatPara(proMap.get("factoryname")) %><%=CommonString.getFormatPara(proMap.get("name")) %><%=CommonString.getFormatPara(proMap.get("catalogname")) %>感兴趣，想要咨询，请您给我回复</textarea>
      </div>
  ...
  <span class="n1"><strong>实付款：</strong><b>￥</b><b>2000.00</b></span>
  <span><strong>联系人：</strong><%=CommonString.getFormatPara(memberInfo.get("username"))%></span>
  <span><strong>电&nbsp;&nbsp;&nbsp;&nbsp;话：</strong><%=CommonString.getFormatPara(memberInfo.get("telephone"))%></span>
  <span></span>
  </div>
  </div>
  <a href="javascript:void(0)" onclick="submitOrder();" class="s_btn">提交订单</a>
  </div>  
  </div>
</div>
<!--foot-->
...
<script>
        function submitOrder(){
            ...
            */
			...
			var place=document.getElementById("the_area").innerHTML;
			var the_message = document.getElementById('the_message').value;
            $.ajax({
                type:"post",
                url:"/action/ajax.jsp",
                data:{flag:"createOrder",userid:'<%=CommonString.getFormatPara(memberInfo.get("id"))%>',add_user:'<%=CommonString.getFormatPara(memberInfo.get("username"))%>',add_user_phone:'<%=CommonString.getFormatPara(memberInfo.get("telephone"))%>',product_id:'<%=CommonString.getFormatPara(proMap.get("id"))%>',product_factoryname:'<%=CommonString.getFormatPara(proMap.get("factoryname"))%>',product_catalogname:'<%=CommonString.getFormatPara(proMap.get("catalogname"))%>',product_factoryid:'<%=CommonString.getFormatPara(proMap.get("factoryid"))%>',product_catalognum:'<%=CommonString.getFormatPara(proMap.get("catalognum"))%>',product_img:'<%=CommonString.getFormatPara(proMap.get("img2"))%>',contents:the_message,area:place,product_name:'<%=CommonString.getFormatPara(proMap.get("name"))%>',file_name:'<%=CommonString.getFormatPara(proMap.get("file_name"))%>'},
                success:function(result){
                    var order_num=$.trim(result);
                    if(order_num != ""&&order_num!="fail"){
                        window.location.href="/member/order_pay.jsp?order_num=" + order_num;
                    }else{
