<%@ page language="java" import="java.util.*" pageEncoding="utf-8"%><%@page import="com.jerehnet.util.common.CommonString,java.sql.Connection,com.jerehnet.util.dbutil.*"%><%@page import="com.jerehnet.util.dbutil.PageBean"%><%@taglib tagdir="/WEB-INF/tags" prefix="tags" %><%@ taglib uri="/WEB-INF/classes/oscache.tld" prefix="cache"%><%
	...
	try{
		connection = dbHelper.getConnection();
				//国内品牌
		List<Map> list = dbHelper.getMapList("select name,id,is_inner from pro_agent_factory where id in(select distinct(pro_products.factoryid) from pro_products  where is_show=1 and catalognum_new like '%"+catalog+"%') and is_inner=1 order by view_count desc",connection) ;
		
			//国外品牌
		List<Map> list2 = dbHelper.getMapList("select name,id,is_inner from pro_agent_factory where id in(select distinct(pro_products.factoryid) from pro_products  where is_show=1 and catalognum_new like '%"+catalog+"%')and is_inner=0 order by view_count desc",connection) ;
        ...
        <div class="ConditionTop">
          <div class="l">
            ...
            <%
									...
									%>
            <span><em><%="".equals(factory) ? "" : CommonString.getFormatPara(brandMap.get(factory))%></em> <a href="javascript:void(0)" title="取消" class="delete"
          ...
          <h1 class="r" style="font-size:14px;"><%="".equals(wordsSelect) ? "工程机械" :""%><%=keyword%><%=showtonandmeter%><%="".equals(factory) ? "" : CommonString.getFormatPara(brandMap.get(factoryid))%><%="".equals(catalog) ? "" : CommonString.getFormatPara(catalogMap.get(catalog))%>图片大全</h1>
        </div>
        <ul class="ConditionBottom">
          ...
          <li class="cblist disp">
            ...
            <dd class="sel02_1">
              ...
              <a href="/list.jsp?<%=catalog.equals("")?"":"catalog="+catalog%><%=offset.equals("")?"":"&offset="+offset%>">不限</a> 
          <%
		 ...
		 %>
             <a <%if(id.equals(factory)){%> class='select' <%} %> href="list.jsp?<%=factory.equals("")?"":"factory="+id%><%=catalog.equals("")?"":"&catalog="+catalog%><%=offset.equals("")?"":"&offset="+offset%>"><%=factoryname%></a> 
		   ...
		   <li class="cblist">
            ...
            <dd>
              ...
              <a href="/list.jsp?<%=catalog.equals("")?"":"catalog="+catalog%><%=offset.equals("")?"":"&offset="+offset%>"  >不限</a> 
              <a href="/list.jsp?<%=factory.equals("")?"":"factory="+factory%><%=catalog.equals("")?"":"&catalog="+catalog%><%=offset.equals("")?"":"&offset="+offset%>"   class='select' > <%=catalog%></a> 
            </dd>
          </li>
          ...
          <li class="cblist disb">
            ...
            <dd class="sel02_3">
              ...
              <a href="/list.jsp?<%=catalog.equals("")?"":"catalog="+catalog%><%=offset.equals("")?"":"&offset="+offset%>"  <%if("".equals(factory)){%> class='select' <%}%>>不限</a> 
          <%
