...
jQuery(document).ready(function(){
    ...
    jQuery(".breadCrumbs span#showotherbrand").click(function(event){
    	event.stopPropagation() ;	
        jQuery(this).toggleClass("bchover");
		showotherbrand(<%= "".equals(catalog)?"''":catalog%>,<%="".equals(factoryid)?"''":factoryid %>,null);
    }); 
    
	
	jQuery(".breadCrumbs span#showothercatalog").click(function(){
        jQuery(this).toggleClass("bchover");
		showothercatalog(<%= "".equals(catalog)?"''":catalog%>,<%="".equals(factoryid)?"''":factoryid %>,null);
    }); 
...
<div class="contain980 mb10">
  ...
  <span id="showotherbrand"><a href="/photo/list.jsp?factoryid=<%=factoryid%>"><%=factoryname%></a>
    <ul class="bclist" id="showotherbrandhtml">
    </ul>
  </span> 
  &gt;&gt; 
  <span id="showothercatalog"><a href="/photo/list.jsp?factoryid=<%=factoryid%>&catalogid=<%=(catalog.length()>6?catalog.substring(0,6):catalog)%>"><%=factoryname+parentcatalogname %>图片</a>
    <ul class="bclist" id="showothercataloghtml">
    </ul>
  </span> 
  &gt;&gt; 
...
<div class="contain980">
    ...
    <div class="photoShow">
      <div class="photoLeft">
        ...
        <div class="checkdetail"><b><a href="/proDetail/<%=CommonString.getFormatPara(main_pics.get("file_name")) %>" target="_blank">查看详细参数&gt;&gt;</a></b> <a href="javascript:;" class="plbtn"  onclick="showinquery(<%=CommonString.getFormatPara(String.valueOf(main_pics.get("id"))) %>)">询价</a></div>
      </div>
...
<script type="text/javascript">
	function showothercatalog(catalog,factoryid,event){
			jQuery("#showothercataloghtml").html("") ;
		    jQuery.post("/action/ajax.jsp",{flag:"showrelatecatalog",catalog:catalog,factoryid:factoryid},function(result){
			jQuery("#showothercataloghtml").append(jQuery.trim(result)) ;
	  });
	}
	function showotherbrand(catalog,factoryid,event){
			jQuery("#showotherbrandhtml").html("") ;
		    jQuery.post("/action/ajax.jsp",{flag:"showrelatebrand",catalog:catalog,factoryid:factoryid},function(result){
			jQuery("#showotherbrandhtml").html(jQuery.trim(result)) ;
	  });
	}
