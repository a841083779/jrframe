...
Kibo.WILDCARDS = {
	...
	letter : [65, 66, 67, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78, 79, 80,
		...
		if (Object.prototype.hasOwnProperty.call(registeredKeys, registeredKey)) {
			...
			if (keyCombination.length && keyCombination[0] !== 'any') {
				...
				for (j = 0; j < keyCombination.length; j++)
					match = match
							&& (Kibo.isModifier(keyCombination[j]) ? this
									.lastKey(keyCombination[j]) : (this
									.lastKey() === keyCombination[j]));
				if (match)
...
Kibo.prototype.handler = function(upOrDown) {
	...
	return function(e) {
		...
		for (i = 0; i < Kibo.WILDCARD_TYPES.length; i++)
			if (Kibo.arrayIncludes(Kibo.WILDCARDS[Kibo.WILDCARD_TYPES[i]],
					that.lastKeyCode))
				for (j = 0; j < registeredKeys['any ' + Kibo.WILDCARD_TYPES[i]].length; j++)
					if ((registeredKeys['any ' + Kibo.WILDCARD_TYPES[i]][j](e) === false)
							&& e.preventDefault)
						e.preventDefault();

		for (i = 0; i < matchingKeys.length; i++)
			for (j = 0; j < registeredKeys[matchingKeys[i]].length; j++)
				if ((registeredKeys[matchingKeys[i]][j](e) === false)
						&& e.preventDefault)
					e.preventDefault();
	};
};
...
Kibo.prototype.registerKeys = function(upOrDown, newKeys, func) {
	...
	for (i = 0; i < newKeys.length; i++) {
		Kibo.assert(Kibo.isString(newKeys[i]), {
					name : 'Type error',
...
Kibo.prototype.delegate = function(action, keys, func) {
	return func !== null ? this.registerKeys(action, keys, func) : this
			.unregisterKeys(action, keys, func);
};
...
Kibo.prototype.lastKey = function(modifier) {
	...
	Kibo.assert(Kibo.arrayIncludes(Kibo.MODIFIERS, modifier), {
				name : 'Modifier error',
				message : 'invalid modifier ' + modifier
						+ ' (valid modifiers are: ' + Kibo.MODIFIERS.join(', ')
						+ ').'
			});
