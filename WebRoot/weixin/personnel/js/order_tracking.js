$(function(){
	...
	$.ajax({
		...
		success: function(json){
			...
			console.log(parseInt(json.provinceNum));
	...
	function pullupRefresh() {
		...
		data : {
			...
			uid : $('#uid').val(),//476 
			orderState: 1 ,
			trackState: $('#orderState').val(),//$('#status_items').find('input:checked').data('value'),
			province: $('#province').val(),//$('#province_items').find('input:checked').data('value'),
			pageNum : count ,
			pageSize : 10
		},
		...
		success: function(json){
			var html='',id='',title='',receiveMan='',orderUser='',createDate='',isTagged='',recordDate='',orderStatename='',contactTel='',contactAddress='';
			...
			$.each(order, function(idx) {
				...
				contactAddress=order[idx]["contactAddress"];
				...
				html += '<div class="info_btns r"><div id="copy_'+id+'"  style="display:none;">'+contactAddress+orderUser+contactTel+'询'+title+'</div><a href="javascript:void(0);" data-id="copy_'+id+'"  class="copy_btn dib">复制订单</a><a href="tel:'+contactTel+'" class="tel_btn dib">拨打电话</a></div>';
				html += '</div></li>';
	...
	function Initialization() {
		...
		success: function(json){
			...
			$.each(order, function(idx) {
				...
				html += '<p class="info_title"><span class="star_status dib" data-id="'+id+'"><i class="ion">&#xe6cc;</i></span><a href="order_detail_edit.jsp?id='+id+'&source='+source+'&ywId='+$('#uid').val()+'&province='+$('#province_items').find('input:checked').data('value')+'&orderState='+$('#status_items').find('input:checked').data('value')+'"><span>'+title+'</span></a></p>';
				html += '<a href="order_detail_edit.jsp?id='+id+'&source='+source+'&ywId='+$('#uid').val()+'&province='+$('#province_items').find('input:checked').data('value')+'&orderState='+$('#status_items').find('input:checked').data('value')+'"><p class="per_info"><span class="name">'+orderUser+'</span><span class="date">'+createDate+'</span></p></a></div>';
				html += '<div class="info_btns r"><div id="copy_'+id+'" class="copyTitle" style="display:none;">'+contactAddress+orderUser+contactTel+'询'+title+'</div><a href="javascript:void(0);"  data-id="copy_'+id+'"  class="copy_btn dib">复制订单</a><a href="tel:'+contactTel+'" class="tel_btn dib">拨打电话</a></div>';
				html += '</div></li>';
	...
	//复制订单
	/**$('body').on('tap','.copy_btn',function(){
		var dataId=$(this).attr("data-id");
		var copyTitle=$("#"+dataId).html();
        var oInput = document.createElement('input');
        oInput.value = copyTitle;
