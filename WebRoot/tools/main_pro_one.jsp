...
<%
	...
	try{
		...
		Map root1 = new HashMap();
			...
			list = dbHelper.getMapList(" select  id,catalognum,param  from pro_products  where id=32479 and id not in (select pro_id  from pro_main_param ) and is_show=0 and param is not null and DATALENGTH(param)>20   ");
			for(Map model:list){
			...
			//主要参数
			List <Map> categoryKeyList= dbHelper.getMapList(" select  id,name from pro_catalog_param  where parent_no='"+catalognum+"'  ");
