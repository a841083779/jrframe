...
String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
	...
	try{
		...
		roots.put("title","详细页面生成——根据订单") ;
		roots.put("content","详细页面生成——根据订单");
		String srt=Common.doPost("http://service.21-sun.com/http/utils/sendmail.jsp", roots) ;
	   } catch (Exception e) {}
		
			//定时更新，有新订单详细页
			list_form = dbHelper.getMapList(" select   product_id  from pro_product_form  where CONVERT(varchar(100),add_date, 23)=CONVERT(varchar(100),GETDATE() , 23)  and product_id!='' group by product_id  ");
			for(Map order_from:list_form){
