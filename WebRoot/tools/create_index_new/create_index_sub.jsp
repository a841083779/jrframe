...
String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
	...
	try{
		...
		else if(cuuid.equals("4715A7A2-4266-475E-A200-5224F12D7588")){////挖掘机4F 迷你挖
		
			String sql = " select top 6 title,url,img,order_no,sub_title,summary from pro_index_data  where channel_uuid = '"+cuuid+"' and is_pub = 1   order by order_no asc ";
			list = dbHelper.getMapList(sql);
			if(list != null && list.size() > 0){
				root.put("list",list);
				freemarkerUtils.generate("create_index/index_pro_list_f.htm",root,realPath+"create_index/index_wjj_pro_f4.htm");
				resule="true";
			}
	
		}
		else if(cuuid.equals("530A0F3F-2B72-424F-AE0A-0FD0E3B6C870")){////挖掘机5F 中小挖
		
			String sql = " select top 6 title,url,img,order_no,sub_title,summary from pro_index_data  where channel_uuid = '"+cuuid+"' and is_pub = 1   order by order_no asc ";
			list = dbHelper.getMapList(sql);
			if(list != null && list.size() > 0){
				root.put("list",list);
				freemarkerUtils.generate("create_index/index_pro_list_f.htm",root,realPath+"create_index/index_wjj_pro_f5.htm");
				resule="true";
			}
