	...
	if (!"".equals(uuids)) {
			...
			for (int i = 0; i < uuid.length; i++) {
				...
				if (null != one_proform) {
			        ...
			        if(catanum.indexOf("101001001")>=0){
						...
						if(auto==1){
							//Common.doPostHttpAsync("http://product.21-sun.com/tools/order_interface/cat_admin_auto_normal.jsp?uuid=" + tempUid, pushProMap);
						}
						}
						}
			       }
					...
					if ("184".equals(factoryid)&&1==2) {
						...
						if (!"".equals(emails) && emails.indexOf(",") != -1 && emails.indexOf("@") != -1) {
							...
							for (int j = 0; email.length > 0 && j < email.length; j++) {
								if (email[j].length() > 0 && email[j].indexOf("@") != -1&&!email_id.equals("1113")&&!email_id.equals("1112") ) {
		...
		/**	 else if("749".equals(factoryid)){  // 洛阳东方红 发送邮件
				...
				if (!"".equals(emails) && emails.indexOf(",") != -1 && emails.indexOf("@") != -1&&!email_id.equals("1113")&&!email_id.equals("1112") ) {
					...
					if(!email_id.equals("1113")&&!email_id.equals("1112")){
					WEBEmail.sendMailByUrl(emails, "", null, CommonString.getFormatPara(one_proform.get("factoryname"))
							+ CommonString.getFormatPara(one_proform.get("product_name")) + CommonString.getFormatPara(one_proform.get("catalogname"))
