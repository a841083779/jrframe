...
<div class="txd_xunjia mt15" id="prodetailOrder">
  <div class="txj_main fix" style="height:500px;">
    <div class="_form">
      <form action="/activity/hctm/order_save_new_s.jsp" method="post" name="theform" id="theform" onSubmit="return submitYN(this);">
...
<script>
   ...
   jQuery("#zd_message").val("您好，我对<%=CommonString.getFormatPara(one_product.get("factoryname"))%><%=frombrand.equals("yes")?"产品":CommonString.getFormatPara(one_product.get("name"))+CommonString.getFormatPara(one_product.get("catalogname")) %>感兴趣，想要咨询，请您给我回复，谢谢。");
	...
	if (window.jereiReceiver) { 
    var _jrTracker= jereiReceiver.getTracker("GWD-100002"); 
	_jrTracker.trackFmSubmit("弹出框询价单","<%=productid%>"); 
...
jQuery("#zds_ifgroup").bind("click",function(){
	...
	if (window.jereiReceiver) { 
    var _jrTracker= jereiReceiver.getTracker("GWD-100002"); 
	_jrTracker.trackFmStart("弹出框询价单","<%=productid%>"); 
    }
	//大数据相关事件
	$('#zd_name').focus(function(e) {
		if (window.jereiReceiver) { 
        var _jrTracker= jereiReceiver.getTracker("GWD-100002"); 
	    _jrTracker.trackFmTry("弹出框询价单","<%=productid%>","姓名") ;
        }
	});
	$('#zd_name').blur(function(e) {
		if (window.jereiReceiver) { 
        var _jrTracker= jereiReceiver.getTracker("GWD-100002"); 
		 _jrTracker.trackFmFill("弹出框询价单","<%=productid%>","姓名",$("#zd_name").val()) ;
        }
	});
	$('#zd_mobile_phone').focus(function(e) {
		if (window.jereiReceiver) { 
        var _jrTracker= jereiReceiver.getTracker("GWD-100002"); 
	    _jrTracker.trackFmTry("弹出框询价单","<%=productid%>","电话") ;
        }
	});
	$('#zd_mobile_phone').blur(function(e) {
		if (window.jereiReceiver) { 
        var _jrTracker= jereiReceiver.getTracker("GWD-100002"); 
		 _jrTracker.trackFmFill("弹出框询价单","<%=productid%>","电话",$("#zd_mobile_phone").val()) ;
        }
	});
	...
	function buyTime(obj){
		...
		if (window.jereiReceiver) { 
        var _jrTracker= jereiReceiver.getTracker("GWD-100002"); 
		 _jrTracker.trackFmFill("弹出框询价单","<%=productid%>","购买时间",buy_time) ;
        }	
    }
    function buyType(obj){
		if (window.jereiReceiver) { 
        var _jrTracker= jereiReceiver.getTracker("GWD-100002"); 
		 _jrTracker.trackFmFill("弹出框询价单","<%=productid%>","购买方式",obj) ;
        }
    }
