	...
	try{
					...
					if(cha<60000){
						...
						if(code.equals(hthsy_code)){
							...
							if(rs.indexOf("exist")!=-1){
								...
								String smsText = "“湖南中旺，约惠三一”大型挖掘机订购会，验证码"+preferential_code+"，线上注册礼上礼，当场兑现，0535-6727793";
								param = new HashMap();
						        param.put("phone",CommonString.getFormatPara(request.getParameter("phone")));
		...
		if("getCode".equals(flag)){
			...
			if(getBool){
		        ...
		        String smsTxt = "您好，本次“湖南中旺，约惠三一”活动的短信口令为："+temp+"。【中国工程机械商贸网】";
		        Map param = new HashMap();
		        param.put("phone",CommonString.getFormatPara(request.getParameter("phone")));
		...
		if("renwei".equals(flag)){
			...
			Integer regCounts = (Integer)dbHelper.getOne(" select count(*) from activities_reg where type_id = 3 and phone = ? ", new Object [] {
				phone
			},connection);
			//判断电话是否重复
			if(regCounts>0){
				...
				dbHelper.execute(sql,new Object [] {
						...
						3,Common.getIp(request),preferential_code,CommonString.getFormatPara(request.getParameter("city"))
				},connection);
				String smsText = "“湖南中旺，约惠三一”大型挖掘机订购会，验证码"+preferential_code+"，线上注册礼上礼，当场兑现，0535-6727793";
				Map param = new HashMap();
		        param.put("phone",CommonString.getFormatPara(request.getParameter("phone")));
