...
<%
	...
	//行业图片
	String  hangye_sql = "select top 7 id,title,add_user,image,vote_count   from  pro_photo_group  where sort_num like '%107001%' and is_show=1 order by add_date desc" ;
	List<Map> hangyeList = null ;
	hangyeList = dbHelper.getMapList(hangye_sql) ;
	//施工图片
	String shigong_sql = "select top 7 id,title,add_user,image,vote_count  from  pro_photo_group  where sort_num like '%107003%' and is_show=1 order by  add_date desc" ;
	List<Map> shigongList = null ;
	shigongList = dbHelper.getMapList(shigong_sql) ;
	//维修图片
	String weixiu_sql = "select top 7 id,title,add_user,image,vote_count  from  pro_photo_group  where sort_num like '%107004%' and is_show=1 order by  add_date desc" ;
	List<Map> weixiuList = null ;
	weixiuList = dbHelper.getMapList(weixiu_sql) ;
	//展会图片
	String zhanhui_sql = "select top 7 id,title,add_user,image,vote_count  from  pro_photo_group  where sort_num like '%107005%' and is_show=1 order by  add_date desc" ;
	List<Map> zhanhuiList = null ;
	zhanhuiList = dbHelper.getMapList(zhanhui_sql) ;
	//趣味图片
	String quwei_sql = "select top 7 id,title,add_user,image,vote_count   from  pro_photo_group  where sort_num like '%107006%' and is_show=1 order by  add_date desc" ;
	List<Map> quweiList = null ;
	quweiList = dbHelper.getMapList(quwei_sql) ;
	//产品图片
	String pro_sql = "select top 7 id,title,add_user,image,vote_count   from  pro_photo_group  where sort_num like '%107002%' and is_show=1 order by  add_date desc" ;
...
<div class="kb_jxsz" id="cc">
	<div class="wrap1000 fix">
        ...
        <div class="jxsz_wznr">
        1.每周产生“活跃奖”1名，由每周上传合格图片最多者获得；<br/>
...
<div class="kb_qbzp" id="ee">
	<div class="wrap1000 fix">
				...
				%>
                	<li>
                        ...
                        <div class="kbtp_sum fix">
                        	<span class="tpuser_name"><%=CommonString.getFormatPara(m.get("add_user"))%>&nbsp;&nbsp;票数(<%=CommonString.getFormatInt(m.get("vote_count"))%>)</span>
                            <a href="javascript:void(0)" class="tp_btn"  onclick="vote('<%=CommonString.getFormatPara(m.get("id"))%>');">我要投票</a>
                        </div>
                    </li>
               <%}}%>    
                    <li class="last">
		...
		<a <%if(username.equals("")){%> href="javascript:void(0)"  onclick="login();"<%}else{%> href="/member/photo.jsp" target="_blank"<%}%>><img src="images/up_ck.gif"/></a>
...
<select class="upimg_sele" id="catalognum" name="catalognum">
  ...
  <option value="105004">潜孔钻机</option>
                ...
                <tr>
                    ...
                    <td>
                    	<img style="display:none" src="http://product.21-sun.com/activity/koubei/images/1_22.jpg" onclick="login();" width="77" height="30">
						<span id="addImgs"></span>(可选择多张图片上传)
                    </td>
                </tr>
				 <tr>
                ...
                <tr>
                    ...
                    <td>
                    	<span style="font-size:14px; color:#ff6600;">单张图片不超过6MB，图片尺寸 高520px × 宽440px。</span>
                    </td>
                </tr>
                <tr>
                	<td class="um_title" align="right">&nbsp;</td>
