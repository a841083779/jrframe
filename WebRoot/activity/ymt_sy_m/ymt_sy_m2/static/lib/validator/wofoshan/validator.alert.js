Validator = {
		...
		if (this.ErrorMessage.length > 0) {
			...
			switch (mode) {
				...
				case 3 :
					for (var i = 1; i < errCount; i++) {
						try {
							var span = $(this.ErrorItem[i]).parent().parent().next();
							span.attr("id","__ErrorMessagePanel").css("color","red").html($(this.ErrorItem[i]).attr("msg"));
	...
	ClearState : function(elem,mode) {
		mode = mode || 1;
		if(mode==3){
			var span=$(elem).parent().parent().next();
			if (span.css("color") == "rgb(255, 0, 0)"||span.css("color") == "red")
					span.css("color","#005b90");
			span.html("*验证通过");
		}else{
			with (elem) {
				if (style.color == "red")
	...
	AddErrorEm : function(em) {
		try {
			var span = $(em).next();
			alert(span.html());
			span.attr("id","__ErrorMessagePanel").css("color","red").html($(em).attr("msg"));
			/*span.id = "__ErrorMessagePanel";
			span.style.color = "red";
			this.ErrorItem[i].parentNode.appendChild(span);
			span.innerHTML = this.ErrorMessage[i].replace(
	...
	AddError : function(index, str) {
		...
		this.ErrorMessage[this.ErrorMessage.length] = (this.ErrorMessage.length+1)
				+ " : " + str;
	},
