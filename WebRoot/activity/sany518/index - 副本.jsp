...
try{
	...
	firstPro=dbHelper.getMap(" select top 1 order_state from activity_sany_pay where product_id=21004 and open_id='"+openId+"' order by order_state desc, id desc ",connection);
	if(firstPro!=null){
		firstState=CommonString.getFormatPara(firstPro.get("order_state"));
	}
	secPro=dbHelper.getMap(" select top 1 order_state from activity_sany_pay where product_id=21005 and open_id='"+openId+"' order by order_state desc, id desc ",connection);
	if(secPro!=null){
	...
	<div class="wrap tc">
		...
		<div class="pro_text">
			...
			<div class="button_box MobilePush">
				...
				%>
				<a href="payAll.jsp?productId=21004&productName=SY16C&productImg=http://product.21-sun.com/activity/sany518/images/machine1_03.jpg&headUrl=<%=headimgurl%>&nickName=<%=nickname%>&openId=<%=openId%>"  class="def">定金 ￥2000 定金可退</a>
				<%}%>
			</div>
	...
	<div class="wrap tc">
		...
		<div class="pro_text">
			...
			<div class="button_box MobilePush">
				...
				<%
				if(!secState.equals("2")){
				%>
				<a href="payAll.jsp?productId=21005&productName=SY35U&productImg=http://product.21-sun.com/activity/sany518/images/machine2_07.jpg&headUrl=<%=headimgurl%>&nickName=<%=nickname%>&openId=<%=openId%>" class="def"> 定金 ￥2000 定金可退</a>
				<%}%>
			</div>
			<div class="message fix">
