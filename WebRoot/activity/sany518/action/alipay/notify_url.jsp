		...
		//if(verify_result){//验证成功
			...
			if(trade_status.equals("TRADE_FINISHED")||trade_status.equals("TRADE_SUCCESS")){
				...
				try{
					...
					Map m=dbHelper.getMap(" select top 1 * from activity_sany_pay where order_no2='"+out_trade_no+"'  ",conn);
					if(m!=null){
						...
						dbHelper.execute("insert into activity_sany_pay_log (parent_id,source,add_date,product_name,price,order_state,open_id) "
								+" values ("+CommonString.getFormatInt(CommonString.getFormatPara(m.get("id")))+",'zfb',getDate(),'"+CommonString.getFormatPara(m.get("product_name"))+"',"+total_amount+",2,'"+CommonString.getFormatPara(m.get("open_id"))+"') ");
					}
