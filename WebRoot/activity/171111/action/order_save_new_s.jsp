...
<%@page import="java.net.URLEncoder"%><%
	...
	//获取询价的session信息
	int inquriyCount=0;Integer keyId =0;
	...
	try{
		...
		if (null != ids && ids.length() > 0) {// 删除
			...
			if (null != idStr && !"".equals(idStr)) {// 修改
				...
				keyId = dbHelper.executeUpdate(sql.toString(), values.toArray(),connection) ; // 
				zhu_jian = keyId;
				...
				if(null!=keyId&&keyId>0){
					...
					%>
                    	<script type="text/javascript"> 
							...
							top.jQuery.jBox.tip("询价成功，快去抽奖吧！") ;
							//alert("成功");
							//location.reload();
		...
		session.setAttribute("order_phone", telephone); 
		session.setAttribute("order_name", zd_name); 
	}
	//如果有后置事件
