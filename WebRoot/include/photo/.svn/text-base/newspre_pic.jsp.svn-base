...
<%
			...
			String do_sql ="select top 1 id,title,image,keywords,brand_keywords  from pro_photo_group where is_show=1  and id <"+id+" and sort_num like '%"+type+"%' order by id asc";
		    proList = dbHelper.getMapList(do_sql) ;
			if(null!=proList && proList.size()>0){
			  for(Map oneMap:proList){
			  %>
				<span>上一图集：</span>
			    <a title='<%=CommonString.getFormatPara(oneMap.get("title"))%>' href='ndetail_for_<%=CommonString.getFormatPara(oneMap.get("id"))%>.htm'>
