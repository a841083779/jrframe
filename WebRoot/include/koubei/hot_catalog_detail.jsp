<%@ page language="java" import="java.sql.Connection,java.util.*,com.jerehnet.util.common.*,com.jerehnet.util.dbutil.*" pageEncoding="UTF-8"%><%@ taglib tagdir="/WEB-INF/tags" prefix="tags" %><%@ taglib uri="/WEB-INF/classes/oscache.tld" prefix="cache"%><%	
	...
	try{
	    ...
	    String sql = " select substring(catalognum,1,6) as catalognum from pro_products where  catalognum is not null and catalognum !='' group by substring(catalognum,1,6) order by catalognum asc ";
...
<%
	  ...
	  %>
	      <li><a title="<%=catalogName%>" href="/<%=catalognum%>-0-0.htm"><%=catalogName%></a></li>
      <%
			}
		}
	  %> 
