...
<%
	List<Map> list = dbHelper.getMapList(sql,connection);
	  ...
	  %>
	   <li class="top3">
          <h4><a target="_blank" href="http://koubei.21-sun.com/list/<%=CommonString.getFormatPara(list.get(i).get("id")) %>.htm" title="<%=factoryname %><%=name %><%=catalogname %>口碑"><%=factoryname %><%=name %><%=catalogname %>口碑</a></h4>
        </li>
      <%
			}
		}
