...
<%
	...
	agentsList = dbHelper.getMapList(" select top 16 title,id from pro_agent_news where is_approval=1 order by newid()") ;
%>
    <%
        if(null != agentsList && agentsList.size()>0){
        	for(Map oneMap:agentsList){
    %>
        <li class="listnews006_ads">·<a target="_blank" title="<%=CommonString.getFormatPara(oneMap.get("title"))%>" href="/news_<%=CommonString.getFormatPara(oneMap.get("id"))%>.htm"><%=CommonString.getFormatPara(oneMap.get("title"))%></a></li>
    <%}}%>
<!--代理商列表结束-->
</cache:cache>