...
%>
  ...
  <div class="treev1" id="treev1">
	...
	%>
        <li id="<%=list2.get(i).get("id").toString()%>" title="<%=list2.get(i).get("name").toString()%>" > <a href="<%=url%>" class="mainBrand"><big><%=list2.get(i).get("name").toString()%></big></a>
        	<ul>
			<%if((Integer)(list2.get(i).get("id"))<=16368){
			  String ids="/include/prolist/prolist_left_"+list2.get(i).get("id").toString()+".jsp";
			%>
                 <jsp:include page="<%=ids%>" />
			<%}%>
			</ul>
        </li>
        <%
