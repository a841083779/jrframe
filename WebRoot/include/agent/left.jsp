...
<div class="w210 l">
  ...
  <div class="smartFloat" id="smartFloat">
    ...
    <ul class="idTabs">
      ...
      <li><a href="/inquiry/" target="_blank">订单</a></li>
    </ul>
  </div>
    ...
    <div class="treev1" id="treev1" style="height: 551px;*height:551px; max-height: none;">
      ...
      %>
          <li id="<%=list2.get(i).get("id").toString()%>" title="<%=list2.get(i).get("name").toString()%>">
              <a href="<%=url%>" class="mainBrand"><big><%=list2.get(i).get("name").toString()%></big></a>
    						...
    						for(Map m1 : sub_catalogs){
    							url = "";
    					      	...
    					      	if(!"".equals(url)){
    					      		...
    					      		url += "&catalogid="+m1.get("num");
    					      	}else{
    					      		url = "/agent/list.jsp?catalogid="+m1.get("num");
    					      	}
    					      	
    							%><li><a href="<%=url %>"><%=m1.get("name") %></a></li><%
    						}
    						%></ul><%
    					}
    				%>
