...
<%
	...
	String where="";
	if(!province.equals("")){
	where+=" and (address like '%"+province+"%' or city like '%"+province+"%' )";
	}
	if(!city.equals("")){
	where+=" and (address like '%"+city+"%' or city like '%"+city+"%' )";
	}
      ...
      <div class="fix jxs_search" id="removeAll">
        ...
        <div class="jxs_search_result fix">
            ...
            <div class="pr fix jxs_filter">
              <select style="width:98px;"  name="zd_province" id="zd_province" onChange="set_my_city(this);" >
			  <%
             ...
             <select  name="zd_city" id="zd_city"  style="width:100px;"  onChange="getNewsMessage('<%=factoryid%>')">
...
jQuery(function(){
    ...
    function getNewsMessage(a){
	var provinceid = document.getElementById("zd_province").options[document.getElementById("zd_province").selectedIndex].getAttribute("area_id");
	var province=$("#zd_province").val();
	var city=$("#zd_city").val();
	var factoryid=a;
	$.post("/tools/getNewsMessage.jsp",{provinceid:provinceid,province:province,city:city,factoryid:factoryid},function(data){
		$("#removeAll").html(data);
		});
