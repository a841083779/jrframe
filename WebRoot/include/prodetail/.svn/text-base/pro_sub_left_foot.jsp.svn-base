...
<%
	for(int i = 0;list!=null && i<list.size();i++){		  
	  ...
	  %>
<li><a target="_blank" href="http://product.21-sun.com/products/prolist.jsp?factory=<%=factoryid%>&catalog=<%=CommonString.getFormatPara(list.get(i).get("catalognum"))%>" title="<%=factoryname%><%=catalogName%>"><%=factoryname%><%=catalogName%></a></li>
<%
		}
	  }
	  %>
