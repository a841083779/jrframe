	...
	try{
		connection = dbHelper.getConnection();
 		if(!"".equals(productid) && !"undefined".equals(productid)){
		 	one_product = dbHelper.getMap(" select id,name, factoryid, factoryname, catalognum,catalogname_spe, catalogname  as  catalogname from pro_products where id='"+productid+"'",connection) ;
		}
 
		if(one_product.size()==0&&!"".equals(factoryid)&&!"".equals("undefined")){
			one_product = dbHelper.getMap(" select top 1 id,name,factoryid,factoryname,catalognum,catalogname as  catalogname from pro_products where factoryid='"+factoryid+"'",connection) ;
		}
...
<div class="top_t1"> 想了解二手<%=CommonString.getFormatPara(one_product.get("factoryname"))%><%=frombrand.equals("yes")?"产品":CommonString.getFormatPara(one_product.get("name"))+CommonString.getFormatPara(one_product.get("catalogname")) %>最低价？请填写如下信息：
</div>
<div class="txd_xunjia mt15" id="prodetailOrder">
  <div class="txj_main fix" style="height:500px;">
    <div class="_form" style="width:330px;">
      <form action="/action/order_save_new_e.jsp" method="post" name="theform" id="theform" onSubmit="return submitYN(this);">
        ...
        <input name = "zd_product_name" type="hidden" id="zd_product_name" value="<%=frombrand.equals("yes")?"":CommonString.getFormatPara(one_product.get("name"))%>(二手)"/>
	    ...
	    <input name="zd_source" type="hidden" id="zd_source"  />
	...
	<div class="r" style="width:40%;  text-align:center;float:left">
  <img src="/images/xiaov.jpg" width="96" height="96">  &nbsp; 
  <p style=" width:136px; margin:0 auto; text-align:center;line-height: 20px;"> 加客服MM微信<br>邀请进群<br>精准对接行业老板</p>
  </div>
  </div>
</div>
...
<script>
   jQuery("#zd_message").val("您好，我对<%=CommonString.getFormatPara(one_product.get("factoryname"))%><%=frombrand.equals("yes")?"产品":CommonString.getFormatPara(one_product.get("name"))+CommonString.getFormatPara(one_product.get("catalogname")) %>感兴趣，想要咨询，请您给我回复，谢谢。");
	...
	var mobile_phone_reg = /^1[3456879][0-9]\d{8}$/
...
//把下过订单的信息取出
jQuery(function(){
	var order_name = jQuery.trim($.cookies.get("order_name"));  // 订单人
    var order_phone = jQuery.trim($.cookies.get("order_phone"));  // 订单电话

	    var source = jQuery.trim($.cookies.get("source")) ;  // 订单电话
