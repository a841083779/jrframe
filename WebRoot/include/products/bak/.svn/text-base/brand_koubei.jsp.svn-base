       	...
       	while(iter01.hasNext())
       	{
       		...
       		List oneList = (null==(ArrayList)oneMap.get("A-B-C-D-E-F-G"))?new ArrayList():(ArrayList)oneMap.get("A-B-C-D-E-F-G") ;
                ...
                while(null !=oneIter&&oneIter.hasNext())
                {
               	  ...
               	  {
               		  if(flag.equals("koubei")){
       %>
        <a href="<%=url%><%=catalog.equals("")?"0":catalog %>-<%=l.get(k+1)%>-0-0-0.htm" target="_top"><%=l.get(k) %></a>
       <%        			  
               		  }else{
      %>
         <a href="<%=url%>?factory=<%=l.get(k+1)%><%=catalog.equals("")?"":"&catalog="+catalog%><%="".equals(tonandmeter)?"":"&tonandmeter="+tonandmeter+""%><%="".equals(order)?"\"":"&order="+order+"\"" %> target="_top"><%=l.get(k) %></a>
      <%
               		  }
               		}
                }
