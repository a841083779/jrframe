...
<%
	...
	List<Map> oneproduct  = null ;
	if(!"".equals(productid)){
	 	 oneproduct = dbHelper.getMapList(" select factoryid,factoryname,catalogname,name,file_name,tonandmeter,catalognum from pro_products where id in ("+productid+") ") ;
	}
...
return ;
	...
	brandname = CommonString.getFormatPara(oneproduct.get(0).get("factoryname")) ;
	tonandmeter = CommonString.getFormatPara(oneproduct.get(0).get("tonandmeter")) ;
	otherchosetj += " and pro1.factoryid=174  and pro1.catalognum like '%"+CommonString.getFormatPara(oneproduct.get(0).get("catalognum"))+"%' ";
	// 根据开通订单推荐的代理商，优先推荐出产品，(条件:先根据区域选择)
	otherProducts = OrderBusiness.getRecommendProducts(province,CommonString.getFormatPara(oneproduct.get(0).get("catalognum"))) ;//代理商管理中开通产品推荐合同，目前与营销宝冲突，该业务应该取消
	...
	if(!"".equals(tonandmeter) &&(null==otherProducts ||otherProducts.size()<3)){
		...
		sql += " INNER JOIN pro_products pp ON pp.id = paap.product_id AND pp.catalognum LIKE '"+CommonString.getFormatPara(oneproduct.get(0).get("catalognum"))+"%' ";
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="95%" style="margin-top:0px;">
          <tr>
            ...
            <td valign="top"><table border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-top:20px;">
                ...
                <tr>
                  <td style="padding-top:10px; text-align:left">询问了
                	<%
                		for(Map p : oneproduct){
                	%>
                	<a href="/proDetail/<%=CommonString.getFormatPara(p.get("file_name")) %>" target="_blank"><font color="#FF0000"><strong><%=CommonString.getFormatPara(p.get("factoryname")) %><%=CommonString.getFormatPara(p.get("name")) %><%=CommonString.getFormatPara(p.get("catalogname")) %> </strong></font></a>&nbsp;&nbsp;&nbsp;&nbsp;
                	<%
                		}
                	%>  
                  的网友还询问了以下类似产品：
                      ...
                      <tr>
                        <%
							if(null != otherProducts && otherProducts.size()==3){//代理商管理中未开通产品推荐合同&&list长度小于两个推荐产品
								...
								Map recomMap = null ;
								if(CommonString.getFormatPara(oneproduct.get(0).get("catalognum")).indexOf("101001")!=-1 && !"".equals(oneproduct.get(0).get("tonandmeter"))){ // 如果询价产品为挖掘机
									...
									String whereStr = " and factoryid=174 and catalognum like '"+CommonString.getFormatPara(oneproduct.get(0).get("catalognum"))+"%' " ;
									String tonandmeterStr = " and tonandmeter between "+(Float.parseFloat(CommonString.getFormatPara(oneproduct.get(0).get("tonandmeter")))-10)+" and "+(Float.parseFloat(CommonString.getFormatPara(oneproduct.get(0).get("tonandmeter")))+10) ;
								...
								// 推土机推荐（随机）
								if(CommonString.getFormatPara(oneproduct.get(0).get("catalognum")).indexOf("101003")!=-1){
									String sel_sql = " select top 1 img2,factoryid,factoryname,catalogname,name,file_name,tonandmeter,catalognum,id as productid from pro_products where 1=1 " ;
									String whereStr = " and factoryid=174 and catalognum like '"+CommonString.getFormatPara(oneproduct.get(0).get("catalognum"))+"%' " ;
									String orderBy = " order by newid()" ;
									recomMap = dbHelper.getMap(sel_sql+whereStr+orderBy) ;
								...
								//四川黑金刚工程机械有限责任公司 
								if(CommonString.getFormatPara(oneproduct.get(0).get("catalognum")).indexOf("113001")!=-1){
									String sel_sql = " select top 1 img2,factoryid,factoryname,catalogname,name,file_name,tonandmeter,catalognum,id as productid from pro_products where 1=1 " ;
    							...
    							// 备注：山工只有轮式装载机
								if(CommonString.getFormatPara(oneproduct.get(0).get("catalognum")).indexOf("101002")!=-1){
									String sel_sql = " select top 1 img2,factoryid,factoryname,catalogname,name,file_name,tonandmeter,catalognum,id as productid from pro_products where 1=1 " ;
									String whereStr = " and factoryid=138 and catalognum like '"+CommonString.getFormatPara(oneproduct.get(0).get("catalognum"))+"%' " ;
									String orderBy = " order by newid()" ;
									recomMap = dbHelper.getMap(sel_sql+whereStr+orderBy) ;
									if(null==recomMap){
										recomMap = dbHelper.getMap(sel_sql+whereStr+orderBy) ;
                         ...
                         <div class="xjList">
                           ...
                           <div class="xjInfo">
                              品牌：<%=CommonString.getFormatPara(oneMap.get("factoryname")) %><br />
                                ...
                                </a> 类别：<%=CommonString.getFormatPara(catalogMap.get(CommonString.getFormatPara(oneMap.get("catalognum")))).equals("")?CommonString.getFormatPara(oneproduct.get(0).get("catalogname")):CommonString.getFormatPara(catalogMap.get(CommonString.getFormatPara(oneMap.get("catalognum"))))  %><br/>
