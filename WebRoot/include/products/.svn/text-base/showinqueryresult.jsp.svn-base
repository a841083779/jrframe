...
return ;
	...
	if(!"".equals(tonandmeter) && otherProductsSize<4){//list长度小于两个推荐产品，吨位不为空，查询左右浮动5吨的产品（较精确）
		...
		tempotherProducts = dbHelper.getMapList("select top "+(4-otherProductsSize)+" pro1.id productid,pro2.factoryid,pro2.factoryname,pro2.catalognum,pro2.name,pro2.file_name,pro2.img2 from pro_products pro1 left join pro_products pro2 on pro1.id=pro2.id "+temptj+" order by newid()") ;
...
.top_t1 { font-size: 24px; height: 115px; line-height: 115px; font-weight: bold; border-bottom: 1px dashed #f1f1f2 }
.top_t1 .img { vertical-align: middle; margin-right: 10px; }
.list_t { font-size: 16px; font-weight: bold; margin-bottom: 30px; }
.list_t span { color: #fe6000 }
.tj li { float: left; width: 50%; margin-bottom: 25px; }
...
.orderbg{ padding-bottom:5px; border-bottom:1px solid #dfdfdf; margin-bottom:15px;}
.orderbg .img { position: absolute; right: 20px; bottom: 0px; width: 210px; height: 45px; }
...
<div class="top_t1"><img src="images/succ1.png" width="74" height="77" class="img">您的询价信息已成功提交，我们将及时联系您进行报价！</div>
<div class="orderbg fix"> <img src="/images/xj_tel.png" alt="客服热线" class="img" />
  ...
  <div class="l" style=" width:58%">
    <ul class="fix tj">
							...
							%>
      <li><img src="/uploadfiles/<%=CommonString.getFormatPara(oneMap.get("img2"))%>"  onerror="showImgDelay(this,'/uploadfiles/no_small.gif',2);" width="103" height="84" class="l"><span>品牌：<%=CommonString.getFormatPara(oneMap.get("factoryname")) %></span>
	  <span>型号：<%=model_name %></span><span>类别：<%=CommonString.getFormatPara(catalogMap.get(CommonString.getFormatPara(oneMap.get("catalognum")))).equals("")?CommonString.getFormatPara(oneproduct.get("catalogname")):CommonString.getFormatPara(catalogMap.get(CommonString.getFormatPara(oneMap.get("catalognum"))))  %></span><span class="n">
        <input type="checkbox" value="<%=CommonString.getFormatPara(oneMap.get("productid")) %>" <%if(buy_factoryid.equals("144")&&CommonString.getFormatPara(oneMap.get("catalognum")).indexOf("101002")!=-1){%> <%}else{%>checked="true"<%}%>>
        请点击选择</span></li>
      <%i++;}%>
    </ul>
  </div>
  <div class="r" style="width:40%; border-left:1px solid #e6e6e6; text-align:center">
  <img src="images/erwei.png" width="136" height="135">  &nbsp; 
  <p style=" width:136px; margin:0 auto; text-align:center">铁臂商城手机版<br>随时随地询价<br>QQ群:250469589</p>
  </div>
</div>
