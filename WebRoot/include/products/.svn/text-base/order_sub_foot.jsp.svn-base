...
<li>
  ...
  java.text.SimpleDateFormat formatter = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm");
   ...
   String str_date2=CommonDate.getFormatDate("yyyy-MM-dd HH:mm",list.get(i).get("add_date"));//需求时间
   String str_date1 = formatter.format(currentTime);
   
	Date d1 =formatter.parse(str_date1);
    Date d2 = formatter.parse(str_date2);
	
	long diff = d1.getTime() - d2.getTime();
    long days = diff / (60*1000 );
	
	String times="";
	if(days>60){times=CommonDate.getFormatDate("MM-dd",list.get(i).get("add_date"));}else{
	times=days+"分钟前";
	}
  %>
