...
<%=CommonString.getFormatPara(one_product.get("factoryname"))%><%=frombrand.equals("yes")?"产品":CommonString.getFormatPara(one_product.get("name"))+CommonString.getFormatPara(one_product.get("catalogname")) %>在线询价<!--<font style="color:grey">（成功询价后，我们会24小时之内与您联系！）</font>--></div>
    ...
    <table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0" class="list_border_bg">
     ...
     <td><select name="zd_province" id="zd_province" onChange="set_my_city(this);" style="width:87px;"  dataType="Require"  msg="请选择省份">
      ...
      <tr>
        ...
        <td height="22" colspan="3" class="list_cell_bg" style="padding:0 0 10px 0;"><textarea class="input03" name="zd_message" cols="77" rows="5"  id="zd_message"style="color:#ccc"></textarea></td>
      </tr>
      <tr>
	  <!--
        <th>验证码:</th>
...
function submitYN(event) {
	...
	if(jQuery("#zd_buy_type").val()==''){
			jQuery.jBox.tip("请选择付款方式") ;
