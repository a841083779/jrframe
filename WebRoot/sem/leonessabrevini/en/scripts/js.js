...
jQuery(document).ready(function(){	
	count=jQuery("http://www.leonessabrevini.com/en/scripts/.b_ .blc").length;
	jQuery(".b_ .blc:not(:first-child)").hide();
	jQuery(".button a").click(function() {
		var i = jQuery(this).text() - 1;
		n = i;
		if (i >= count) return;
		jQuery("http://www.leonessabrevini.com/en/scripts/.b_ .blc").filter(":visible").fadeOut(500).parent().children().eq(i).fadeIn(1000);
