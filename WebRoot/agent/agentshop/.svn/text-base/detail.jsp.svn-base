<%@ page language="java" import="com.jerehnet.util.dbutil.PageBean,java.util.*,com.jerehnet.util.common.*,com.jerehnet.util.dbutil.*,java.sql.*" pageEncoding="UTF-8"%><%@ taglib tagdir="/WEB-INF/tags" prefix="tags" %><%
	...
	String title = "代理商首页_"+CommonString.getFormatPara(map.get("full_name")) +"-中国工程机械商贸网";
	String keywords = "代理商首页,"+CommonString.getFormatPara(map.get("full_name"))+",工程机械" ;
    ...
    <div class="shopRight"> 
      ...
      <div class="sRightPart">
        <div class="sTitle" id="recommendProT">
          <h2>推荐产品</h2>
          <span class="more"><a href="/agentshop/<%=usern%>/agent_products.htm">查看更多产品</a></span>
         </div>
      ...
      <div class="sRightPart">
        <div class="sTitle">
          <h2>主营产品</h2>
          <span class="more"><a href="/agentshop/<%=usern%>/agent_products.htm">查看更多产品</a></span> </div>
      ...
      <%
			...
			if(usern.equals("ytht")){//代理商宏通特殊判断订单条件，对应品牌、类别、区域
				whereStr = " and factoryid in(133) and ( (catanum like '101001%' and (contact_address like'%烟台%' or contact_address like'%威海%' or contact_address like'%青岛%' or contact_address like'%潍坊%' or contact_address like'%济南%' or contact_address like'%东营%' or contact_address like'%滨州%')) or (catanum like '102%' and (contact_address like'%云南%' or contact_address like'%山东%')) or (catanum like '103%' and (contact_address like'%烟台%' or contact_address like'%威海%' or contact_address like'%青岛%' or contact_address like'%潍坊%' or contact_address like'%日照%' or contact_address like'%临沂%' or contact_address like'%济宁%' or contact_address like'%枣庄%')) or (catanum like '105003%' and (contact_address like'%河南%' or contact_address like'%山东%')) )";	 
			}
	...
	%>
      ...
      <div class="wfull l">
        <div id="pagination" class="page" style="<%=pageBean.getPageCount()>1?"margin-right:10px;":"display:none;" %>">
          <tags:page pageBean="<%=pageBean %>" url="/agentshop/detail.jsp"/>
        </div>
      </div>
      <%
		}
