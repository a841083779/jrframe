...
<meta name="mobile-agent" content="format=html5;url=http://m.product.21-sun.com/agent_detail.jsp?agent_id=<%=CommonString.getFormatPara(map.get("id"))%>">
...
<div class="contain980 mb10">
  <div class="asiteTop mb10">
    <div class="wfull l">
      <div class="comInfo">
        ...
        <span style='<%=(CommonString.getFormatPara(map.get("is_qijiandian")).equals("1")&&CommonString.getFormatPara(map.get("is_made")).equals("1"))?"":"display:none;" %>' class="soSpan" onclick="window.open('/agentstore/<%=CommonString.getFormatPara(map.get("usern")) %>/')">进入<%=CommonString.getFormatPara(map.get("name"))%>官方旗舰店</span>
        <span class="r">
        <%if(!CommonString.getFormatPara(map.get("logo")).equals("")){%>
        <img src="/uploadfiles/<%=CommonString.getFormatPara(map.get("logo"))%>" width="172" height="46" />
        <%}%>
        </span> </div>
      ...
      %>
        <li class="anSelect"><a href="/agent/<%=usern%>/">代理商首页</a></li>
  ...
  <div class="w697 r"> 
    ...
    <div class="w695 border03 l mb10">
      ...
      <div class="wfull l agentIntro">
        <div class="agentimg"><img src="/uploadfiles/<%=CommonString.getFormatPara(map.get("img1"))%>" width="195" height="145"   onerror="showImgDelay(this,'/uploadfiles/no_big.gif',2);"/></div>
        <div class="agentIntroText"> 　　<%=(CommonHtml.filterHTML(CommonString.getFormatPara(map.get("introduce"))).length()>=220)?CommonHtml.filterHTML(CommonString.getFormatPara(map.get("introduce"))).substring(0,220)+"...":CommonHtml.filterHTML(CommonString.getFormatPara(map.get("introduce")))%><b><a href="/agent/<%=usern%>/introduce.shtm">[查看详情]</a></b></div>
      </div>
    </div>
    <!--代理商介绍结束--> 
    <!--代理产品-->
    <jsp:include page="/include/agent/agent_products.jsp" flush="true">
    <jsp:param name="agentid" value='<%=CommonString.getFormatPara(map.get("id"))%>'></jsp:param>
    <jsp:param name="usern" value='<%=usern%>'></jsp:param>
    </jsp:include>
    <!--代理产品结束--> 
    <!--免费询价-->
    <div class="w695 border03 l"> 
      <!--免费询价-->
      <div style="font-size:14px; height:15px; line-height:12px; padding:10px 0px 10px 15px;font-weight:bold; background:#f1f1f1"> 在线询价</div>
        ...
        <td nowrap="nowrap" class="list_cell_bg">
          ...
          *
            ...
            <tr>
              ...
              <td height="22" colspan="3" style="padding-top:5px; padding-left:10px;"><input class="orderBtn01" name="Submit" type="submit" value="提交"/>
...
<script language="javascript" type="text/javascript">
	...
	jQuery.ajax({
		...
		type:"post",
		data:{"flag":"brandAgentCatalogList","agentid":"<%=CommonString.getFormatPara(map.get("id"))%>"},
