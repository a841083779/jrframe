...
<%
	String tableName = "pro_product_pic";
...
%>
  <div class="single_img" id="<%=model.get("uuid") %>" title="<%=model.get("name") %>">
    <div><img class="show_img" width="130px;" src="/uploadfiles/<%=model.get("img_name") %>" /></div>
    <div class="toobar">
...
<script type="text/javascript">
	...
	function doSort(){
		...
		jQuery.ajax({
			...
			type:"post",
			data:{'flag':'saveSort',"tableName":"pro_product_pic","sortId":sortHtml},
			async:false,
			cache:false
		});
	}
	...
	var flashOptions = {
		...
		url:'/admin/action/upload.jsp',                                           // 上传处理页面的url地址
		ext:'{}',                                 //可向服务器提交的自定义参数列表
	...
	function saveImgToDb(){
		...
		doAjax("/admin/action/ajax_cud.jsp",data,function(data){
	...
	jQuery(".miaoshu").blur(function(){
		...
		if(oldval!=selfval){
			jQuery.post("/admin/action/ajax_cud.jsp",{
				'flag':'commonUpdate',
	...
	jQuery("#del_img").click(function(){
		...
		jrConfirm("确定要删除选中的"+blue.length+"张图片吗？",function(v){
			if(v){
				...
				jQuery.ajax({
					url:'/admin/action/ajax_cud.jsp',
					data:{'flag':'groupDel','ids':ids,'tableName':'pro_product_pic'},
					type:'post',
					async:false,
