...
<%
	...
	try{
		...
		if(null==factoryInfo){
			...
			factoryInfo = dbHelper.getMap(" select * from pro_agent_factory where uuid = ? ",new Object [] {uuid},connection);
		}
		...
		while (itr.hasNext()) {
			...
			if (!item.isFormField()) {
				if(null!=factoryInfo){
					params.add(factoryInfo.get("usern"));
				}else{
					params.add("");
				}
				...
				sql = " insert into "+table_prefix+"common_upload ( add_user , add_date , add_ip , uuid , title , description , old_name , new_name , small_new_name , file_size , file_type ) ";
				...
				if(null!=factoryInfo){
					...
					postMap.put("tableName",table_prefix+"common_upload");
