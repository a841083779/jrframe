<%@ page language="java" import="java.sql.Connection,com.jerehnet.webservice.*,org.json.*,java.util.*,com.jerehnet.util.common.*,com.jerehnet.util.dbutil.*,java.util.regex.*" pageEncoding="UTF-8"%><%@ taglib tagdir="/WEB-INF/tags" prefix="tags"%><%
	...
	StringBuffer whereStr = new StringBuffer(" and is_show=1 and img2 is not null and catalognum like '"+catalognum+"%'") ;
...
<title><%=CommonString.getFormatPara(brandMap.get(factoryid))%><%=catalogname %>图片_<%=CommonString.getFormatPara(brandMap.get(factoryid))%><%=catalogname %>图片大全 – 铁臂<%=catalogname %>专区</title>
<meta name="keywords" content="<%=CommonString.getFormatPara(brandMap.get(factoryid))%><%=catalogname %>图片,<%=CommonString.getFormatPara(brandMap.get(factoryid))%><%=catalogname %>图片大全" />
<meta name="description" content="铁臂<%=catalogname %>专区,为您提供最新最全的<%=CommonString.getFormatPara(brandMap.get(factoryid))%><%=catalogname %>图片,帮助您全面了解<%=CommonString.getFormatPara(brandMap.get(factoryid))%><%=catalogname %>产品,买<%=CommonString.getFormatPara(brandMap.get(factoryid))%><%=catalogname %>产品就上铁臂商城。" />
...
<div class="s_navBar">
  <div class="contain980">
    <ul class="fix">
      ...
      <li class="selected"><a href="http://<%=usernn%>.21-sun.com/pic.htm"><%=catalogname%>图片</a></li>
...
<div class="contain980 mb10 mt10">
  <div class="breadCrumbs"><a href="/">铁臂<%=catalogname%>专区</a> &gt;&gt; <%=catalogname%>图片<%if(!factoryid.equals("")&&!factoryid.equals("0")){%> &gt;&gt; <%=CommonString.getFormatPara(brandMap.get(factoryid))%><%=catalogname%>图片<%}%></div>
</div>
<!--main-->
<div class="contain980 fix">
	<div class="s_mleft">
        <div class="comBox">
            <div class="cg mb10 cp_brandmore"> 
			  ...
			  %>
			  <a href="/pic_<%=CommonString.getFormatPara(oneMap.get("id"))%>_0.htm" title="<%=CommonString.getFormatPara(oneMap.get("name"))%>"><%=CommonString.getFormatPara(oneMap.get("name"))%></a><em>|</em>
              <%}}%>
        ...
        <div class="pcpic_val">
			...
			%>
            	<li>
                	<div class="pcpic_img"><a href="http://photo.21-sun.com/detail_for_<%=CommonString.getFormatPara(oneMap.get("id"))%>.htm" title="<%=CommonString.getFormatPara(oneMap.get("factoryname"))%><%=CommonString.getFormatPara(oneMap.get("name"))%><%=CommonString.getFormatPara(oneMap.get("catalogname"))%>" target="_blank"><img title="<%=CommonString.getFormatPara(oneMap.get("factoryname"))%><%=CommonString.getFormatPara(oneMap.get("name"))%><%=CommonString.getFormatPara(oneMap.get("catalogname"))%>" onerror="showImgDelay(this,'/uploadfiles/no_small.gif',2);" src="<%=img%>" alt="<%=CommonString.getFormatPara(oneMap.get("factoryname"))%><%=CommonString.getFormatPara(oneMap.get("name"))%><%=CommonString.getFormatPara(oneMap.get("catalogname"))%>"/></a></div>
                    <div class="pcpic_name"><a href="http://photo.21-sun.com/detail_for_<%=CommonString.getFormatPara(oneMap.get("id"))%>.htm" title="<%=CommonString.getFormatPara(oneMap.get("factoryname"))%><%=CommonString.getFormatPara(oneMap.get("name"))%><%=CommonString.getFormatPara(oneMap.get("catalogname"))%>"  target="_blank"><%=CommonString.getFormatPara(oneMap.get("factoryname"))%><%=CommonString.getFormatPara(oneMap.get("name"))%><%=CommonString.getFormatPara(oneMap.get("catalogname"))%>图片</a></div>
                </li> 
            <%}}%>
            </ul>
        </div>
		  <div class="pages">
			...
			<div id="pagination" class="paging  page" style="margin-right:3px;">
				<tags:page_catalog_pic pageBean="<%=pageBean%>" />
			</div>
	        </form>
      	</div>
    </div>
